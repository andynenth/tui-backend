...
async function testFrontendConnection() {
  ...
  try {
    ...
    console.log('  - Redeal multiplier:', backendEvent.data.phase_data.redeal_multiplier);
    ...
    console.log('  - GameService.handlePhaseChange() ✓ (processes snake_case data)');
    console.log('  - UI calculation methods ✓ (compute derived state)');
    console.log('  - Type safety ✓ (PhaseData interface updated)');
    
    console.log('\n✅ UI Component Integration:');
    console.log('  - PreparationUI ✓ (receives calculated isMyHandWeak, handValue)');
    console.log('  - DeclarationUI ✓ (receives calculated estimatedPiles, handStrength)');
    console.log('  - TurnUI ✓ (receives calculated canPlayAnyCount, selectedPlayValue)'); 
