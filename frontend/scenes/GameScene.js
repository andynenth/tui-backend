...
export class GameScene extends Container {
  ...
  async promptDeclaration() {
    ...
    console.log(`\n🟨 ${this.playerName}, declare how many piles you want to capture (options: [${options.join(', ')}]):`);
    ...
    const value = await this.showInput("Your declaration:", (input) => {
      ...
      if (!options.includes(num)) {
        return { valid: false, message: `Invalid declaration. Choose from [${options.join(', ')}]` };
      }
      return { valid: true };
    });
  ...
  async promptTurnPlay(isFirstPlayer) {
    ...
    const input = await this.showInput(prompt, (value) => {
      const indices = value.trim().split(/\s+/).map(i => parseInt(i));
      
      if (indices.some(i => isNaN(i) || i < 0 || i >= this.myHand.length)) {
        return { valid: false, message: "Invalid indices. Please check your hand." };
      }
      ...
      if (isFirstPlayer) {
        ...
        if (indices.length !== this.requiredPieceCount) {
          return { valid: false, message: `Must play exactly ${this.requiredPieceCount} pieces` };
        }
      }
