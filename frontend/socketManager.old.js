        ...
        socket.onclose = (event) => {
            ...
            if (!isIntentionalClose && reconnectAttempts < maxReconnectAttempts) {
                ...
                if (listeners["reconnecting"]) {
                    listeners["reconnecting"].forEach((fn) => fn({ 
                ...
                if (listeners["connection_failed"]) {
                    listeners["connection_failed"].forEach((fn) => fn({ 
...
export function emit(event, data) {
    if (socket && socket.readyState === WebSocket.OPEN) {
        ...
        console.warn(`⚠️ WebSocket not ready (state: ${socket?.readyState}). Cannot emit event: ${event}`);
