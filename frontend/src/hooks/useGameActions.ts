...
export function useGameActions(config: GameActionsConfig = {}): GameActions {
  ...
  const validateActionState = useCallback(
    (
      ...
      if (requiredPhase && gameState.phase !== requiredPhase) {
        console.warn(
          `Cannot ${actionName}: Wrong phase (current: ${gameState.phase}, required: ${requiredPhase})`
        );
        return false;
      }
      ...
      if (gameState.error) {
        console.warn(
          `Cannot ${actionName}: Game has error - ${gameState.error}`
        );
        return false;
      }
  ...
  // Declaration Phase Actions
  const makeDeclaration = useCallback(
    async (value: number) => {
      try {
        ...
        if (isLastPlayer) {
          const currentTotal = Object.values(gameState.declarations).reduce(
            (sum, val) => sum + val,
            0
          );
  ...
  const playPieces = useCallback(
    async (indices: number[]) => {
      try {
        ...
        if (
          gameState.requiredPieceCount !== null &&
          indices.length !== gameState.requiredPieceCount
        ) {
          throw new Error(
            `Must play exactly ${gameState.requiredPieceCount} pieces`
          );
        }
  ...
  const triggerRecovery = useCallback(
    async (errorType?: string): Promise<ErrorRecoveryResult> => {
      try {
