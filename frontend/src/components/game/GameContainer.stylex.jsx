 ...
 * 
 * Now with StyleX integration for improved performance and type-safe styling
 *
 * Features:
 ...
 * ✅ StyleX-powered animations and transitions
 */
...
// Import Pure UI Components - now all using StyleX versions
...
import DeclarationUI from './DeclarationUI.stylex';
import TurnUI from './TurnUI.stylex';
import TurnResultsUI from './TurnResultsUI.stylex';
...
const styles = stylex.create({
  ...
  phaseTransition: {
    animation: `${fadeIn} ${motion.durationSlow} ${motion.easeInOut}`,
 ...
 */
export function GameContainer({ roomId, onNavigateToLobby, className = '' }) {
  ...
  const gameOverProps = useMemo(() => {
    ...
    return {
      ...
      gameStats: gameState.game_stats || {
        total_rounds: 0,
    ...
    return (
      <ErrorBoundary>
        <div {...stylex.props(styles.loadingContainer)}>
          <WaitingUI {...waitingProps} message="Not connected to game" />
        </div>
      </ErrorBoundary>
    );
  }
          ...
          switch (gameState.phase) {
            ...
            case 'turn': {
              const turnRequirement = (() => {
                ...
                const { currentPlayer, playerName, requiredPieceCount } =
                  turnProps;
                ...
                if (!isMyTurn) {
                  return {
                    type: 'waiting',
                    text: `Waiting for ${currentPlayer} to play`,
                  };
                }
                ...
                if (requiredPieceCount === 0 || requiredPieceCount === null) {
                  return {
                    type: 'active',
                    text: 'Play any number of pieces or pass',
                  };
                }
