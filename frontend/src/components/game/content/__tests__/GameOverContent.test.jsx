...
jest.mock('../../shared/FooterTimer', () => ({
  ...
  default: ({ onComplete, duration, prefix, suffix, variant }) => (
    <div data-testid="footer-timer" data-duration={duration}>
      ...
      <button data-testid="complete-timer" onClick={onComplete}>Complete Timer</button>
    </div>
  )
}));
...
describe('GameOverContent', () => {
  ...
  describe('Basic Display Tests', () => {
    test('displays winner name correctly', () => {
      ...
      expect(winnerSection.querySelector('.go-winner-name')).toHaveTextContent('Alice');
    ...
    test('displays game statistics', () => {
      ...
      const durationItem = Array.from(document.querySelectorAll('.go-stat-item')).find(
        item => item.querySelector('.go-stat-label')?.textContent === 'Game Duration'
      );
      expect(durationItem.querySelector('.go-stat-value').textContent).toBe('45');
    });
    ...
    test('displays countdown timer', () => {
      ...
      expect(screen.getByTestId('footer-timer')).toHaveAttribute('data-duration', '10');
    });
  });
  ...
  describe('Edge Case Tests', () => {
    ...
    test('sorts players by score correctly', () => {
      ...
      const playerNamesInOrder = Array.from(rankItems).map(item => 
        item.querySelector('.go-player-name').textContent
      );
    ...
    test('formats game duration correctly for edge cases', () => {
      ...
      const { rerender } = renderComponent({ gameStats: { duration: 0, totalRounds: 1, highestScore: 0 } });
      
      // Find the duration value specifically by looking for the Game Duration label
      let durationItem = Array.from(document.querySelectorAll('.go-stat-item')).find(
        item => item.querySelector('.go-stat-label')?.textContent === 'Game Duration'
      );
      expect(durationItem).toBeTruthy();
      expect(durationItem.querySelector('.go-stat-value').textContent).toBe('0');
      ...
      durationItem = Array.from(document.querySelectorAll('.go-stat-item')).find(
        item => item.querySelector('.go-stat-label')?.textContent === 'Game Duration'
      );
      expect(durationItem).toBeTruthy();
      expect(durationItem.querySelector('.go-stat-value').textContent).toBe('1');
  ...
  describe('Accessibility Tests', () => {
    ...
    test('has proper semantic structure', () => {
      ...
      expect(document.querySelector('.go-trophy-container')).toBeInTheDocument();
      expect(document.querySelector('.go-winner-section')).toBeInTheDocument();
      expect(document.querySelector('.go-rankings-container')).toBeInTheDocument();
  ...
  describe('Integration Tests', () => {
    ...
    test('renders correctly with minimal data', () => {
      ...
      const winnerSection = document.querySelector('.go-winner-section');
      expect(winnerSection.querySelector('.go-winner-name')).toHaveTextContent('Test Player');
      expect(screen.getByText('Champion!')).toBeInTheDocument();
      
      // Check score is displayed
      const finalScores = document.querySelectorAll('.go-final-score');
      expect(Array.from(finalScores).some(el => el.textContent === '100')).toBe(true);
    });
  });
});