...
import { createMockNetworkService, createTestGameState, createTestPhaseData, createMockEventDetail, expectToThrow, waitForNextTick, mockConsole } from './testUtils';
...
describe('GameService', () => {
  ...
  describe('Singleton Pattern', () => {
    ...
    test('multiple getInstance calls return same instance', () => {
      const instances = Array.from({ length: 5 }, () => GameService.getInstance());
  ...
  describe('Room Management', () => {
    describe('joinRoom', () => {
      ...
      test('handles connection failure', async () => {
        ...
        mockNetworkService.connectToRoom.mockRejectedValue(new Error(errorMessage));
    ...
    describe('leaveRoom', () => {
      test('successfully leaves room', async () => {
        ...
        expect(mockNetworkService.disconnectFromRoom).toHaveBeenCalledWith('test-room');
  ...
  describe('Game Actions', () => {
    ...
    describe('Redeal Actions', () => {
      ...
      test('redeal actions fail in wrong phase', () => {
        ...
        expect(() => gameService.acceptRedeal()).toThrow('Invalid action ACCEPT_REDEAL for phase turn');
        expect(() => gameService.declineRedeal()).toThrow('Invalid action DECLINE_REDEAL for phase turn');
      });
    });
    ...
    describe('Declaration', () => {
      ...
      test('makeDeclaration validates value range', () => {
        expect(() => gameService.makeDeclaration(-1)).toThrow('Invalid declaration value: -1');
        expect(() => gameService.makeDeclaration(9)).toThrow('Invalid declaration value: 9');
        expect(() => gameService.makeDeclaration('invalid')).toThrow('Invalid declaration value: invalid');
      });
      ...
      test('makeDeclaration fails when not player turn', () => {
        ...
        expect(() => gameService.makeDeclaration(2)).toThrow('Not your turn to declare');
      });
      ...
      test('last player cannot make total equal 8', () => {
        ...
        expect(() => gameService.makeDeclaration(2)).toThrow('Last player cannot make total equal 8');
      });
    });
    ...
    describe('Play Pieces', () => {
      ...
      test('playPieces validates indices array', () => {
        expect(() => gameService.playPieces([])).toThrow('Must select at least one piece');
        expect(() => gameService.playPieces('invalid')).toThrow('Must select at least one piece');
        expect(() => gameService.playPieces(null)).toThrow('Must select at least one piece');
      });
      ...
      test('playPieces validates not player turn', () => {
        ...
        expect(() => gameService.playPieces([0])).toThrow('Not your turn to play');
      });

      test('playPieces validates piece indices', () => {
        expect(() => gameService.playPieces([-1])).toThrow('Invalid piece index: -1');
        expect(() => gameService.playPieces([3])).toThrow('Invalid piece index: 3');
        expect(() => gameService.playPieces([0, 1, 5])).toThrow('Invalid piece index: 5');
      });
      ...
      test('playPieces fails in wrong phase', () => {
        ...
        expect(() => gameService.playPieces([0])).toThrow('Invalid action PLAY_PIECES for phase declaration');
      });
    });
  });
  ...
  describe('Event Handling', () => {
    ...
    test('processes error events', async () => {
      ...
      const eventDetail = createMockEventDetail('error', { message: errorMessage });
  ...
  describe('Service Lifecycle', () => {
    ...
    test('operations fail after destroy', async () => {
      ...
      expect(() => gameService.getState()).toThrow('GameService has been destroyed');
    });
  });
});