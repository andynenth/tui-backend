...
type UserNotificationHandler = (message: string, severity: ErrorSeverityType) => void;
...
export class ErrorHandlingService {
  ...
  public getRetryStrategy(error: StandardError): RetryStrategy | null {
    ...
    return this.retryStrategies.get(error.code) || {
  ...
  public async withRetry<T>(
    ...
    while (attempt === 0) { // First attempt always runs
      try {
  ...
  private calculateRetryDelay(strategy: RetryStrategy, attempt: number): number {
    const baseDelay = strategy.backoffMs[Math.min(attempt, strategy.backoffMs.length - 1)];
   ...
   */
  public handleWebSocketError(message: { event: string; data: StandardErrorData }): StandardError {
