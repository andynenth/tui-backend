...
export class SocketManager {
    ...
    send(event, data = {}) {
        ...
        if (this.enableMessageQueue) {
            ...
            this.eventEmitter.emit('message_failed', { message, reason: 'Not connected' });
        }
    ...
    _handleMessage(data) {
        ...
        if (data.event === 'pong') {
            this.monitor.recordPong(data.data.timestamp);
            return;
        }
    ...
    _handleClose(event) {
        ...
        if (this.enableReconnection && !this.isIntentionalDisconnect && wasConnected) {
    ...
    async _attemptReconnection() {
        ...
        this.eventEmitter.emit('reconnecting', this.reconnectionManager.getStatus());
