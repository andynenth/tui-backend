...
async def test_json_serialization():
    ...
    await turn_state.update_phase_data({
        'turn_plays': {
            'TestPlayer': {
                'pieces': [Piece("GENERAL_RED")],  # This should be converted to string
                'piece_count': 1
            }
        },
        'current_player': 'Bot1'
    }, "JSON serialization test", broadcast=False)  # Don't actually broadcast
...
async def test_no_duplicate_notifications():
    ...
    try:
        ...
        if notification_count <= 1:
            print(f"✅ No duplicate notifications - got {notification_count} notification(s)")
            return True
        else:
            print(f"❌ Duplicate notifications detected - got {notification_count} notifications")
            return False
