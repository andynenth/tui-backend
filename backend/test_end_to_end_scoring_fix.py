...
class MockReactRenderer:
    ...
    def render_value(value, path="root"):
        ...
        if isinstance(value, dict):
            raise ReactError31(
                f"Objects are not valid as a React child at {path}. Object keys: {list(value.keys())}"
            )
            ...
            raise ReactError31(
                f"Complex object not renderable at {path}: {type(value)}"
            )
        else:
...
class FrontendGameServiceSimulator:
    ...
    def __init__(self):
        self.state = {"roundScores": {}, "totalScores": {}, "playersWithScores": []}

    def handlePhaseChange(self, phase_data: Dict[str, Any]):
        """Simulate GameService.ts handlePhaseChange for scoring phase"""

        print(
            f"🔄 FRONTEND_SIM: Processing phase_change with data keys: {list(phase_data.keys())}"
        )
        ...
        if True:  # Simulate case 'scoring':
            ...
            print(
                f"🔍 FRONTEND_SIM: Raw round_scores sample: {dict(list(rawRoundScores.items())[:1]) if rawRoundScores else {}}"
            )

            for playerName, scoreData in rawRoundScores.items():
                print(
                    f"🔍 FRONTEND_SIM: Processing {playerName}: {type(scoreData)} = {scoreData}"
                )
            ...
            print(
                f"🔍 FRONTEND_SIM: Raw total_scores sample: {dict(list(rawTotalScores.items())[:1]) if rawTotalScores else {}}"
            )

            for playerName, scoreData in rawTotalScores.items():
                print(
                    f"🔍 FRONTEND_SIM: Processing total {playerName}: {type(scoreData)} = {scoreData}"
                )
            ...
            if (
                phase_data.get("players")
                and phase_data.get("round_scores")
                and phase_data.get("total_scores")
            ):
                ...
                print(
                    f"✅ FRONTEND_SIM: Generated {len(self.state['playersWithScores'])} playersWithScores"
                )

            print(
                f"✅ FRONTEND_SIM: Final state roundScores: {self.state['roundScores']}"
            )
            print(
                f"✅ FRONTEND_SIM: Final state totalScores: {self.state['totalScores']}"
            )

    def calculatePlayersWithScores(
        self, players, roundScores, totalScores, redealMultiplier, winners
    ):
        ...
        for player in players:
            ...
            baseScore = (
                playerRoundScore.get("base_score")
                if isinstance(playerRoundScore, dict)
                else round(roundScore / max(1, redealMultiplier))
            )

            actualPiles = (
                playerRoundScore.get("actual")
                if isinstance(playerRoundScore, dict)
                else 0
            )  # Simplified
...
class MockScoringUI:
    ...
    def render(roundScores, totalScores, playersWithScores, players, winners=[]):
        ...
        if len(playersWithScores) > 0:
            ...
            for player in players:
                player_name = (
                    player["name"] if isinstance(player, dict) else player.name
                )
                ...
                print(
                    f"🖥️  Player {player_name}: roundScore={type(roundScore)}({roundScore}), totalScore={type(totalScore)}({totalScore})"
                )
        ...
        for i, player in enumerate(sortedPlayers):
            ...
            MockReactRenderer.render_value(
                player.get("roundScore"), f"player[{i}].roundScore"
            )
            MockReactRenderer.render_value(
                player.get("totalScore"), f"player[{i}].totalScore"
            )
            MockReactRenderer.render_value(
                player.get("baseScore", 0), f"player[{i}].baseScore"
            )
            MockReactRenderer.render_value(
                player.get("actualPiles", 0), f"player[{i}].actualPiles"
            )
...
async def test_end_to_end_react_error_fix():
    ...
    phase_change_data = {
        ...
        "total_scores": {player.name: player.score for player in players},
        "redeal_multiplier": game.redeal_multiplier,
    }

    print(f"📡 WebSocket data prepared: {len(phase_change_data)} keys")
    print(
        f"   round_scores type: {type(phase_change_data['phase_data']['round_scores'])}"
    )
    ...
    for player_data in frontend_service.state["playersWithScores"]:
        for key, value in player_data.items():
            if isinstance(value, dict):
                print(
                    f"❌ playersWithScores[{player_data['name']}].{key} is still an object: {value}"
                )
                validation_passed = False
...
async def test_edge_cases():
    ...
    try:
        ...
        for name, score in frontend_service.state["roundScores"].items():
            assert isinstance(
                score, (int, float)
            ), f"{name} roundScore is not a number: {type(score)}"

        for name, score in frontend_service.state["totalScores"].items():
            assert isinstance(
                score, (int, float)
            ), f"{name} totalScore is not a number: {type(score)}"
    ...
    try:
        ...
        assert (
            frontend_service.state["roundScores"]["Bot 3"] == 0
        )  # Should default to 0 for object without final_score
...
if __name__ == "__main__":
    ...
    async def run_all_tests():
        ...
        print(
            f"   Overall: {'✅ SUCCESS' if main_result and edge_result else '❌ FAILURE'}"
        )
