...
async def test_scoring_object_format():
    ...
    for player in players:
        print(
            f"   {player.name}: declared={player.declared_piles}, actual={player.captured_piles}, prev_score={player.score}"
        )
    print()
    ...
    for player_name, score_data in scoring_state.round_scores.items():
        ...
        for field, value in score_data.items():
            if not isinstance(value, (int, float)):
                print(
                    f"  ❌ ERROR: Field '{field}' should be numeric, got {type(value)}: {value}"
                )
                all_tests_passed = False
            else:
                print(f"  ✅ {field}: {value}")
        ...
        expected_base = calculate_expected_base_score(
            score_data["declared"], score_data["actual"]
        )
        expected_final = expected_base * score_data["multiplier"]

        if score_data["base_score"] != expected_base:
            print(
                f"  ❌ ERROR: Base score mismatch. Expected {expected_base}, got {score_data['base_score']}"
            )
            all_tests_passed = False

        if score_data["final_score"] != expected_final:
            print(
                f"  ❌ ERROR: Final score mismatch. Expected {expected_final}, got {score_data['final_score']}"
            )
            all_tests_passed = False
    ...
    try:
        ...
        for player, score in frontend_scores.items():
            if not isinstance(score, (int, float)):
                print(
                    f"❌ ERROR: Frontend score for {player} is not numeric: {type(score)}"
                )
                all_tests_passed = False
...
async def test_edge_cases():
    ...
    if score_data["final_score"] == expected_final:
        ...
        print(
            f"❌ Zero multiplier error: expected {expected_final}, got {score_data['final_score']}"
        )
        return False
    ...
    if score_data["final_score"] == expected_final:
        ...
        print(
            f"❌ Large multiplier error: expected {expected_final}, got {score_data['final_score']}"
        )
        return False
    ...
    if score_data["base_score"] < 0 and score_data["final_score"] < 0:
        ...
        print(
            f"❌ Negative score error: base={score_data['base_score']}, final={score_data['final_score']}"
        )
        return False
...
if __name__ == "__main__":
    ...
    async def run_all_tests():
        ...
        print(
            f"   Overall: {'✅ ALL TESTS PASSED' if main_result and edge_result else '❌ SOME TESTS FAILED'}"
        )
