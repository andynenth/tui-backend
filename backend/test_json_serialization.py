        ...
        try:
            ...
            for key, value in event_data.items():
                if hasattr(value, '__dict__') and not isinstance(value, (str, int, float, bool, list, dict)):
                    print(f"   Found non-serializable object in key '{key}': {type(value)} - {value}")
                elif isinstance(value, dict):
                    for sub_key, sub_value in value.items():
                        if hasattr(sub_value, '__dict__') and not isinstance(sub_value, (str, int, float, bool, list, dict)):
                            print(f"   Found non-serializable object in '{key}.{sub_key}': {type(sub_value)} - {sub_value}")
                elif isinstance(value, list):
                    for i, item in enumerate(value):
                        if hasattr(item, '__dict__') and not isinstance(item, (str, int, float, bool, list, dict)):
                            print(f"   Found non-serializable object in '{key}[{i}]': {type(item)} - {item}")
