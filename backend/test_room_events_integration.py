...
async def test_join_room(room_code):
    ...
    try:
        async with websockets.connect(WS_URL_LOBBY) as websocket:
            ...
            assert (
                result["event"] == "room_joined"
            ), f"Expected 'room_joined', got {result['event']}"
...
async def test_room_state_operations(room_id):
    ...
    try:
        async with websockets.connect(WS_URL_ROOM.format(room_id=room_id)) as websocket:
            ...
            assert (
                result["event"] == "player_removed"
            ), f"Expected 'player_removed', got {result['event']}"
            ...
            add_bot_msg = {
                "event": "add_bot",
                "data": {"difficulty": "medium", "bot_name": "TestBot"},
            }
            ...
            assert (
                result["event"] == "bot_added"
            ), f"Expected 'bot_added', got {result['event']}"
            ...
            logger.info(
                f"✅ Verified room state after add/remove operations: {new_player_count} players"
            )
...
async def test_leave_room(room_id, player_id):
    ...
    try:
        async with websockets.connect(WS_URL_ROOM.format(room_id=room_id)) as websocket:
            ...
            # Test leave_room
            leave_msg = {"event": "leave_room", "data": {"player_id": player_id}}
...
async def test_error_handling():
    ...
    try:
        async with websockets.connect(WS_URL_LOBBY) as websocket:
            ...
            bad_create_msg = {"event": "create_room", "data": {"max_players": 4}}
            ...
            assert (
                result["event"] == "error"
            ), f"Expected 'error', got {result['event']}"
            assert (
                result["data"]["type"] == "validation_error"
            ...
            bad_join_msg = {
                "event": "join_room",
                "data": {"player_name": "TestPlayer", "room_id": "NONEXISTENT"},
            }
            ...
            logger.info(f"Error response: {result}")
            assert (
                result["event"] == "join_failed"
            ), f"Expected 'join_failed', got {result['event']}"
...
async def main():
    ...
    try:
        ...
        logger.info(
            "Room events are working correctly with direct use case integration"
        )
