...
async def test_full_game_flow():
    ...
    try:
        ...
        print(f"✅ Preparation complete, transitioned to: {state_machine.current_phase}")
        ...
        print(f"✅ Declaration complete, transitioned to: {state_machine.current_phase}")
        ...
        while turn_count < max_turns:
            ...
            for player_name in ["Alice", "Bob", "Charlie", "Diana"]:
                if player_name in game.player_hands and len(game.player_hands[player_name]) >= 2:
            ...
            if turn_count < max_turns and not all(len(hand) == 0 for hand in game.player_hands.values()):
        ...
        if hasattr(scoring_state, 'scores_calculated'):
            ...
            if hasattr(scoring_state, 'round_scores'):
                ...
                for player, data in scoring_state.round_scores.items():
                    print(f"   {player}: declared {data.get('declared', 0)}, "
        ...
        if next_phase == GamePhase.PREPARATION:
            ...
            print(f"✅ Scoring complete, transitioned to: {state_machine.current_phase}")
            assert state_machine.current_phase == GamePhase.PREPARATION
    ...
    try:
        ...
        for round_num in [1, 2]:
            ...
            if round_num < 2:
                ...
                print(f"✅ Round {round_num} complete, starting round {game.round_number}")
