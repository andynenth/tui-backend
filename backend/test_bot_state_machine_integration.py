...
async def test_bot_state_machine_integration():
    ...
    try:
        current_order = (
            game_state.current_order if hasattr(game_state, "current_order") else []
        )
        players_list = game_state.players if hasattr(game_state, "players") else []
        print(
            f"✅ Property access works: {len(current_order)} in order, {len(players_list)} total"
        )
...
async def test_bot_fallback_mode():
    ...
    print(
        f"✅ Fallback mode works: {len(game_state.players)} players via direct game access"
    )
