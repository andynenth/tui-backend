...
def is_combo_viable_simple(
    combo_type: str, pieces: List[Piece], has_opener: bool, field_strength: str
) -> bool:
    """
...
def choose_declare_simplified(
    ...
    if verbose:
        ...
        print(
            f"  Openers found: {opener_count} - {[f'{p.name}({p.point})' for p in openers]}"
        )
    ...
    if best_combo and opener_count > 0:
        ...
        if verbose:
            print(
                f"  Decision: Opener control (1) + {best_combo[0]} ({best_combo_size}) = {declaration}"
            )
    elif opener_count > 0:
        ...
        if verbose:
            print(
                f"  Decision: {opener_count} openers, no viable combos = {declaration}"
            )
    elif best_combo:
        ...
        if verbose:
            print(
                f"  Decision: No openers, but {best_combo[0]} ({best_combo_size}) = {declaration}"
            )
    else:
        # Weak hand
        declaration = 0
