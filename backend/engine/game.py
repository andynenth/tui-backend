...
class Game:
    def __init__(
        ...
        self.turn_history_this_round = []  # Complete history of all turns this round
    ...
    def validate_player_declaration(self, player_name: str, declaration: int) -> dict:
        ...
        try:
            ...
            return {"valid": False, "reason": str(e), "valid_options": []}
    ...
    def _deal_weak_hand(self, weak_player_indices=None, max_weak_points=9, limit=None):
        ...
        # Validate indices
        weak_player_indices = [
            i for i in weak_player_indices if 0 <= i < len(self.players)
        ]
        ...
        for player_idx in non_weak_players:
            ...
            available_for_this_player = (
                remaining_weak + strong_pieces[strong_piece_index:]
            )
            ...
            for piece in available_for_this_player:
                if pieces_added >= 8:
                    break
                if piece not in [
                    p for player in self.players for p in player.hand
                ]:  # Not already dealt
                    ...
                    if piece in remaining_weak:
                        ...
                        strong_pieces[strong_piece_index:] = [
                            p for p in strong_pieces[strong_piece_index:] if p != piece
                        ]
    ...
    def _deal_guaranteed_no_redeal(self, red_general_player_index=None):
        ...
        if red_general and red_general_player_index is not None:
            ...
            if 0 <= red_general_player_index < len(self.players):
                ...
                players_needing_strong = [
                    i for i in range(len(self.players)) if i != red_general_player_index
                ]
            else:
