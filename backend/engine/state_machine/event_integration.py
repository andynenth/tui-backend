...
class StateChangeEventPublisher:
    ...
    async def on_phase_data_update(
        ...
        try:
            ...
            if self._last_phase != phase:
                await self._publish_phase_change(
                    room_id, self._last_phase, phase, metadata
                )
    ...
    async def _publish_phase_specific_events(
        ...
        # Scoring phase events
            ...
            await self._handle_scoring_phase_events(
                room_id, updates, old_data, metadata
            )

        # Round transitions
        if "round_number" in updates and updates["round_number"] != old_data.get(
            "round_number"
        ):
    ...
    async def _handle_turn_phase_events(
        ...
        # New turn started
        if "current_player" in updates and updates["current_player"] != old_data.get(
            "current_player"
        ):
    ...
    async def _handle_game_end(
        self, room_id: str, updates: Dict[str, Any], metadata: EventMetadata
    ):
