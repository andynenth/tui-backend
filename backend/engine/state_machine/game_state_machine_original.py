...
class GameStateMachine:
    ...
    async def _do_transition(self, new_phase: GamePhase, reason: str):
        ...
        # 🔧 FIX: Add stack trace to understand where transitions come from
        import traceback
        stack = traceback.extract_stack()
        caller_info = "Unknown"
        for frame in reversed(stack):
            if "_do_transition" not in frame.name and "_immediate_transition_to" not in frame.name:
                caller_info = f"{frame.filename}:{frame.lineno} in {frame.name}"
                break
        
        print(f"🔍 TRANSITION_SOURCE: {self.current_phase} -> {new_phase} called from {caller_info}")
        print(f"🔍 TRANSITION_REASON: {reason}")
        ...
        try:
            ...
            # 🔧 FIX: Clear stale broadcast messages on major transitions
            if new_phase == GamePhase.SCORING or new_phase == GamePhase.PREPARATION:
                try:
                    room_id = getattr(self, 'room_id', getattr(self.game, 'room_id', None))
                    if room_id:
                        # Import here to avoid circular imports
                        from backend.socket_manager import clear_room_queue
                        # Clear old turn_complete messages that might be stuck
                        cleared = await clear_room_queue(room_id, ['turn_complete'])
                        if cleared > 0:
                            print(f"🧹 TRANSITION_CLEANUP: Cleared {cleared} stale turn_complete messages on transition to {new_phase}")
                except Exception as e:
                    print(f"⚠️ TRANSITION_CLEANUP: Could not clear stale messages: {e}")
