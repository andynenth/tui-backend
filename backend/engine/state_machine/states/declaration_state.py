    ...
    async def _validate_action(self, action: GameAction) -> bool:
        if action.action_type == ActionType.DECLARE:
            ...
            if action.player_name != current_player:
                self.logger.warning(
                    f"Wrong player turn: {action.player_name}, expected: {current_player}"
                )
                return False

            return await self._check_declaration_restrictions(
                action.player_name, declared_value
            )
    ...
    async def _handle_declaration(self, action: GameAction) -> Dict[str, Any]:
        ...
        for player in self.state_machine.game.players:
            if getattr(player, "name", str(player)) == player_name:
                player.record_declaration(
                    declared_value
                )  # This updates zero_declares_in_a_row
                break
