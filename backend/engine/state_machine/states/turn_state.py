...
class TurnState(GameState):
    ...
    async def _setup_phase(self) -> None:
        ...
        self.current_turn_starter = getattr(game, "round_starter", None) or getattr(
        ...
        if not self.current_turn_starter:
            ...
            if hasattr(game, "players") and game.players:
                ...
                self.current_turn_starter = getattr(
                    first_player, "name", str(first_player)
                )
            else:
    ...
    async def check_transition_conditions(self) -> Optional[GamePhase]:
        """Check if ready to transition to turn_results phase"""
        # Always transition to TURN_RESULTS when turn is complete
        # TURN_RESULTS will handle the logic for SCORING vs next TURN
        if self.turn_complete:
            self.logger.info("🏁 Turn complete - transitioning to turn_results")
            return GamePhase.TURN_RESULTS
    ...
    def _determine_turn_winner(self) -> Optional[str]:
        ...
        if turn_result_data and turn_result_data.get("winner"):
            ...
            turn_number = getattr(self.state_machine.game, "turn_number", 0)
            self.logger.info(
                f"🎯 Round {current_round}, Turn {turn_number} winner: {winner_name} with {winner_play_data.get('play_type', 'unknown')} value {winner_play_data.get('play_value', 0)}"
            )
            return winner_name
    ...
    def _get_turn_resolution_data(self) -> Optional[Dict[str, Any]]:
        ...
        for player_name in self.turn_order:
            if player_name in self.turn_plays:
                ...
                turn_play = TurnPlay(
                    player=player_obj, pieces=pieces, is_valid=is_valid
                )
        ...
        result_data = {"all_plays": [], "winner": None, "winner_play": None}
        ...
        for turn_play in turn_result.plays:
            ...
            play_info = {
                ...
                "play_type": self.turn_plays.get(player_name, {}).get(
                    "play_type", "unknown"
                ),
        ...
        if turn_result.winner:
            winner_name = getattr(
                turn_result.winner.player, "name", str(turn_result.winner.player)
            )
            winner_play_data = self.turn_plays.get(winner_name, {})
    ...
    async def _process_turn_completion(self) -> None:
        ...
        if all_hands_empty:
            ...
            if self.winner:
                ...
                self.logger.info(
                    f"🏁 All hands are now empty - round complete. Last turn winner: {self.winner}"
                )
            else:
            ...
            if self.winner:
                ...
                if turn_started:
                    self.logger.info(
                        "🚀 Enterprise: New turn auto-started with automatic broadcasting"
                    )
            else:
                self.logger.info(
                    f"🤝 No winner - starter remains: {self.current_turn_starter}"
                )
    ...
    async def _auto_play_for_player(self, player: str) -> None:
        ...
        pieces = (
            player_hand[:required_count]
            if len(player_hand) >= required_count
            else player_hand
        )
    ...
    async def restart_turn_for_testing(self) -> None:
        ...
        if hasattr(game, "get_player_order_from"):
            ...
            if isinstance(players[0], str):
                ...
                if self.current_turn_starter in player_names:
                    start_idx = player_names.index(self.current_turn_starter)
                    self.turn_order = (
                        player_names[start_idx:] + player_names[:start_idx]
                    )
                else:
                    self.turn_order = player_names
        ...
        self.logger.info(
            f"🔄 Restarted turn - starter: {self.current_turn_starter}, order: {self.turn_order}"
        )

    async def _broadcast_play_event_enterprise(
        self, player_name: str, pieces: List, piece_count: int
    ):
    ...
    async def _broadcast_play_event(
        self, player_name: str, pieces: List, piece_count: int
    ):
        ...
        try:
            ...
            await self.broadcast_custom_event(
                "turn_complete",
                {
                    ...
                    "turn_players_data": turn_players_data,  # Turn-specific data (base_state.py handles standard players data)
            ...
            self.logger.error(
                f"Failed to broadcast turn completion: {e}", exc_info=True
            )
