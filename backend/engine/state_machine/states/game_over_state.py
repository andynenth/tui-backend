...
class GameOverState(GameState):
    ...
    async def _cleanup_phase(self) -> None:
        ...
        self.logger.info(f"Game over phase completed for room {self.state_machine.room_id}")
    ...
    async def _process_action(self, action: GameAction) -> None:
        ...
        if action.action_type == ActionType.PLAYER_DISCONNECT:
            ...
            self.logger.info(f"Player {action.player_name} disconnected during game over")
        
        elif action.action_type == ActionType.PLAYER_RECONNECT:
            # Just log it - players can reconnect to see results
            self.logger.info(f"Player {action.player_name} reconnected during game over")
