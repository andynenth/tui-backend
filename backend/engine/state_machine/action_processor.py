# backend/engine/state_machine/action_processor.py
...
class ActionProcessor:
    """
    🎯 **ActionProcessor** - Extracted from GameStateMachine
    
    Handles all action validation, execution, and bot notifications.
    Part of Phase 2.1 God Class Decomposition.
    
    Responsibilities:
    - Action validation against current state
    - Action execution delegation to states
    - Bot manager notifications for actions
    ...
    async def execute_action(self, action: GameAction) -> Dict:
        ...
        try:
            ...
            if result.get("success", False):
                ...
                await self._notify_bot_manager_action_failed(action, result.get("error", "Unknown error"))
            ...
            await self._notify_bot_manager_action_failed(action, error_msg)
    ...
    # === Bot Manager Notification Methods ===
