...
class AsyncGameAdapter:
    ...
    async def deal_pieces(self) -> Optional[Dict[str, Any]]:
        ...
        if self.is_async:
            ...
            return {"success": True, "sync": True, "message": "Pieces dealt (sync)"}
    ...
    async def play_turn(
        ...
        if self.is_async:
            ...
            return await loop.run_in_executor(
                None, self.game.play_turn, player_name, piece_indexes
            )
    ...
    async def execute_redeal_for_player(self, player_name: str) -> Dict[str, Any]:
        ...
        if self.is_async:
            ...
            loop = asyncio.get_event_loop()
            return await loop.run_in_executor(
                None, self.game.execute_redeal_for_player, player_name
            )
    ...
    async def record_player_declaration(
        ...
        if self.is_async:
            ...
            loop = asyncio.get_event_loop()
            return await loop.run_in_executor(
                None, self.game.record_player_declaration, player_name, declaration
            )
    ...
    async def get_game_state(self) -> Dict[str, Any]:
        ...
        if self.is_async:
            ...
            return {
                ...
                "current_player": (
                    self.game.current_player.name if self.game.current_player else None
                ),
