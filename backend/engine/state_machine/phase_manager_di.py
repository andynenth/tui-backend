# backend/engine/state_machine/phase_manager_di.py
...
from ..dependency_injection.interfaces import IBroadcaster, IBotNotifier, IEventStore
...
class PhaseManager:
    """
    🎯 **PhaseManager with Dependency Injection** - Phase 3 Circular Dependency Resolution
    
    Handles all phase transitions and state management using dependency injection
    instead of direct imports to eliminate circular dependencies.
    
    Dependencies injected:
    - IBroadcaster: For WebSocket broadcasting (replaces direct socket_manager import)
    ...
    async def _do_transition(self, new_phase: GamePhase, reason: str):
        ...
        try:
            ...
            # Step 5: Broadcast phase change (using dependency injection)
            await self._broadcast_phase_change(new_phase)
            
            # Step 6: Store phase change event (using dependency injection)
            await self._store_phase_change_event(old_phase, new_phase)
            
            # Step 7: Notify bot manager (using dependency injection)
    ...
    # === Broadcasting Methods (Using Dependency Injection) ===
    
    async def _broadcast_phase_change(self, new_phase: GamePhase):
        """Broadcast phase change using injected broadcaster."""
        try:
            room_id = getattr(self.state_machine, 'room_id', None)
            if room_id:
                phase_data = self.get_phase_data()
                await self.broadcaster.broadcast_phase_change(room_id, new_phase.name, phase_data)
                logger.debug(f"📡 PHASE_BROADCAST: Phase change {new_phase.name} broadcasted")
            else:
                logger.warning(f"⚠️ PHASE_BROADCAST: No room_id available for broadcasting")
    ...
    async def _store_phase_change_event(self, old_phase: Optional[GamePhase], new_phase: GamePhase):
        ...
        try:
            ...
            if room_id:
                ...
                await self.event_store.store_event(room_id, phase_change_data)
    ...
    async def _notify_bot_manager(self, new_phase: GamePhase):
        ...
        try:
            ...
            if room_id:
                phase_data = self.get_phase_data()
                await self.bot_notifier.notify_phase_change(room_id, new_phase.name, phase_data)
