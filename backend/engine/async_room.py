...
class AsyncRoom:
    ...
    def __init__(self, room_id: str, host_name: str):
        ...
        self.players[0] = Player(
            host_name, is_bot=False, available_colors=self._get_available_colors()
        )
    ...
    async def assign_slot(self, slot: int, name_or_none: Optional[str]) -> None:
        ...
        async with self._assign_lock:
            ...
            logger.info(
                f"Assigned {name_or_none} to slot {slot} in room {self.room_id}"
            )
    ...
    async def summary(self) -> Dict[str, Any]:
        ...
        async with self._state_lock:
            ...
            return {
                ...
                "players": [slot_info(p, i) for i, p in enumerate(self.players)],
    ...
    async def migrate_host(self) -> Optional[str]:
        ...
        async with self._state_lock:
            ...
            logger.warning(
                f"No suitable host found for migration in room {self.room_id}"
            )
            return None
    ...
    def validate_state(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.host_name:
                ...
                if not host_found:
                    validation["errors"].append(
                        f"Host '{self.host_name}' not found in players"
                    )
            ...
            if self.started and not self.game:
                validation["errors"].append(
                    "Room marked as started but no game instance"
                )
                validation["valid"] = False

            if self.game and not self.started:
                validation["warnings"].append(
                    "Game instance exists but room not marked as started"
                )

            # Check player slots
            if self.started and validation["player_count"] < 4:
                validation["warnings"].append(
                    f"Game started with only {validation['player_count']} players"
                )
