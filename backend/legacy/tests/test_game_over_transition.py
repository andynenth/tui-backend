...
class MockGameForGameOver:
    ...
    def __init__(self, winner_name="Andy", winner_score=51):
        ...
        self.player_declarations = {"Andy": 4, "Bot 2": 1, "Bot 3": 1, "Bot 4": 1}
...
def mock_game_no_winner():
    ...
    game.players = [
        MockPlayer(
            "Andy", 46, False, 3
        ),  # Andy: 46 - 1 (declared 4, got 3) = 45 (not quite 50)
        MockPlayer("Bot 2", 38, True, 4),  # Bot 2: 38 - 3 (declared 1, got 4) = 35
        MockPlayer("Bot 3", 31, True, 0),  # Bot 3: 31 - 1 (declared 1, got 0) = 30
        MockPlayer("Bot 4", 26, True, 0),  # Bot 4: 26 - 1 (declared 1, got 0) = 25
    ]
...
class TestGameOverTransition:
    ...
    @pytest.mark.asyncio
    async def test_scoring_transitions_to_game_over_with_winner(
        self, state_machine_with_winner, mock_broadcast
    ):
        ...
        phase_change_calls = [
            call for call in calls if len(call[0]) >= 3 and call[0][1] == "phase_change"
        ]
    ...
    @pytest.mark.asyncio
    async def test_scoring_transitions_to_preparation_without_winner(
        self, state_machine_no_winner, mock_broadcast
    ):
