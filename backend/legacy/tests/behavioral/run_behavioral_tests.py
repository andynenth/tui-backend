...
class BehavioralTestRunner:
    ...
    def run_test_suite(self, test_file: str) -> dict:
        ...
        cmd = [
            ...
            "-v", "--tb=short",
            "--json-report", f"--json-report-file={self.results_dir / f'{test_file}.json'}"
        ]
    ...
    def generate_summary_report(self, start_time: datetime, end_time: datetime):
        ...
        # Save summary report
        report_path = self.results_dir / f"behavioral_test_summary_{start_time.strftime('%Y%m%d_%H%M%S')}.json"
        ...
        print(f"Total duration: {report['behavioral_test_summary']['total_duration']:.2f}s")
...
def main():
    ...
    print("2. Run golden master capture: python tests/contracts/capture_golden_masters.py")
