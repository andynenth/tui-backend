...
class RateLimitAnalyzer:
    ...
    def analyze_websocket_handler(self):
                ...
                self.issues.append(
                    {
                        ...
                        "line": content[
                            : content.find("if not rate_limit_allowed:")
                        ].count("\n")
                        + 1,
                    }
                )
        ...
        if "check_websocket_rate_limit" in content:
            ...
            check_context = self._get_context_around(
                content, "check_websocket_rate_limit", 5
            )
    ...
    def analyze_websocket_rate_limiter(self):
        ...
        ws_rate_limit_file = (
            self.project_root / "backend/api/middleware/websocket_rate_limit.py"
        )
    ...
    def analyze_middleware_integration(self):
        ...
        if "RateLimitMiddleware" in content:
            self.suggestions.append(
                {"file": "main.py", "suggestion": "Rate limit middleware is integrated"}
            )
    ...
    def generate_report(self):
        ...
        print(
            f"  High Priority: {len([i for i in self.issues if i['severity'] in ['ERROR', 'HIGH']])}"
        )
