...
class GoldenMasterCapture:
    ...
    async def capture_message_behavior(
        ...
        state_changes = {
            "room_state_after": room_state if room_state else {}
        }
    ...
    def load_golden_master(self, message_name: str, test_id: str) -> Optional[GoldenMasterRecord]:
...
class GoldenMasterComparator:
    ...
    def compare_behavior(
        ...
        actual_behavior: GoldenMasterRecord
    ) -> Dict[str, Any]:
        """
        ...
        if response_diff:
            ...
            comparison["differences"].append({
                "type": "response",
                "details": response_diff
            })
        
        # Compare broadcasts
        broadcast_diff = self._compare_broadcasts(
            golden_master.broadcasts,
            actual_behavior.broadcasts
        )
        if broadcast_diff:
            comparison["match"] = False
            comparison["differences"].append({
                "type": "broadcasts",
                "details": broadcast_diff
            })
        
        # Compare timing
        timing_analysis = self._analyze_timing(
            golden_master.timing,
            actual_behavior.timing
        )
    ...
    def _analyze_timing(
        self,
        expected_timing: Dict[str, float],
        actual_timing: Dict[str, float]
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "within_threshold": abs(actual_duration - expected_duration) < 100  # 100ms threshold
        }