...
async def test_connection_events():
    ...
    try:
        async with websockets.connect(WS_URL) as websocket:
            ...
            ping_msg = {
                "event": "ping",
                "data": {"timestamp": datetime.now().timestamp(), "sequence": 1},
            }
            ...
            assert (
                ready_ack["event"] == "client_ready_ack"
            ), f"Expected 'client_ready_ack', got {ready_ack['event']}"

            # Test 3: Ack
            logger.info("Testing ack event...")
            ack_msg = {"event": "ack", "data": {"message_id": "test_msg_123"}}
            await websocket.send(json.dumps(ack_msg))
            response = await websocket.recv()
            ack_response = json.loads(response)
            logger.info(f"Ack response: {ack_response}")
            assert (
                ack_response["event"] == "ack_received"
            ), f"Expected 'ack_received', got {ack_response['event']}"

            # Test 4: Sync Request
            logger.info("Testing sync_request event...")
            sync_msg = {"event": "sync_request", "data": {"last_sequence": 0}}
            await websocket.send(json.dumps(sync_msg))
            response = await websocket.recv()
            sync_response = json.loads(response)
            logger.info(f"Sync response: {sync_response}")
            assert (
                sync_response["event"] == "sync_response"
            ), f"Expected 'sync_response', got {sync_response['event']}"
...
async def test_lobby_events():
    ...
    try:
        async with websockets.connect(WS_URL) as websocket:
            ...
            ), f"Expected 'room_list', got {room_list['event']}"
            assert "rooms" in room_list["data"], "Response should contain rooms"
            assert (
                "total_count" in room_list["data"]
            ...
            assert (
                rooms_response["event"] == "room_list"
            ), f"Expected 'room_list', got {rooms_response['event']}"
...
async def test_error_handling():
    ...
    try:
        async with websockets.connect(WS_URL) as websocket:
            ...
            ), f"Expected 'error', got {error_response['event']}"
            assert (
                error_response["data"]["type"] == "routing_error"
            ), "Should be routing error"

            # Test 2: Unsupported event
            logger.info("Testing unsupported event...")
            unsupported_msg = {"event": "unsupported_event_xyz", "data": {}}
            ...
            assert (
                error_response["data"]["type"] == "unsupported_event"
            ), "Should be unsupported event"
...
async def main():
    ...
    try:
        ...
        logger.info(
            "Connection and lobby events are working with direct use case integration"
        )
