...
def test_poor_choice_scenario():
    ...
    if len(selected) == 2 and all(
        p.name == "SOLDIER" and p.color == "BLACK" for p in selected
    ):
...
def test_all_pairs_scenario():
    ...
    hand_str = [
        "SOLDIER_BLACK",
        "SOLDIER_BLACK",  # 2 points
        "ELEPHANT_BLACK",
        "ELEPHANT_BLACK",  # 18 points
        "HORSE_RED",
        "CANNON_RED",  # Not a pair
    ]
    ...
    for combo in combinations(hand, 2):
        ...
        if is_valid_play(pieces):
            ...
            if play_type == "PAIR":
                ...
                print(
                    f"  - {pieces[0].name}_{pieces[0].color} + {pieces[1].name}_{pieces[1].color} = {total} pts (never-win: {is_never_win})"
                )
