...
class RealisticAIAnalyzer:
    ...
    def analyze_declarations(self) -> Dict[str, Any]:
        ...
        for pos, stats in position_stats.items():
            position_analysis[f"position_{pos}"] = {
                "avg_declared": stats['sum_declared'] / stats['total'] if stats['total'] > 0 else 0,
                "declaration_count": stats['total'],
                "avg_accuracy": sum(accuracy_by_position[pos]) / len(accuracy_by_position[pos]) if accuracy_by_position[pos] else 0
            }
            
        return {
            "position_impact": position_analysis,
            "zero_streak_patterns": dict(zero_streak_usage),
            "starter_advantage": position_analysis.get("position_0", {}).get("avg_accuracy", 0)
        }
    ...
    def analyze_play_patterns(self) -> Dict[str, Any]:
        ...
        for game in self.games:
            ...
            for result in turn_results:
                for play in result.get('plays', []):
                    ...
                    if result.get('winner') and play['player'] == result['winner']['player']:
        ...
        return {
            ...
            "disposal_rate": len(disposal_plays) / sum(play_types.values()) if play_types else 0,
    ...
    def analyze_hand_strength(self) -> Dict[str, Any]:
        ...
        for game in self.games:
            ...
            starts = [e for e in game['events'] if e['event'] in ['game_start', 'round_start']]
        ...
        avg_strength = sum(hand_strengths) / len(hand_strengths) if hand_strengths else 0
        
        return {
            "weak_hand_frequency": len(weak_hands),
            "weak_hand_percentage": (len(weak_hands) / len(hand_strengths) * 100) if hand_strengths else 0,
    ...
    def calculate_performance(self) -> Dict[str, Any]:
        ...
        player_stats = defaultdict(lambda: {
            ...
            "perfect_rounds": 0, "total_rounds": 0
        })
        ...
        for player, stats in player_stats.items():
            performance[player] = {
                ...
                "perfect_round_rate": (stats['perfect_rounds'] / stats['total_rounds'] * 100) if stats['total_rounds'] > 0 else 0,
...
def main():
    ...
    for pos_key, stats in decl['position_impact'].items():
        print(f"{pos_key}: avg_declared={stats['avg_declared']:.1f}, "
    ...
    if plays.get('combo_effectiveness'):
        for combo, stats in sorted(plays['combo_effectiveness'].items(), 
                                  key=lambda x: x[1]['win_rate'], reverse=True):
            print(f"{combo}: {stats['win_rate']:.1f}% win rate "
                  f"({stats['total_won']}/{stats['total_played']} turns won)")
    
    # Hand Strength
    print("\n--- HAND STRENGTH ---")
    hands = results['hand_strength']
    print(f"Weak hand frequency: {hands['weak_hand_frequency']} ({hands['weak_hand_percentage']:.1f}%)")
    ...
    for player, metrics in results['performance_metrics'].items():
        print(f"{player}: Win rate={metrics['win_rate']:.1f}%, "
