#!/usr/bin/env python3
"""
Quiet Benchmark AI Debug Mode Performance
Tests speed, memory usage, and scalability with minimal output
"""
...
# Suppress all AI decision logging
logging.getLogger('backend.services.ai_player').setLevel(logging.ERROR)
logging.getLogger('backend.ai_debug_simple').setLevel(logging.ERROR)
logging.getLogger('backend.services.ai_logger').setLevel(logging.ERROR)
logging.getLogger('backend.engine.ai').setLevel(logging.ERROR)
logging.getLogger('backend.engine.ai_turn_strategy').setLevel(logging.ERROR)
...
def benchmark_games(num_games: int, log_level: str = 'summary'):
    ...
    for i in range(num_games):
        ...
        # Run game with output suppressed
...
def benchmark_log_levels():
    ...
    for level in levels:
        r = results[level]
        print(f"{level:<10} {r['games_per_second']:<12.2f} "
    ...
    for size in test_sizes:
        ...
        try:
            ...
            if result['games_per_second'] < 1.0:
                print(f"WARNING: Performance degraded to {result['games_per_second']:.2f} games/sec")
...
def main():
    parser = argparse.ArgumentParser(
        description='Benchmark AI Debug Mode performance (quiet version)'
    )
