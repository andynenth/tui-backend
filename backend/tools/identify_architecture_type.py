...
class ArchitectureAnalyzer:
    ...
    CLEAN_DIRS = [
        ...
        'api/adapters/', 'api/dto/', 'api/interfaces/'
    ]
    
    # Legacy directory patterns
    LEGACY_DIRS = [
        'engine/', 'models/', 'backend/engine/',
        'backend/models/'
    ]
    ...
    def analyze_file(self, file_path: Path) -> FileAnalysis:
        ...
        indicators = self._get_indicators(
            relative_path, imports, content, arch_type
        )
    ...
    def _analyze_imports(self, imports: List[str]) -> Dict[str, float]:
        ...
        return {
            'clean': clean_count / total,
            'legacy': legacy_count / total
        }
    ...
    def _analyze_patterns(self, content: str) -> Dict[str, float]:
        ...
        return {
            'clean': clean_matches / total,
            'legacy': legacy_matches / total
        }
    ...
    def generate_report(self) -> Dict:
        ...
        for result in self.results:
            type_counts[result.architecture_type] = type_counts.get(
                result.architecture_type, 0
            ) + 1
        ...
        for result in self.results:
            if result.architecture_type == 'clean':
                for dep in result.dependencies:
                    if any(legacy in dep for legacy in ['engine', 'socket_manager', 'shared_instances']):
                        cross_deps.append({
                            'clean_file': result.file_path,
                            'legacy_dependency': dep
                        })
        ...
        return {
            ...
            'recommendations': {
                'immediate_action': len([r for r in self.results if r.architecture_type == 'error']),
                'phase_7_removal': len([r for r in self.results if r.architecture_type == 'legacy']),
                'review_needed': len([r for r in self.results if r.architecture_type == 'hybrid'])
            }
        }
...
def main():
    ...
    if args.file:
        ...
        print(f"\nRecommendations:")
        print(f"  Files needing immediate attention: {report['recommendations']['immediate_action']}")
        print(f"  Files for Phase 7 removal: {report['recommendations']['phase_7_removal']}")
        ...
        if args.verbose:
            legacy_files = [r for r in analyzer.results if r.architecture_type == 'legacy']
            if legacy_files:
                print(f"\nLegacy files (first 10):")
                for result in legacy_files[:10]:
                    print(f"  - {result.file_path}")
                    
            hybrid_files = [r for r in analyzer.results if r.architecture_type == 'hybrid']
            if hybrid_files:
