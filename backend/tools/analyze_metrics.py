...
class MetricsAnalyzer:
    ...
    def generate_metrics_report(self) -> str:
        ...
        if self.winning_scores:
            ...
            report.append(f"Average Winning Score: {statistics.mean(self.winning_scores):.1f}")
            report.append(f"Median Winning Score: {statistics.median(self.winning_scores):.1f}")
            report.append(f"Min/Max Winning Scores: {min(self.winning_scores)}/{max(self.winning_scores)}")
            ...
            for range_name, count in score_ranges.items():
                percentage = (count / len(self.winning_scores)) * 100 if self.winning_scores else 0
        ...
        if self.game_lengths:
            ...
            report.append("-"*30)
            report.append(f"Average Game Length: {statistics.mean(self.game_lengths):.1f} rounds")
            report.append(f"Median Game Length: {statistics.median(self.game_lengths)} rounds")
            report.append(f"Min/Max Rounds: {min(self.game_lengths)}/{max(self.game_lengths)}")
            ...
            for rounds in sorted(round_counter.keys()):
                ...
                report.append(f"  {rounds} rounds: {count} games ({percentage:.1f}%) {bar}")
        ...
        if self.score_by_position:
            ...
            for position in sorted(self.score_by_position.keys()):
                ...
                if scores:
                    ...
                    report.append(f"  Times Won: {len([g for g in self.games if g['positions'].get(g['winner']) == position])}")
        ...
        report.append(f"Quick Wins (≤2 rounds): {len(quick_wins)} ({len(quick_wins)/len(self.games)*100:.1f}%)")
        report.append(f"Long Games (≥5 rounds): {len(long_games)} ({len(long_games)/len(self.games)*100:.1f}%)")
        
        # Score correlations
        if quick_wins:
            quick_win_scores = [g['winner_score'] for g in quick_wins if g['winner_score'] > 0]
            if quick_win_scores:
                report.append(f"Average Quick Win Score: {statistics.mean(quick_win_scores):.1f}")
                
        if long_games:
            long_game_scores = [g['winner_score'] for g in long_games if g['winner_score'] > 0]
            if long_game_scores:
                report.append(f"Average Long Game Score: {statistics.mean(long_game_scores):.1f}")
    ...
    def export_metrics(self, output_file: str):
        ...
        metrics = {
            'summary': {
                ...
                'avg_winning_score': statistics.mean(self.winning_scores) if self.winning_scores else 0,
                'avg_game_length': statistics.mean(self.game_lengths) if self.game_lengths else 0,
            },
            'winning_scores': {
                'all_scores': self.winning_scores,
                'distribution': Counter(self.winning_scores),
                'statistics': {
                    'mean': statistics.mean(self.winning_scores) if self.winning_scores else 0,
                    'median': statistics.median(self.winning_scores) if self.winning_scores else 0,
            ...
            'game_lengths': {
                ...
                'statistics': {
                    'mean': statistics.mean(self.game_lengths) if self.game_lengths else 0,
                    'median': statistics.median(self.game_lengths) if self.game_lengths else 0,
