...
class AIGameLogReader:
    ...
    def _display_round(self, round_number: int, events: List[Dict]):
        ...
        for turn_num in sorted(turn_plays.keys()):
            self._display_turn(
                turn_num, turn_plays[turn_num], turn_results.get(turn_num)
            )
    ...
    def _display_declaration_phase(self, declarations: List[Dict]):
        ...
        # Sort by position
        sorted_decls = sorted(
            declarations, key=lambda x: x.get("phase_data", {}).get("position", 0)
        )
        ...
        for decl in sorted_decls:
            ...
            pos_text = (
                ...
                if position < 4
                else f"{position+1}th"
            )
            print(
                f"{pos_text}: {Colors.BOLD}{player}{Colors.ENDC} declares {Colors.YELLOW}{declared}{Colors.ENDC} - {reasoning}"
            )
    ...
    def _display_turn(self, turn_num: int, plays: List[Dict], result: Optional[Dict]):
        ...
        if starter == "Unknown" and plays:
            ...
            starter = next(
                (
                    p.get("player")
                    for p in plays
                    if p.get("turn_context", {}).get("am_starter")
                ),
                "Unknown",
            )
        ...
        if result and "plays" in result:
            ...
            for play_data in result["plays"]:
                ...
                turn_play_event = next(
                    (p for p in plays if p.get("player") == player), {}
                )
                ...
                print(
                    f"  {Colors.BOLD}{player}{Colors.ENDC} {status}: {play_color}{play_type}{Colors.ENDC} {pieces_str} - {reasoning}"
                )
        else:
    ...
    def _display_turn_result(self, result: Dict):
        ...
        if winner:
            ...
            print(
                f"  {Colors.GREEN}→ Winner: {Colors.BOLD}{winner_name}{Colors.ENDC} with {winning_type} ({points} points){Colors.ENDC}"
            )
        else:
    ...
    def _display_round_end(self, event: Dict):
        ...
        for player, perf in perfs.items():
            ...
            print(
                f"  {Colors.BOLD}{player}{Colors.ENDC}: Declared {declared}, Got {captured} {perfect} → {score_color}{score_str}{Colors.ENDC} points"
            )
    ...
    def _format_pieces(self, pieces: List[str]) -> str:
        ...
        sorted_pieces = sorted(
            parsed_pieces, key=lambda x: (0 if x[2] == "RED" else 1, -x[3])
        )
...
def find_game_by_id(
    game_id: str, search_dir: Path = Path("logs/ai_debug")
) -> Optional[Path]:
    ...
    for log_file in search_dir.glob("*.json"):
        try:
            with open(log_file, "r") as f:
                ...
                if (
                    f'"game_id": "{game_id}"' in content
                    or f'"game_id":"{game_id}"' in content
                ):
                    return log_file
        except:
            continue
...
def main():
    ...
    if log_path.exists() and log_path.suffix == ".json":
        ...
        if found_path:
            ...
            print(
                f"{Colors.RED}Error: Game ID '{arg}' not found in {search_dir}{Colors.ENDC}"
            )
            sys.exit(1)
    else:
        print(
            f"{Colors.RED}Error: '{arg}' is not a valid file path or game ID{Colors.ENDC}"
        )
