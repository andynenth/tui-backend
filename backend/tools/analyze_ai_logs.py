...
class AILogAnalyzer:
        ...
        for event in game_info["events"]:
            if event["event"] == "declaration":
                ...
                for player, perf in event.get("player_performance", {}).items():
                    self.player_stats[player]["declarations"].append(
                        perf.get("declared", 0)
                    )
                    self.player_stats[player]["captures"].append(
                        perf.get("captured", 0)
                    )
    ...
    def generate_report(self) -> str:
        ...
        if self.games:
            rounds_per_game = [
                g["rounds_played"] for g in self.games if g["rounds_played"] > 0
            ]
            if rounds_per_game:
                report.append(
                    f"Avg Rounds per Game: {statistics.mean(rounds_per_game):.1f}"
                )
                report.append(
                    f"Min/Max Rounds: {min(rounds_per_game)}/{max(rounds_per_game)}"
                )
        ...
        if declaration_events:
            ...
            for pos in sorted(by_position.keys()):
                ...
                report.append(
                    f"Position {pos}: avg {avg:.2f}, range {min(values)}-{max(values)}"
                )
        ...
        with open(output_file, "w", newline="") as f:
            ...
            for player in sorted(self.player_stats.keys()):
                ...
                if stats["games_played"] > 0:
                    ...
                    avg_score = stats["total_score"] / stats["games_played"]
                    avg_declared = (
                        statistics.mean(stats["declarations"])
                        if stats["declarations"]
                        else 0
                    )
                    avg_captured = (
                        statistics.mean(stats["captures"]) if stats["captures"] else 0
                    )
                    accuracy = (
...
def main():
    ...
    parser.add_argument(
        "--export-stats", type=str, help="Export statistics to CSV file"
    )
