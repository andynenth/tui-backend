...
def main():
    ...
    if total_stats['files_with_details'] == 0:
        ...
        print("  To detect never-win combos, you need detailed game logs with piece information.")
    ...
    if all_never_wins:
        ...
        for i, play in enumerate(all_never_wins[:20]):
            pieces_str = ', '.join([f"{p.get('name', 'Unknown')}({p.get('point', '?')})" for p in play['pieces']])
            starter_info = " (STARTER)" if play.get('is_starter', False) else ""
            print(f"  {i+1}. Game {play['game_id']}, Round {play['round']}, Turn {play['turn']} (req: {play.get('required_pieces', '?')} pieces)")
            print(f"      {play['player']}{starter_info} played {play['play_type']}: [{pieces_str}]")
        ...
        starter_plays = sum(1 for play in all_never_wins if play.get('is_starter', False))
        non_starter_plays = len(all_never_wins) - starter_plays
        print(f"\n🎯 Starter vs Non-starter:")
        print(f"  Starter plays: {starter_plays} ({starter_plays/len(all_never_wins)*100:.1f}%)")
        print(f"  Non-starter plays: {non_starter_plays} ({non_starter_plays/len(all_never_wins)*100:.1f}%)")
    else:
        if total_stats['total_plays'] > 0:
