...
def analyze_never_win_play(
    game_data: Dict, event: Dict, play: Dict, current_round: int, initial_hands: Dict
) -> Optional[Dict]:
    ...
    return {
        ...
        "piles_needed": player_declared - player_captured
        if player_declared is not None
        else None,
    }
...
def main():
    ...
    print(
        f"📁 Found {len(detailed_files)} detailed log files (out of {len(game_files)} total)"
    )
    ...
    print(
        f"  Strategic starter plays (intentionally weak): {len(starter_strategic_plays)}"
    )
    ...
    if poor_choices:
        ...
        print(
            f"   All {len(poor_choices)} cases are RESPONDERS who could have played stronger combos"
        )
        ...
        for i, analysis in enumerate(poor_choices[:10]):  # Show up to 10 examples
            ...
            print(
                f"   Player: {analysis['player']} ({'STARTER' if analysis['is_starter'] else 'RESPONDER'})"
            )
            print(
                f"   Declared: {analysis['declared']}, Captured: {analysis['captured']}, Piles needed: {analysis['piles_needed']}"
            )

            pieces_str = ", ".join(
                [f"{p['name']}({p['point']})" for p in analysis["pieces"]]
            )
            print(f"   Played: {analysis['play_type']} [{pieces_str}]")
            print(
                f"   Had {analysis['non_never_win_alternatives']} better alternatives!"
            )
            ...
            if analysis["best_alternative"]:
                ...
                print(
                    f"   Best alternative: {alt['type']} [{alt_str}] ({alt['points']} pts)"
                )

            print(
                f"   Remaining hand ({len(analysis['remaining_hand'])} pieces): {', '.join(analysis['remaining_hand'])}"
            )
    ...
    if forced_plays:
        ...
        for i, analysis in enumerate(forced_plays[:3]):
            ...
            print(
                f"   Had {analysis['total_alternatives']} valid plays, ALL were never-win combos"
            )
    ...
    if starter_strategic_plays:
        ...
        for i, analysis in enumerate(starter_strategic_plays[:3]):
            ...
            print(f"   Played: {analysis['play_type']} [{pieces_str}]")
            print(
                f"   This is a VALID strategic choice - starters can intentionally play weak"
            )
    ...
    if poor_choices:
        print(
            f"  • {len(poor_choices)} RESPONDER never-win plays could have been avoided"
        )
    ...
    if starter_strategic_plays:
        print(
            f"  • {len(starter_strategic_plays)} starter plays were strategic choices (NOT bugs)"
        )
