...
async def test_event_store():
    ...
    try:
        ...
        event2 = await event_store.store_event(
            ...
            payload={"player_name": "alice", "player_data": {"hand": ["Red 10", "Blue 12"]}},
            player_id="alice"
        )
        ...
        print(f"  ✅ Stored 3 events with sequences: {event1.sequence}, {event2.sequence}, {event3.sequence}")
        ...
        assert len(events_since_0) == 3, f"Expected 3 events since 0, got {len(events_since_0)}"
        assert len(events_since_2) == 1, f"Expected 1 event since 2, got {len(events_since_2)}"
