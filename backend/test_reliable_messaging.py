...
async def test_client_synchronization():
    ...
    try:
        ...
        mock_ws2 = AsyncMock()
        socket_manager.pending_messages["test_room"] = {7: PendingMessage(
            message={"test": "data"},
            websocket=mock_ws2,
            timestamp=time.time(),
            room_id="test_room"
...
async def test_websocket_integration():
    ...
    try:
        ...
        assert hasattr(sm, 'request_client_sync'), "request_client_sync method should exist"
...
async def test_statistics_and_monitoring():
    ...
    try:
        ...
        print(f"  ✅ Stats: {formatted_stats['success_rate']}% success, {formatted_stats['average_latency_ms']}ms latency")
...
async def test_performance_and_scalability():
    ...
    try:
        ...
        print(f"  ✅ Generated 1000 sequences in {generation_time:.3f}s ({1000/generation_time:.0f} seq/s)")
        ...
        assert pending_count == 100, f"Expected 100 pending messages, got {pending_count}"
        ...
        remaining_pending = len(socket_manager.pending_messages.get("perf_test_room", {}))
        assert remaining_pending == 0, f"Expected 0 pending messages, got {remaining_pending}"
        
        stats = socket_manager.get_message_stats("perf_test_room")
        assert stats["messages_acknowledged"] == 100
        assert stats["success_rate"] == 100.0
        
        print(f"  ✅ Acknowledged all 100 messages (success rate: {stats['success_rate']}%)")
