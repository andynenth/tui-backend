...
class MockGame:
    ...
    def __init__(self):
        ...
        for player in self.players:
            player.hand = [
                Piece("GENERAL_RED"), Piece("GENERAL_BLACK"), Piece("ADVISOR_RED"),
                Piece("ADVISOR_BLACK"), Piece("ELEPHANT_RED"), Piece("ELEPHANT_BLACK"),
                Piece("HORSE_RED"), Piece("HORSE_BLACK")
            ]
...
async def test_turn_state_action_processing():
    ...
    if initial_player != updated_player:
        print(f"✅ SUCCESS: Current player advanced from '{initial_player}' to '{updated_player}'")
    else:
    ...
    if required_count == len(pieces_to_play):
        ...
        print(f"❌ FAILURE: Required piece count incorrect. Expected {len(pieces_to_play)}, got {required_count}")
    ...
    if final_player != "Bot 1":
        ...
        print(f"❌ FAILURE: Current player did not advance from Bot 1, still '{final_player}'")
    ...
    if len(final_turn_plays) == 2:
        ...
        print(f"❌ FAILURE: Expected 2 plays, got {len(final_turn_plays)}: {list(final_turn_plays.keys())}")
...
async def test_action_queue_processing():
    ...
    actions = []
    piece_kinds = [("GENERAL_RED", "GENERAL_BLACK"), ("ADVISOR_RED", "ADVISOR_BLACK"), ("ELEPHANT_RED", "ELEPHANT_BLACK")]
