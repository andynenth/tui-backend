...
async def test_api_simulation():
    ...
    try:
        ...
        if prep_state and hasattr(prep_state, 'weak_players') and prep_state.weak_players:
            ...
            for player in weak_players_list:
                if hasattr(prep_state, 'current_weak_player') and prep_state.current_weak_player == player:
                    result = await simulate_redeal_decline(room_manager, room_id, player)
        ...
        if room.game_state_machine.current_phase == GamePhase.DECLARATION:
            ...
            for player, value in zip(players, declarations):
                result = await simulate_declare_endpoint(room_manager, room_id, player, value)
