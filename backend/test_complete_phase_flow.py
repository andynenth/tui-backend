...
class PhaseFlowTester:
    ...
    def log_event(self, event: str, data: Dict = None):
        ...
        current_phase = None
        if self.state_machine and hasattr(self.state_machine, 'current_phase') and self.state_machine.current_phase:
    ...
    def create_realistic_hands(self):
        ...
        while len(pieces) < 32:
            pieces.extend([Piece(kind) for kind in piece_kinds[:4]])  # Add some duplicates
    ...
    def setup_test_environment(self):
        ...
        self.log_event("Test environment setup complete", {
    ...
    async def test_waiting_phase(self):
        ...
        self.log_event(f"✅ Connected players tracked: {len(waiting_state.connected_players)}")
        ...
        assert self.state_machine.current_phase in [GamePhase.PREPARATION, GamePhase.DECLARATION]
        self.log_event(f"✅ Successfully transitioned from WAITING → {self.state_machine.current_phase.value}")
    ...
    async def test_turn_results_phase(self):
        ...
        self.log_event(f"✅ TURN_RESULTS auto-transitioned to {self.state_machine.current_phase.value}")
    ...
    def print_test_summary(self):
        ...
        print(f"⏱️  Test duration: {self.test_events[-1]['timestamp']} - {self.test_events[0]['timestamp']}")
