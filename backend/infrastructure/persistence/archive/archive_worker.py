...
class ArchivalWorker:
            ...
            while len(batch) < batch_size:
                try:
                    ...
                    request = await asyncio.wait_for(
                        queue.get(), timeout=min(remaining, 0.1)  # Check every 100ms
                    )
    ...
    async def _process_batch(self, batch: List[ArchivalRequest]) -> None:
        ...
        for entity_type, requests in by_type.items():
            ...
            results = await asyncio.gather(
                *[process_with_limit(req) for req in requests], return_exceptions=True
            )
    ...
    async def _process_request(
        self, request: ArchivalRequest, policy: ArchivalPolicy
    ) -> ArchivalResult:
        ...
        try:
            ...
            if result.success:
                ...
                logger.warning(f"Failed to archive {request.entity_id}: {result.error}")
    ...
    async def _monitor_loop(self) -> None:
        ...
        while self._running:
            try:
                ...
                if (
                    self.metrics.total_batches > 0
                    and self.metrics.total_batches % 100 == 0
                ):
                    logger.info(
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "metrics": {
                ...
                "last_batch_time": (
                    self.metrics.last_batch_time.isoformat()
