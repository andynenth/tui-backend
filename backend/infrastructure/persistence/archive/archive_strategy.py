...
class ArchivalStrategy(ABC):
    ...
    async def should_archive(
        self, entity_id: str, entity: Any, policy: ArchivalPolicy
    ) -> bool:
    ...
    async def archive_entity(
        ...
        try:
            ...
            location = await self.backend.archive(
                request.entity_id, request.entity_type, data
            )
...
class GameArchivalStrategy(ArchivalStrategy):
        ...
        archive_data = {
            ...
            "metadata": {
                ...
                "game_duration_seconds": (
                    getattr(entity, "completed_at", datetime.utcnow())
                    - entity.created_at
                ).total_seconds(),
            },
        }
    ...
    async def restore_from_archive(self, data: bytes) -> Any:
        ...
        game = Game(id=archive_data["id"], room_id=archive_data["room_id"])
