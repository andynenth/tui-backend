...
class FileSystemArchiveBackend:
    ...
    async def list_archives(
        ...
        try:
            ...
            if self.partition_by_date and (start_date or end_date):
                ...
                search_paths = self._get_date_partitions(entity_type, start_date, end_date)
            else:
    ...
    def _generate_path(self, entity_id: str, entity_type: str) -> Path:
        ...
        if self.partition_by_date:
            ...
            path = self.base_path / entity_type / date_str / id_hash[:2] / f"{entity_id}.json"
        else:
            # Just use hash distribution
            path = self.base_path / entity_type / id_hash[:2] / id_hash[2:4] / f"{entity_id}.json"
