...
from .archive_backends import FileSystemArchiveBackend, IArchivalBackend
...
class IEntityProvider(Protocol):
    ...
    async def list_entities(
        self, entity_type: str, filter_func: Optional[callable] = None
    ) -> List[Any]:
...
class ArchiveManager:
    ...
    def __init__(
        ...
        self.worker = None
        if enable_worker:
            self.worker = ArchivalWorker(strategy=self.strategy, policies=self.policies)
    ...
    async def retrieve_entity(self, entity_id: str, entity_type: str) -> Optional[Any]:
        """
        ...
        try:
            ...
            if not location:
                # Search in backend
                archives = await self.backend.list_archives(entity_type=entity_type)
    ...
    async def cleanup_old_archives(
        self, entity_type: Optional[str] = None, older_than: Optional[timedelta] = None
    ) -> int:
        """
        ...
        try:
            ...
            for etype in entity_types:
                ...
                # Get old archives
                archives = await self.backend.list_archives(
                    entity_type=etype, end_date=cutoff_date
                )
    ...
    async def backup_archives(
        self, destination: str, entity_type: Optional[str] = None
    ) -> int:
        """
