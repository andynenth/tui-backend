        ...
        if self.strategy == RepositoryStrategy.MEMORY_ONLY:
            if not self.memory_config:
                self.memory_config = PersistenceConfig(
                    backend=PersistenceBackend.MEMORY, options={"max_items": 100000}
                )

        elif self.strategy == RepositoryStrategy.HYBRID_ASYNC:
            if not self.memory_config:
                self.memory_config = PersistenceConfig(
                    backend=PersistenceBackend.MEMORY, options={"max_items": 10000}
                )
...
class AdapterRegistry:
    ...
    def create_adapter(
        self, backend: PersistenceBackend, config: PersistenceConfig
    ) -> IPersistenceAdapter:
        """
...
class RepositoryFactory:
    ...
    def _setup_defaults(self) -> None:
        ...
        self._default_configs["room"] = RepositoryConfig(
            ...
            memory_config=PersistenceConfig(
                ...
                options={"max_items": 10000, "enable_archives": False},
            ),
        )
        ...
        self._default_configs["game"] = RepositoryConfig(
            ...
            memory_config=PersistenceConfig(
                backend=PersistenceBackend.MEMORY,
                options={"max_items": 1000, "enable_archives": True},
            ),
        ...
        self._default_configs["player_stats"] = RepositoryConfig(
            ...
            memory_config=PersistenceConfig(
                backend=PersistenceBackend.MEMORY,
                options={"max_items": 50000, "enable_archives": True},
            ),
            archival_policy=TimeBasedArchivalPolicy(max_idle_time=timedelta(days=7)),
        )
        ...
        self._default_configs["connection"] = RepositoryConfig(
            ...
            memory_config=PersistenceConfig(
                backend=PersistenceBackend.MEMORY,
                options={"max_items": 100000, "enable_archives": False},
            ),
        )
    ...
    def _create_memory_repository(
        self, entity_class: Type[T], config: RepositoryConfig
    ) -> BaseRepository[T]:
        """Create a memory-only repository."""
        memory_adapter = self._adapter_registry.create_adapter(
            PersistenceBackend.MEMORY, config.memory_config
        )
    ...
    def _create_hybrid_repository(
        ...
        if config.persistent_config:
            persistent_adapter = self._adapter_registry.create_adapter(
                config.persistent_config.backend, config.persistent_config
            )
