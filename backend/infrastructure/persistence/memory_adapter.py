...
class MemoryAdapter(
    ...
    def __init__(self, config: Optional[PersistenceConfig] = None):
        ...
        # Metrics
        self._operations = {"gets": 0, "saves": 0, "deletes": 0, "queries": 0}
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            return {
                ...
                "total_archives": sum(
                    len(archives) for archives in self._archives.values()
                ),
    ...
    async def prune_archives(self, before_date: datetime) -> int:
        ...
        async with self._lock:
            ...
            for key in list(self._archives.keys()):
                ...
                self._archives[key] = [
                    (ts, entity)
                    for ts, entity in self._archives[key]
                    if ts >= before_date
                ]
    ...
    async def _archive_internal(
        self, key: str, entity: T, timestamp: Optional[datetime] = None
    ) -> None:
