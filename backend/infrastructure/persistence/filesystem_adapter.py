...
class FilesystemAdapter(
    IPersistenceAdapter[T], IQueryableAdapter[T], IArchivableAdapter[T], Generic[T]
):
    """
    ...
    def __init__(self, config: Optional[PersistenceConfig] = None):
        ...
        self._base_path = Path(
            self._config.options.get("base_path", "./data/persistence")
        )
        self._archive_path = Path(
            self._config.options.get("archive_path", "./data/archives")
        )
        ...
        self._index: Dict[str, Dict[str, List[str]]] = defaultdict(
            lambda: defaultdict(list)
        )
        self._lock = asyncio.Lock()

        # Metrics
        self._operations = {"reads": 0, "writes": 0, "deletes": 0, "queries": 0}
    ...
    async def delete_many(self, keys: List[str]) -> int:
        ...
        results = await asyncio.gather(*[self.delete(key) for key in keys])
    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        # Calculate size
        total_size = sum(f.stat().st_size for f in self._base_path.rglob("*.json"))
    ...
    async def query_sorted(
        ...
        # Sort
        results.sort(
            key=lambda e: self._get_field_value(e, sort_field), reverse=not ascending
        )
    ...
    async def _archive_file(self, key: str, file_path: Path) -> None:
        ...
        try:
            ...
            await self.archive(
                key, entity, {"timestamp": datetime.utcnow(), "reason": "auto_archive"}
            )
        except Exception:
            pass
