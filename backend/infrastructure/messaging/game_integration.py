...
class GameEventQueue:
    ...
    def __init__(
        ...
        self._dlq = DeadLetterQueue(
            f"{name}_dlq",
            DeadLetterPolicy(max_retries=3, ttl=timedelta(hours=24))
        )
    ...
    async def publish(
        ...
        # Determine priority
        if event.event_type in (GameEventType.ERROR_OCCURRED, GameEventType.PLAYER_DISCONNECTED):
        ...
        logger.debug(f"Published event {event.event_type.value} to {len(destinations)} queues")
...
class GameTaskProcessor:
    ...
    def __init__(
        self,
        task_queue: Optional[PriorityInMemoryQueue] = None,
        max_workers: int = 5
    ):
