...
class DeadLetterPolicy:
    ...
    def should_dead_letter(self, message: Message[Any], error: Optional[Exception] = None) -> bool:
    ...
    def calculate_retry_delay(self, attempts: int) -> timedelta:
        ...
        delay_seconds = self.retry_delay.total_seconds() * (self.backoff_multiplier ** (attempts - 1))
...
class DeadLetterQueue:
    ...
    async def retry(
        self,
        message_id: str,
        target_queue: Optional[IMessageQueue[Any]] = None
    ) -> bool:
...
class RetryableDeadLetterQueue(DeadLetterQueue):
    ...
    def register_retry_queue(
        self,
        queue_name: str,
        queue: IMessageQueue[Any]
    ) -> None:
    ...
    async def add(
        ...
        if (original_queue and 
            original_queue in self._retry_queues and
            reason in (DeadLetterReason.PROCESSING_FAILED, DeadLetterReason.TTL_EXPIRED)):
    ...
    async def _scheduled_retry(
        ...
        try:
            ...
            if success:
                logger.info(f"Successfully retried message {message_id} to queue {queue_name}")
            else:
