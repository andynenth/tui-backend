...
from .base import Message, IMessageRouter, IMessageQueue, T
...
class MessageRouter(IMessageRouter):
    ...
    def __init__(self, name: str = "router"):
        ...
        self._stats = {"routed": 0, "unroutable": 0, "by_destination": {}}
    ...
    def register_route(
        ...
        if filter_func:
            ...
            if existing_filter:
                route.filter_func = lambda msg: existing_filter(msg) and filter_func(
                    msg
                )
            else:
    ...
    def get_routes(self) -> Dict[str, List[str]]:
        ...
        return {
            pattern: list(route.destinations) for pattern, route in self._routes.items()
        }
...
class ContentBasedRouter(MessageRouter):
    ...
    def add_content_rule(
        self, rule_func: Callable[[Message[Any]], Optional[str]], priority: int = 0
    ) -> None:
        """
