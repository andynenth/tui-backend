        ...
        try:
            ...
            data = {
                ...
                'metadata': {
                    ...
                    'expires_at': message.metadata.expires_at.isoformat() 
                        if message.metadata.expires_at else None
                },
    ...
    def deserialize(self, data: bytes) -> Message[Any]:
        ...
        try:
            ...
            metadata = MessageMetadata(
                ...
                expires_at=datetime.fromisoformat(metadata_data['expires_at'])
                    if metadata_data.get('expires_at') else None
            )
        ...
        try:
            ...
            data = {
                ...
                'metadata': {
                    ...
                    'attempts': message.metadata.attempts,
                    'last_error': message.metadata.last_error,
                    'expires_at': message.metadata.expires_at.timestamp()
                        if message.metadata.expires_at else None
                },
                'status': message.status.value,
                'priority': message.priority.value
    ...
    def deserialize(self, data: bytes) -> Message[Any]:
        ...
        try:
            ...
            metadata = MessageMetadata(
                ...
                attempts=metadata_data.get('attempts', 0),
                last_error=metadata_data.get('last_error'),
                expires_at=datetime.fromtimestamp(metadata_data['expires_at'])
                    if metadata_data.get('expires_at') else None
            )
...
class CompositeSerializer(IMessageSerializer[Any]):
    ...
    def _looks_like_msgpack(self, data: bytes) -> bool:
        ...
        return (
            ...
            (first_byte in (0xdc, 0xdd, 0xde, 0xdf))  # array/map markers
        )