...
class GameMetricsCollector:
    ...
    def __init__(
        ...
        self._hourly_stats: defaultdict = defaultdict(lambda: {
            'games_started': 0,
            'games_completed': 0,
            'games_abandoned': 0,
            'total_players': 0,
        ...
        # Player statistics
        self._player_stats: defaultdict = defaultdict(lambda: {
            'games_played': 0,
            'games_won': 0,
            'total_score': 0,
            'actions_taken': 0,
        ...
        if abandoned:
            ...
            if winner_id:
                ...
                self.collector.increment(
                    ...
                    tags=[MetricTag("player_type", "human" if not winner_id.startswith("bot_") else "bot")]
                )
    ...
    async def record_player_action(
        ...
        self.collector.increment(
            ...
            tags=[
                ...
                MetricTag("player_type", "human" if not player_id.startswith("bot_") else "bot")
            ]
        )
    ...
    async def record_state_transition(
        ...
        self.collector.increment(
            ...
            tags=[
                MetricTag("from_state", from_state),
                MetricTag("to_state", to_state)
            ]
        )
    ...
    async def record_websocket_broadcast(
        ...
        if success:
            ...
            self.collector.increment(
                f"{GameMetricType.WS_MESSAGES.value}.broadcast",
                recipient_count
            )
        else:
            self.collector.increment(
    ...
    def _calculate_games_per_hour(self) -> float:
        ...
        total_games = sum(stats['games_started'] for stats in self._hourly_stats.values())
    ...
    def _cleanup_old_data(self) -> None:
        ...
        while self._state_transitions and self._state_transitions[0]['timestamp'] < cutoff:
