...
class PrometheusExporter:
    ...
    def _format_game_metrics(self) -> List[str]:
        ...
        lines.append(
            "# HELP liap_tui_games_completed_total Total number of completed games"
        )
        lines.append(
            f"liap_tui_games_completed_total {analytics.get('completed_games', 0)}"
        )
        ...
        lines.append(
            "# HELP liap_tui_game_duration_minutes Average game duration in minutes"
        )
        lines.append(
            f"liap_tui_game_duration_minutes {analytics.get('avg_duration_minutes', 0)}"
        )
        ...
        lines.append(
            f'liap_tui_player_win_rate{{type="bot"}} {player_metrics.get("bot_win_rate", 0)}'
        )
        lines.append(
            f'liap_tui_player_win_rate{{type="human"}} {player_metrics.get("human_win_rate", 0)}'
        )
        ...
        for phase, stats in phase_metrics.items():
            lines.append(
                f'liap_tui_phase_duration_seconds_sum{{phase="{phase}"}} {stats["avg_seconds"] * 100}'
            )  # Fake sum
            lines.append(
                f'liap_tui_phase_duration_seconds_count{{phase="{phase}"}} 100'
            )  # Fake count
            ...
            for bucket in buckets:
                ...
                lines.append(
                    f'liap_tui_phase_duration_seconds_bucket{{phase="{phase}",le="{bucket}"}} {count}'
                )
            lines.append(
                f'liap_tui_phase_duration_seconds_bucket{{phase="{phase}",le="+Inf"}} 100'
            )

        # WebSocket metrics
        lines.append("# TYPE liap_tui_websocket_connections_active gauge")
        lines.append(
            "# HELP liap_tui_websocket_connections_active Number of active WebSocket connections"
        )
        lines.append(
            f"liap_tui_websocket_connections_active {len(self.game_metrics._active_connections)}"
        )
    ...
    def _format_system_metrics(self) -> List[str]:
        ...
        lines.append(
            f"liap_tui_memory_usage_mb{{type=\"rss\"}} {memory_summary.get('current_rss_mb', 0)}"
        )
        lines.append(
            f"liap_tui_memory_usage_mb{{type=\"vms\"}} {memory_summary.get('current_vms_mb', 0)}"
        )

        lines.append("# TYPE liap_tui_memory_growth_rate gauge")
        lines.append(
            "# HELP liap_tui_memory_growth_rate Memory growth rate in MB per hour"
        )
        lines.append(
            f"liap_tui_memory_growth_rate {memory_summary.get('growth_rate_mb_per_hour', 0)}"
        )
        ...
        lines.append(
            "# HELP liap_tui_gc_collections_total Total garbage collections by generation"
        )

        for gen, stats in gc_summary.get("by_generation", {}).items():
            lines.append(
                f"liap_tui_gc_collections_total{{generation=\"{gen}\"}} {stats['collections']}"
            )

        lines.append("# TYPE liap_tui_gc_objects_collected_total counter")
        lines.append(
            "# HELP liap_tui_gc_objects_collected_total Total objects collected by GC"
        )
        lines.append(
            f"liap_tui_gc_objects_collected_total {gc_summary.get('total_collected', 0)}"
        )
    ...
    def _format_game_analytics(self) -> List[str]:
        ...
        lines.append(
            'liap_tui_business_metric{name="daily_active_users"} 0'
    ...
    def _format_visualization_metrics(self) -> List[str]:
        ...
        for state_name, node in viz_provider._state_nodes.items():
            lines.append(
                f'liap_tui_state_active_games{{state="{state_name}"}} {node.active_games}'
            )
