...
request_metadata_var: ContextVar[Dict[str, Any]] = ContextVar(
...
class CorrelationContext:
    ...
    def cleanup_old_correlations(self, max_age_hours: int = 1) -> int:
        ...
        old_ids = [
            corr_id
            for corr_id, data in self._active_correlations.items()
...
def with_correlation(
    generate_new: bool = True, propagate: bool = True
) -> Callable[[T], T]:
    """
...
async def propagate_correlation_to_task(
    coro: Any, correlation_id: Optional[str] = None
) -> Any:
    """
...
def inject_correlation_header(
    headers: Dict[str, str], correlation_id: Optional[str] = None
) -> Dict[str, str]:
    """
...
def extract_correlation_header(
    headers: Dict[str, str], header_name: str = "X-Correlation-ID"
) -> Optional[str]:
    """
    Extract correlation ID from headers.
