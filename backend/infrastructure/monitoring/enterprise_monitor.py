...
class EnterpriseMonitor:
    ...
    async def record_game_start(
        ...
        event = SystemEvent(
            ...
            game_id=game_id,
            room_id=room_id,
            data={"player_count": player_count, "bot_count": bot_count},
        )
    ...
    async def record_websocket_event(
        ...
        if event_type == "connect":
            await self.game_metrics.record_websocket_connection(
                connection_id=connection_id, connected=True
            )

            event = SystemEvent(
                event_type=EventType.CONNECTION_OPENED, room_id=room_id, data=data or {}
            )
            await self.event_stream.publish_event(event)

        elif event_type == "disconnect":
            await self.game_metrics.record_websocket_connection(
                connection_id=connection_id, connected=False
            )

            event = SystemEvent(
                event_type=EventType.CONNECTION_CLOSED, room_id=room_id, data=data or {}
            )
            await self.event_stream.publish_event(event)
    ...
    async def record_phase_duration(
        ...
        await self.game_metrics.record_phase_duration(
            game_id=game_id, phase=phase, duration_seconds=duration_seconds
        )

        # Update visualization
        self.visualization.update_state_node(phase, duration_sample=duration_seconds)
    ...
    async def subscribe_to_events(
        self, subscriber_id: Optional[str] = None, event_filter: Optional[Any] = None
    ) -> Any:
