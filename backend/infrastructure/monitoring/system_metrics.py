...
class SystemMetricsCollector:
    ...
    def _collect_memory_metrics(self) -> Dict[str, float]:
        ...
        self.collector.gauge("system.memory.available_gb", sys_mem.available / (1024**3))
    ...
    def _collect_fd_metrics(self) -> None:
        ...
        try:
            ...
            for conn in connections:
                conn_type = f"{conn.type}_{conn.status}" if hasattr(conn, 'status') else str(conn.type)
                conn_types[conn_type] = conn_types.get(conn_type, 0) + 1
            
            for conn_type, count in conn_types.items():
                self.collector.gauge(
                    "system.connections.by_type",
                    count,
                    [MetricTag("type", conn_type)]
                )
    ...
    def _collect_gc_metrics(self) -> None:
        ...
        for i, gen_stats in enumerate(stats):
            self.collector.gauge(
                f"system.gc.collections.gen{i}",
                gen_stats.get('collections', 0)
            )
            self.collector.gauge(
                f"system.gc.collected.gen{i}",
                gen_stats.get('collected', 0)
            )
            self.collector.gauge(
                f"system.gc.uncollectable.gen{i}",
                gen_stats.get('uncollectable', 0)
            )
    ...
    def record_gc_collection(self, generation: int, collected: int, uncollectable: int) -> None:
        """Record a garbage collection event."""
        self.collector.increment(
            "system.gc.events",
            tags=[MetricTag("generation", str(generation))]
        )
    ...
    def get_memory_summary(self) -> Dict[str, Any]:
        ...
        if len(recent) > 1:
            ...
            time_diff = (current['timestamp'] - first['timestamp']).total_seconds() / 3600
