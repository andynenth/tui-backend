...
class StateVisualizationProvider:
    ...
    def get_performance_metrics(self, window_minutes: int = 60) -> Dict[str, Any]:
        ...
        recent_data = [
            d for d in self._performance_timeline
            if datetime.fromisoformat(d['timestamp']) > cutoff
        ]
        
        # Calculate aggregates
        if recent_data:
            avg_response_time = sum(d.get('response_time', 0) for d in recent_data) / len(recent_data)
            error_rate = sum(1 for d in recent_data if d.get('error', False)) / len(recent_data)
        else:
            avg_response_time = 0
            error_rate = 0
    ...
    def cleanup_old_data(self, hours: int = 24) -> None:
        ...
        inactive_rooms = [
            room_id for room_id, room in self._room_visualizations.items()
