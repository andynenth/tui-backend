...
class EventMetrics:
    ...
    def record_event(self, duration_ms: float, success: bool, error_type: Optional[str] = None):
...
class ConnectionMetrics:
    ...
    def record_disconnection(self, room_id: str, duration_sec: float):
        ...
        if room_id in self.connections_by_room:
            self.connections_by_room[room_id] = max(0, self.connections_by_room[room_id] - 1)
        ...
        if self.total_disconnections > 0:
            self.avg_connection_duration_sec = (
                (self.avg_connection_duration_sec * (self.total_disconnections - 1) + duration_sec) /
...
class MessageMetrics:
    ...
    message_size_bytes: Dict[str, List[int]] = field(default_factory=lambda: defaultdict(list))
...
class WebSocketMetricsCollector:
    ...
    async def record_broadcast(self, room_id: str, event_type: str, recipient_count: int):
    ...
    def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "connections": {
                ...
                "avg_duration_sec": round(self.connection_metrics.avg_connection_duration_sec, 2),
            ...
            "messages": {
                ...
                "avg_broadcast_recipients": round(self.message_metrics.avg_broadcast_recipients, 2)
            },
            "events": {
                event_name: {
                    "count": metrics.count,
    ...
    def get_time_series_data(self, minutes: int = 5) -> Dict[str, Any]:
        ...
        # Filter events
        recent_events = [
            e for e in self.time_series_events
            if e["timestamp"] > cutoff
        ]
        
        # Filter connections
        recent_connections = [
            c for c in self.time_series_connections
    ...
    async def _cleanup_old_data(self):
        ...
        while True:
            try:
                ...
                # Clean up old events
                while self.time_series_events and self.time_series_events[0]["timestamp"] < cutoff:
                    self.time_series_events.popleft()
                
                # Clean up old connections
                while self.time_series_connections and self.time_series_connections[0]["timestamp"] < cutoff:
