...
class EventStream:
    ...
    def __init__(self, history_size: int = 10000, max_subscribers: int = 100):
    ...
    async def subscribe(
        self, subscriber_id: Optional[str] = None, filter: Optional[EventFilter] = None
    ) -> EventSubscriber:
        ...
        async with self._lock:
            if len(self._subscribers) >= self.max_subscribers:
                raise RuntimeError(
                    f"Maximum subscribers ({self.max_subscribers}) reached"
                )
    ...
    def get_history(
        self, limit: Optional[int] = None, filter: Optional[EventFilter] = None
    ) -> List[SystemEvent]:
        """Get event history."""
    ...
    async def cleanup_inactive_subscribers(
        self, inactive_timeout_minutes: int = 30
    ) -> int:
...
class EventLogger:
    ...
    async def log_state_transition(
        ...
        event_type = (
            EventType.STATE_TRANSITION_COMPLETE
            if success
    ...
    async def log_action(
        ...
        event = SystemEvent(
            ...
            player_id=player_id,
            correlation_id=correlation_id,
            data={"action_type": action_type, "status": status, **(data or {})},
            metadata=self.default_metadata,
        )
...
def configure_event_stream(
    history_size: int = 10000, max_subscribers: int = 100
) -> EventStream:
