...
class StateTracer:
    ...
    def __init__(
        self, service_name: str = "liap-tui", enable_console_export: bool = False
    ):
    ...
    def start_span(self, name: str, kind: SpanKind = SpanKind.STATE_TRANSITION) -> Any:
        ...
        if self._tracer:
            return self._tracer.start_span(
                name, kind=trace.SpanKind.INTERNAL, attributes={"span.kind": kind.value}
            )
    ...
    def trace_action_processing(
        ...
        with self.start_span(span_name, SpanKind.ACTION_PROCESSING) as span:
            ...
            try:
                ...
                span.add_event(
                    "action_failed", {"error": str(e), "action_type": action_type}
                )
    ...
    def trace_phase_execution(
        self, game_id: str, phase: str, context: Optional[TraceContext] = None
    ) -> ContextManager[Any]:
        ...
        with self.start_span(span_name, SpanKind.PHASE_EXECUTION) as span:
            ...
            try:
                ...
                span.add_event(
                    "phase_completed", {"phase": phase, "duration_seconds": duration}
                )
                ...
                span.add_event("phase_failed", {"error": str(e), "phase": phase})

                if OTEL_AVAILABLE:
                    span.set_status(Status(StatusCode.ERROR, f"Phase failed: {str(e)}"))
    ...
    def propagate_context(self, context: TraceContext, carrier: Dict[str, Any]) -> None:
        ...
        **kwargs,
        ...
        with self.start_span(operation_name) as span:
            ...
            try:
                ...
                span.add_event(
                    "operation_failed", {"error": str(e), "operation": operation_name}
                )
...
def configure_tracing(
    service_name: str = "liap-tui", enable_console_export: bool = False
) -> StateTracer:
