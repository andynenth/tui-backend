...
class PerformanceMonitor:
    ...
    def get_current_metrics(self) -> PerformanceMetrics:
        ...
        with self._lock:
            ...
            durations = [
                ...
                if (datetime.utcnow() - entry['timestamp']).total_seconds() < self.window_size_seconds
            ]
            ...
            if durations:
                ...
                for p in self.percentiles:
                    ...
                    percentiles[p] = sorted_durations[min(idx, len(sorted_durations) - 1)]
    ...
    def get_error_rate(self, window_seconds: Optional[int] = None) -> float:
        ...
        with self._lock:
            total_requests = sum(
                1 for entry in self._request_durations
                if entry['timestamp'] > cutoff
            )
            
            error_count = sum(
                1 for timestamp in self._error_timestamps
                if timestamp > cutoff
            )
    ...
    def _cleanup_old_data(self) -> None:
        ...
        with self._lock:
            ...
            while self._request_durations and self._request_durations[0]['timestamp'] < cutoff:
        ...
        with self._lock:
            for rule_name, rule in self._rules.items():
                ...
                if message:
                    ...
                    if self._consecutive_failures[rule_name] >= rule.consecutive_failures:
                        alert = Alert(
                            rule_name=rule_name,
        ...
        with self._lock:
            ...
            return {
                ...
                'bot_metrics': {
                    ...
                    'avg_ms': statistics.mean(recent_bot_times) if recent_bot_times else 0,
                    'max_ms': max(recent_bot_times) if recent_bot_times else 0,
                    'within_target': sum(1 for t in recent_bot_times if 500 <= t <= 1500) / len(recent_bot_times) if recent_bot_times else 0
                },
                'websocket_metrics': {
                    ...
                    'p99_ms': self._percentile(recent_latencies, 99) if recent_latencies else 0
                }
            }
