...
class SpanContext:
    ...
    def from_traceparent(cls, traceparent: str) -> "SpanContext":
        ...
        return cls(trace_id=parts[1], span_id=parts[2], trace_flags=int(parts[3], 16))
...
class ITracer(ABC):
    ...
    @abstractmethod
    def inject(
        self, carrier: Dict[str, Any], context: Optional[SpanContext] = None
    ) -> None:
...
class InMemoryTracer(ITracer):
    ...
    def _start_export_worker(self) -> None:
        ...
        self._export_task = threading.Thread(target=export_loop, daemon=True)
...
def configure_tracing(
    app_name: str = "liap-tui", config: Optional[TraceConfig] = None
) -> None:
