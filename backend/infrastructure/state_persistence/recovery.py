...
class RecoveryStrategy(ABC):
    ...
    @abstractmethod
    async def recover(
        self, state_machine_id: str, options: RecoveryOptions
    ) -> RecoveryResult:
...
class SnapshotRecovery(RecoveryStrategy):
        ...
        try:
            if options.mode == RecoveryMode.SNAPSHOT and options.snapshot_id:
                ...
                return await self._recover_specific_snapshot(
                    options.snapshot_id, options
                )

            elif options.mode == RecoveryMode.LATEST:
                # Recover latest snapshot
                return await self._recover_latest_snapshot(state_machine_id, options)

            elif (
                options.mode == RecoveryMode.POINT_IN_TIME and options.target_timestamp
            ):
                # Find snapshot closest to target time
                return await self._recover_point_in_time(
                    state_machine_id, options.target_timestamp, options
                )
            ...
            return RecoveryResult(status=RecoveryStatus.FAILED, error=str(e))
    ...
    async def _recover_specific_snapshot(
        self, snapshot_id: str, options: RecoveryOptions
    ) -> RecoveryResult:
        ...
        return RecoveryResult(
            status=RecoveryStatus.NOT_FOUND, error=f"Snapshot {snapshot_id} not found"
        )
    ...
    async def _recover_latest_snapshot(
        ...
        for store in self.snapshot_stores:
            try:
                ...
                if snapshots and (
                    not latest_point or snapshots[0].timestamp > latest_point.timestamp
                ):
            ...
            if state:
                return RecoveryResult(
                    ...
                    metadata={
                        ...
                        "time_difference": (
                            target_timestamp - best_point.timestamp
                        ).total_seconds(),
                    },
                )
...
class EventSourcedRecovery(RecoveryStrategy):
    ...
    def __init__(
        ...
        transition_applier: Optional[
            Callable[[Dict[str, Any], StateTransition], Dict[str, Any]]
        ] = None,
    ):
            ...
            try:
                if options.mode == RecoveryMode.LATEST:
                    ...
                    return await log.get_transitions(
                        state_machine_id, limit=options.max_transitions_to_replay
                    )
...
class HybridRecovery(RecoveryStrategy):
    ...
    async def recover(
        ...
        try:
            ...
            snapshot_result = await self.snapshot_recovery.recover(
                state_machine_id,
                RecoveryOptions(
                    mode=RecoveryMode.LATEST, validate_state=options.validate_state
                ),
            )
            ...
            # Check snapshot age
            snapshot_age = (
                datetime.utcnow() - snapshot_result.recovered_state.created_at
            )
            ...
            transitions = await self._get_transitions_since_snapshot(
                state_machine_id, snapshot_result.recovered_state.created_at
            )
    ...
    def supports_mode(self, mode: RecoveryMode) -> bool:
        """Check if strategy supports recovery mode."""
        return self.snapshot_recovery.supports_mode(
            mode
        ) or self.event_recovery.supports_mode(mode)

    async def _get_transitions_since_snapshot(
        self, state_machine_id: str, snapshot_timestamp: datetime
    ) -> List[StateTransition]:
        """Get transitions since snapshot."""
        for log in self.event_recovery.transition_logs:
            try:
                return await log.get_transitions(
                    state_machine_id, from_timestamp=snapshot_timestamp
                )
            except Exception as e:
                logger.error(f"Error getting transitions: {e}")
                continue
...
class StateRecoveryManager(IStateRecovery):
    ...
    async def create_recovery_point(
        ...
        return RecoveryPoint(
            ...
            metadata={"created_by": "recovery_manager", "state_size": len(str(state))},
        )
    ...
    async def recover_state(
        ...
        if recovery_point.state_snapshot:
            ...
            return PersistedState(
                ...
                current_state=recovery_point.state_snapshot.get(
        ...
        options = RecoveryOptions(
            mode=RecoveryMode.SNAPSHOT, snapshot_id=recovery_point.recovery_id
        )

        result = await self.recover_with_options(
            recovery_point.state_machine_id, options
        )
    ...
    async def validate_recovery(
        self, state_machine_id: str, recovered_state: Dict[str, Any]
    ) -> bool:
    ...
    async def recover_with_options(
        ...
        if result.is_successful and options.validate_state and result.recovered_state:
            is_valid = await self.validate_recovery(
                state_machine_id, result.recovered_state.state_data
            )
    ...
    def _select_strategy(self, mode: RecoveryMode) -> Optional[RecoveryStrategy]:
        ...
        return (
            self.default_strategy if self.default_strategy.supports_mode(mode) else None
        )
