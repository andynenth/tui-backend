...
class StatePersistenceManager:
    ...
    def __init__(
        ...
        ) if config.snapshot_enabled else None
        
        self.transition_logger = StateTransitionLogger(
            stores=transition_logs,
            analyze_patterns=True
        ) if config.event_sourcing_enabled else None
    ...
    def _initialize_strategies(self) -> None:
        ...
        # Hybrid strategy
            ...
            if not self.snapshot_manager or not self.event_store:
                raise ValueError("Both snapshot and event store required for hybrid strategy")
            
            self._persistence_impls[PersistenceStrategy.HYBRID] = (
                HybridPersistence(
    ...
    async def save_state(
        ...
        try:
            ...
            if not impl:
                raise ValueError(f"No implementation for strategy: {self.config.strategy}")
        ...
        try:
            ...
            if self.config.cache_enabled and not version:
                ...
                if cached:
                    cache_hit = True
                    duration_ms = (datetime.utcnow() - start_time).total_seconds() * 1000
                    self.metrics.get_metrics(state_machine_id).record_load(duration_ms, True)
            ...
            # Update metrics
            duration_ms = (datetime.utcnow() - start_time).total_seconds() * 1000
            self.metrics.get_metrics(state_machine_id).record_load(duration_ms, cache_hit)
    ...
    async def handle_transition(
        ...
        if policy.should_persist('transition'):
            ...
            if transition.metadata.get('is_phase_change') and policy.persist_on_phase_change:
                # Load current state
                current = await self.load_state(state_machine_id)
                if current:
                    await self.save_state(state_machine_id, current.state_data, force=True)
        ...
        if self._pending_persists:
            ...
            for state_machine_id in list(self._pending_persists):
                try:
                    ...
                    logger.error(f"Error persisting {state_machine_id} on shutdown: {e}")
        ...
        while not self._shutdown:
            try:
                ...
                if to_persist:
                    ...
                    for batch_start in range(0, len(to_persist), self.config.batch_size):
                        batch = to_persist[batch_start:batch_start + self.config.batch_size]
