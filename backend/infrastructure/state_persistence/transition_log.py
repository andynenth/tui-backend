...
class InMemoryTransitionLog(TransitionLogStore, IStateTransitionLog):
    ...
    async def log_transition(
        ...
        async with self._lock:
            ...
            self._by_state_machine[state_machine_id].append(
            ...
            if state_machine_id in self._state_machines:
                self._state_machines[state_machine_id] = await self._apply_transition(
                    self._state_machines[state_machine_id],
                    transition
                )
    ...
    async def replay_transitions(
        ...
        for transition in transitions:
            try:
                current_state = await self._apply_transition(
                    current_state,
                    transition
                )
            except Exception as e:
                logger.error(
                    f"Error replaying transition {transition.action}: {e}"
                )
    ...
    async def query_transitions(
        ...
        async with self._lock:
            ...
            for event in events_to_check:
                ...
                if query.transition_type and event.transition_type != query.transition_type:
                    continue
...
class FileSystemTransitionLog(TransitionLogStore, IStateTransitionLog):
    ...
    async def log_transition(
        ...
        async with self._lock:
            ...
            self._file_sizes[log_file] = self._file_sizes.get(log_file, 0) + len(log_entry)
...
class StateTransitionLogger:
    ...
    def __init__(
        self,
        stores: List[IStateTransitionLog],
        analyze_patterns: bool = True
    ):
        """Initialize transition logger."""
        self.stores = stores
        self.analyze_patterns = analyze_patterns
        self._pattern_analyzer = TransitionPatternAnalyzer() if analyze_patterns else None
...
class TransitionPatternAnalyzer:
    ...
    async def analyze_transition(
        ...
        pattern_key = f"{transition.from_state}:{transition.action}:{transition.to_state}"
