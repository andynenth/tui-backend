...
class SmartInvalidator:
    ...
    async def _immediate_invalidation(
        self, rule: InvalidationRule, event_data: Any
    ) -> int:
    ...
    async def _cascade_invalidation(
        ...
        if rule.cascade_rules:
            for cascade_rule in rule.cascade_rules:
                ...
                cascade_event = {**event_data, "cascade_from": rule.event_type}
                invalidated = await self._immediate_invalidation(
                    cascade_rule, cascade_event
                )
                total += invalidated
    ...
    async def _pattern_invalidation(
        ...
        all_keys = (
            await self.cache.list_keys() if hasattr(self.cache, "list_keys") else []
        )
...
async def warm_frequently_accessed(
    cache: ICache, access_log: List[str], data_source: Callable, top_n: int = 100
) -> int:
    """
...
async def warm_by_pattern(
    cache: ICache, pattern: str, data_source: Callable, key_generator: Callable
) -> int:
    """
