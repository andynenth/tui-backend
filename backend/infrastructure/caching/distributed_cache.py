    ...
    async def exists(self, *keys: str) -> int:
        async with self._lock:
            ...
            for key in keys:
                if key in self._storage:
                    if (
                        key not in self._expiry
                        or datetime.utcnow() <= self._expiry[key]
                    ):
                        count += 1
            return count
...
class DistributedCache(IFullCache, IDistributedCache):
        ...
        try:
            ...
            if tags:
                ...
                await self._client.set(
                    meta_key, self._serialize(list(tags)), ex=ttl_seconds
                )
    ...
    async def lock(self, key: str, timeout: timedelta) -> "DistributedCacheLock":
        ...
        return DistributedCacheLock(lock_key, lock_value, self._client, timeout)
...
class DistributedCacheLock(CacheLock):
    ...
    def __init__(self, key: str, value: str, client: IRedisClient, timeout: timedelta):
        ...
        while not acquired:
            ...
            if existing is None:
                ...
                await self.client.set(
                    self.key, self.value.encode("utf-8"), ex=timeout_seconds
                )
