...
class MemoryManager:
    ...
    def force_gc(self, full: bool = True) -> int:
        ...
        with self._lock:
            ...
            logger.info(
                f"GC collected {collected} objects " f"(objects: {before} -> {after})"
            )
    ...
    def track_allocation(
        self, size: int, tag: Optional[str] = None, obj: Optional[Any] = None
    ) -> int:
        """
        ...
        with self._lock:
            ...
            allocation = MemoryAllocation(size=size, traceback=tb, tag=tag)
    ...
    def get_memory_leaks(
        self, age_threshold: timedelta = timedelta(minutes=5)
    ) -> List[Dict[str, Any]]:
        """
