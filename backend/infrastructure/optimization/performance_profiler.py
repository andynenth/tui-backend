...
class ProfileReport:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "overhead_percentage": (
                ((self.duration - total_time) / self.duration * 100)
                if self.duration > 0
                else 0
            ),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {"summary": self.get_summary(), "scopes": self.root_scope.to_dict()}
    ...
    def to_text(self, indent: int = 2) -> str:
        ...
        def format_scope(scope: ProfileScope, level: int = 0):
            ...
            for child in sorted(
                scope.children.values(), key=lambda s: s.timing.total_time, reverse=True
            ):
...
def profile(
    operation: Optional[str] = None, profiler_name: str = "default", **metadata
) -> Callable:
    """
