...
class TokenBucketRateLimiter(IRateLimiter):
    ...
    def __init__(
        ...
        self._refill_interval = 1.0 / config.refill_rate if config.refill_rate > 0 else float('inf')
...
class AdaptiveTokenBucket(TokenBucketRateLimiter):
    ...
    async def consume(self, key: str, cost: int = 1) -> bool:
        ...
        self._usage_history[key] = [
            (ts, c) for ts, c in self._usage_history[key]
            if ts > cutoff
        ]
    ...
    async def _calculate_adaptive_capacity(self, key: str) -> int:
        ...
        total_usage = sum(
            cost for ts, cost in self._usage_history[key]
            if ts > window_start
        )
