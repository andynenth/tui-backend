...
class SlidingWindowRateLimiter(IRateLimiter):
    ...
    def __init__(
        ...
        if config.algorithm != RateLimitAlgorithm.SLIDING_WINDOW:
            raise ValueError(f"Expected SLIDING_WINDOW algorithm, got {config.algorithm}")
    ...
    async def check_rate_limit(self, key: str, cost: int = 1) -> RateLimitResult:
        ...
        if not allowed:
            ...
            retry_seconds = (requests_to_expire / count) * self.config.window_size.total_seconds()
    ...
    async def _cleanup_worker(self) -> None:
        ...
        while True:
            try:
                ...
                cutoff = datetime.utcnow() - self.config.window_size - timedelta(minutes=1)
...
class OptimizedSlidingWindow(IRateLimiter):
    ...
    def _get_or_create_buffer(self, key: str) -> 'CircularBuffer':
        ...
        if key not in self._buffers:
            self._buffers[key] = CircularBuffer(
                self.buckets,
