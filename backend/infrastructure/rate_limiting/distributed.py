...
class RedisRateLimitStore(RateLimitStore):
    ...
    async def set_bucket(self, key: str, data: Dict[str, Any], ttl: Optional[timedelta] = None) -> None:
        ...
        if 'last_refill' in bucket_data and isinstance(bucket_data['last_refill'], datetime):
            bucket_data['last_refill'] = bucket_data['last_refill'].isoformat()
        if 'created_at' in bucket_data and isinstance(bucket_data['created_at'], datetime):
