...
class ReconnectionHandler:
    ...
    async def attempt_recovery(
        ...
        if success:
            ...
            logger.info(f"Successfully recovered {old_connection_id} to {new_connection.connection_id}")
        else:
    ...
    async def _perform_recovery(
        ...
        try:
            ...
            await self.connection_manager.registry.update_connection_state(
                ...
                ConnectionState.AUTHENTICATED if context.player_id else ConnectionState.CONNECTED
            )
...
class ConnectionRecoveryManager:
    ...
    async def _on_disconnect(self, connection: ConnectionInfo) -> None:
        """Handle connection disconnection."""
        # Create recovery context
        context = await self.reconnection_handler.handle_disconnection(
            connection
        )
