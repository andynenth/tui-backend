...
class StateSnapshot:
    ...
    @classmethod
    def from_state(
        cls, entity_type: str, entity_id: str, version: int, state: Dict[str, Any]
    ) -> "StateSnapshot":
...
class FullSync(ISyncStrategy):
    ...
    def __init__(
        self, connection_manager: ConnectionManager, event_propagator: EventPropagator
    ):
    ...
    async def sync_state(
        ...
        snapshot = StateSnapshot.from_state(
            entity_type, entity_id, current_state.get("version", 0), current_state
        )
    ...
    async def handle_conflict(
        ...
        await self.sync_state(connection, entity_type, entity_id, server_state)
...
class DeltaSync(ISyncStrategy):
    ...
    async def sync_state(
        ...
        if client_version is None or client_version < server_version - 10:
            await self._send_full_state(
                connection, entity_type, entity_id, current_state
            )
            return
        ...
        deltas = self._get_deltas(
            entity_type, entity_id, client_version, server_version
        )
    ...
    def _get_deltas(
        self, entity_type: str, entity_id: str, from_version: int, to_version: int
    ) -> List[StateDelta]:
    ...
    async def _send_full_state(
        ...
        """Send full state to client."""
        snapshot = StateSnapshot.from_state(
            entity_type, entity_id, state.get("version", 0), state
        )
    ...
    async def _send_deltas(
        self, connection: ConnectionInfo, deltas: List[StateDelta]
    ) -> None:
...
class OptimisticSync(ISyncStrategy):
    ...
    async def handle_conflict(
        ...
        resolved_state = await self.conflict_resolver(
            client_state, server_state, entity_type, entity_id
        )
...
class StateSynchronizer:
    ...
    def __init__(
        ...
        self.strategies = {
            ...
            SyncStrategy.OPTIMISTIC: OptimisticSync(
                connection_manager, event_propagator
            ),
        }
    ...
    async def sync_state(
        ...
        for conn_id in target_connections:
            ...
            await strategy.sync_state(
                connection, entity_type, entity_id, current_state, client_version
            )
    ...
    async def handle_sync_request(
        ...
        # Perform sync
        await self.sync_state(entity_type, entity_id, current_state, [connection_id])
    ...
    async def cleanup_connection(self, connection_id: str) -> None:
        ...
        keys_to_remove = [
            key for key in self._last_sync if key.startswith(f"{connection_id}:")
        ]
