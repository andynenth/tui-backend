...
class TargetedBroadcast(BroadcastStrategy):
    ...
    def __init__(self, connection_ids: Union[str, List[str]]):
        ...
        self.connection_ids = (
            [connection_ids] if isinstance(connection_ids, str)
            else connection_ids
        )
...
class RoomBroadcast(BroadcastStrategy):
    ...
    async def get_recipients(
        ...
        return [
            ...
            if conn.connection_id not in self.exclude
        ]
...
class EventPropagator:
    ...
    async def propagate(
        self,
        event: Event,
        strategy: Optional[BroadcastStrategy] = None
    ) -> int:
        """
        Propagate an event.
    ...
    async def _get_recipients_by_scope(self, event: Event) -> List[ConnectionInfo]:
        ...
        if event.scope == BroadcastScope.CONNECTION and event.target_id:
            conn = await self.connection_manager.registry.get_connection(event.target_id)
    ...
    async def _filter_by_subscriptions(
        self,
        event: Event,
        recipients: List[ConnectionInfo]
    ) -> List[ConnectionInfo]:
    ...
    async def _process_queue(self, priority: EventPriority) -> None:
        ...
        while self._running:
            try:
                # Collect batch
                deadline = asyncio.get_event_loop().time() + (self.batch_delay_ms / 1000)
