...
class SubscriptionManager:
    ...
    async def get_subscribers(self, event: ChangeEvent) -> List[Subscription]:
        """
...
class WebSocketRoomRepository(OptimizedRoomRepository, RealtimeRepositoryMixin):
    ...
    def __init__(
        ...
        RealtimeRepositoryMixin.__init__(
            self, "room", connection_manager, subscription_manager
        )
    ...
    async def save(self, room: Room) -> None:
        ...
        await self.broadcast_change(
            room.id, ChangeType.CREATED if is_new else ChangeType.UPDATED, room
        )
    ...
    async def delete(self, room_id: str) -> None:
        ...
        if room:
            await self.broadcast_change(
                ...
                ChangeType.DELETED,
                metadata={
                    "deleted_room": room.to_dict() if hasattr(room, "to_dict") else {}
                },
            )
    ...
    def _serialize_entity(self, room: Room) -> Dict[str, Any]:
        ...
        return {
            ...
            "code": room.code,
            "state": (
                room.state.value if hasattr(room.state, "value") else str(room.state)
            ),
            "players": [
                {"id": p.id, "name": p.name, "is_bot": p.is_bot} for p in room.players
            ],
...
class WebSocketGameRepository(OptimizedGameRepository, RealtimeRepositoryMixin):
    ...
    def __init__(
        ...
        RealtimeRepositoryMixin.__init__(
            self, "game", connection_manager, subscription_manager
        )
    ...
    def _serialize_entity(self, game: Game) -> Dict[str, Any]:
        ...
        return {
            ...
            "players": [
                {"id": p.id, "score": p.total_score, "is_active": p.is_active}
...
class WebSocketPlayerStatsRepository(
    OptimizedPlayerStatsRepository, RealtimeRepositoryMixin
):
    ...
    def __init__(
        ...
        RealtimeRepositoryMixin.__init__(
            self, "player_stats", connection_manager, subscription_manager
        )
...
class WebSocketRepository(Generic[T]):
    ...
    async def save(self, entity: T) -> None:
        ...
        if self._broadcast_enabled:
            await self._broadcast_change(
                entity_id, ChangeType.CREATED if is_new else ChangeType.UPDATED, entity
            )
    ...
    async def delete(self, entity_id: str) -> None:
        ...
        # Broadcast
        if self._broadcast_enabled:
            await self._broadcast_change(entity_id, ChangeType.DELETED, entity)

    async def _broadcast_change(
        self, entity_id: str, change_type: ChangeType, entity: Optional[T] = None
    ) -> None:
...
class WebSocketUnitOfWork(InMemoryUnitOfWork):
    ...
    def __init__(
        ...
        self.rooms = WebSocketRoomRepository(connection_manager, subscription_manager)
        self.games = WebSocketGameRepository(connection_manager, subscription_manager)
        self.player_stats = WebSocketPlayerStatsRepository(
            connection_manager, subscription_manager
        )
