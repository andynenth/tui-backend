...
class WebSocketMiddleware(ABC):
    ...
    async def process_connect(
        self, websocket: WebSocket, connection_info: ConnectionInfo
    ...
    async def process_disconnect(
        self, connection_info: ConnectionInfo, code: int, reason: str
    ) -> None:
...
class WebSocketInfrastructureMiddleware:
    ...
    async def handle_message(
        ...
        for middleware in self.middlewares:
            try:
                message = await middleware.process_message(connection_info, message)
                if message is None:
                    return None
            except Exception as e:
    ...
    async def handle_send(
        ...
        for middleware in self.middlewares:
            try:
                message = await middleware.process_send(connection_info, message)
                if message is None:
                    return False
            except Exception as e:
        ...
        return await self.connection_manager.send_to_connection(
            connection_info.connection_id, message
        )
    ...
    async def handle_disconnect(
        ...
        for middleware in reversed(self.middlewares):
            try:
                await middleware.process_disconnect(connection_info, code, reason)
            except Exception as e:
                logger.error(f"Middleware error on disconnect: {e}")

        # Disconnect
        await self.connection_manager.disconnect(
            connection_info.connection_id, code, reason
        )
...
class RateLimitingMiddleware(WebSocketMiddleware):
    ...
    def __init__(self, rate_limiter: Optional[GameWebSocketRateLimiter] = None):
    ...
    async def process_connect(
        ...
        allowed = await self.rate_limiter.on_connect(
            connection_info.player_id or "anonymous", connection_info.connection_id
        )

        if not allowed:
            await websocket.close(
                code=1008, reason="Rate limit exceeded"  # Policy violation
            )
            return None
...
class ErrorHandlingMiddleware(WebSocketMiddleware):
    ...
    def __init__(self, max_errors: int = 10, error_window_seconds: int = 60):
        """
    ...
    async def process_connect(
        ...
        if connection_info.connection_id in self._circuit_broken:
            await websocket.close(
                code=1011, reason="Service temporarily unavailable"  # Server error
            )
            return None
        ...
        try:
            ...
            if self._should_break_circuit(connection_info.connection_id):
                ...
                await connection_info.websocket.close(
                    code=1011, reason="Too many errors"
                )
    ...
    def _record_error(self, connection_id: str) -> None:
        ...
        self._error_counts[connection_id] = [
            ts for ts in self._error_counts[connection_id] if ts.timestamp() > cutoff
        ]
...
class AuthenticationMiddleware(WebSocketMiddleware):
    ...
    def __init__(self, auth_validator: Optional[Callable] = None):
        """
