...
class InMemoryConnectionRegistry(IConnectionRegistry):
    ...
    async def add_connection(self, connection: ConnectionInfo) -> None:
        ...
        async with self._lock:
            ...
            if connection.room_id:
                self._room_connections[connection.room_id].add(
                    connection.connection_id
                )
    ...
    async def remove_connection(self, connection_id: str) -> None:
        ...
        async with self._lock:
            ...
            # Clean up indexes
            if connection.player_id:
                self._player_connections[connection.player_id].discard(
                    connection_id
                )
            ...
            if connection.room_id:
                self._room_connections[connection.room_id].discard(
                    connection_id
                )
...
class ConnectionManager:
    ...
    async def authenticate(
        ...
        if hasattr(self.registry, 'update_connection_player'):
            await self.registry.update_connection_player(
                connection_id,
                player_id
            )
    ...
    async def join_room(
        ...
        if hasattr(self.registry, 'update_connection_room'):
            await self.registry.update_connection_room(
                connection_id,
                room_id
            )
    ...
    async def _heartbeat_loop(self) -> None:
        ...
        while self._running:
            try:
                # Get all connections
                connections = list(self._connections.values()) if hasattr(self, '_connections') else []
