...
class PiecesPlayedEvent(DomainEvent):
    ...
    def __init__(self, player_name: str, pieces: List[Dict[str, Any]], pile_winner: Optional[str]):
        super().__init__({
    ...
    async def record_game_completed(
        ...
        event = Event(
            ...
            data={
                ...
                'duration_seconds': (datetime.utcnow() - game.created_at).total_seconds()
                    if hasattr(game, 'created_at') else None
            },
            metadata={'source': 'game_engine'},
            timestamp=datetime.utcnow(),
            sequence_number=0
        ...
        for event in created_events:
            if player_name in event.data.get('players', []):
                ...
                player_actions = [
                    ...
                    if e.type == EventType.PLAYER_ACTION and
                    e.data.get('player_name') == player_name
                ]
    ...
    async def get_game_statistics(self) -> Dict[str, Any]:
        ...
        for event in all_events:
            if event.type == EventType.GAME_CREATED:
                ...
                stats['phase_statistics'][phase] = stats['phase_statistics'].get(phase, 0) + 1
        ...
        stats['most_active_players'] = dict(
            sorted(stats['most_active_players'].items(),
            ...
            if action_type == 'weak_hand_request':
                ...
                if player not in weak_hand_stats['by_player']:
                    weak_hand_stats['by_player'][player] = {
                        'requests': 0, 'accepted': 0, 'declined': 0
                    }
