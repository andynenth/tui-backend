...
class CorrelationIdMiddleware(IMiddleware[TRequest, TResponse]):
    ...
    def __init__(
        self, header_name: str = "X-Correlation-ID", generate_if_missing: bool = True
    ):
    ...
    async def process(
        ...
        try:
            ...
            await self.cache.set(
                cache_key, response, ttl=int(self.cache_ttl.total_seconds())
            )
...
class CircuitBreakerMiddleware(IMiddleware[TRequest, TResponse]):
    ...
    async def process(
        ...
        return await self.circuit_breaker.call_async(next_handler, request, context)
...
class RetryMiddleware(IMiddleware[TRequest, TResponse]):
    ...
    async def process(
        ...
        async def handler_with_context(
            req: TRequest, ctx: MiddlewareContext
        ) -> TResponse:
            nonlocal attempt
            attempt += 1
        ...
        response = await self.retry.execute_async(
            handler_with_context, request, context
        )
...
class RateLimitMiddleware(IMiddleware[TRequest, TResponse]):
    ...
    def __init__(
        ...
        self.rate_limiter = TokenBucketRateLimiter(
            rate=requests_per_second, capacity=burst_size
        )
        self.key_extractor = key_extractor or self._default_key_extractor
...
class MetricsMiddleware(IMiddleware[TRequest, TResponse]):
    ...
    async def process(
        ...
        # Record request
        self.metrics.increment(
            f"{self.service_name}.requests", tags=[("method", method), ("path", path)]
        )
