...
class OptimizedPlayerStatsRepository(PlayerStatsRepository):
        ...
        async with self._lock:
            ...
            if 'highest_score' in stats:
                player_stats.highest_score = max(
                    player_stats.highest_score,
                    stats.get('highest_score', 0)
                )
            
            # Update calculated fields
            if player_stats.games_played > 0:
                player_stats.average_score = player_stats.total_score / player_stats.games_played
    ...
    async def get_player_rank(self, player_id: str, metric: str = 'wins') -> Optional[int]:
    ...
    def _is_leaderboard_cache_valid(self) -> bool:
        ...
        return (time.time() - self._leaderboard_last_update) < self._leaderboard_cache_ttl
    ...
    async def _rebuild_leaderboards(self):
        ...
        active_players = [
            (pid, stats) for pid, stats in self._stats.items()
        ...
        eligible_for_rate = [
            (pid, stats) for pid, stats in active_players
            if stats.games_played >= 10
        ]
        self._leaderboard_by_rate = sorted(
            eligible_for_rate,
            key=lambda x: x[1].calculate_win_rate(),
            reverse=True
        )
