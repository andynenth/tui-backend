    ...
    async def run_check(self) -> HealthCheckResult:
        ...
        try:
            ...
            result = await asyncio.wait_for(
                self.check_health(), timeout=self.config.timeout.total_seconds()
            )
...
class SystemResourceHealthCheck(HealthCheck):
    ...
    def __init__(self, config: Optional[HealthCheckConfig] = None):
        ...
        super().__init__("system_resources", ComponentType.SYSTEM_RESOURCE, config)
...
class HealthCheckManager:
    ...
    async def get_aggregate_health(self) -> HealthCheckResult:
        ...
        if status_counts[HealthStatus.UNHEALTHY] > 0:
            ...
            status = HealthStatus.DEGRADED
            message = (
                f"{status_counts[HealthStatus.UNKNOWN]} components in unknown state"
            )
        else:
            status = HealthStatus.HEALTHY
