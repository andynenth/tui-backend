...
class LoadMetrics:
    ...
    def get_load_score(self) -> float:
        ...
        normalized_response_time = min(100, self.response_time_p99 * 10)  # Assume 10s is max
...
class SheddingConfig:
    ...
    shedding_rates: Dict[LoadLevel, float] = field(default_factory=lambda: {
        LoadLevel.LOW: 0.0,
...
class AdaptiveLoadShedder(LoadShedder):
    ...
    def _apply_strategy(self, context: RequestContext, shedding_rate: float) -> bool:
        ...
        if self.config.strategy == SheddingStrategy.RANDOM:
            ...
            priority_factor = (self.config.priority_levels - context.priority + 1) / \
    ...
    def get_stats(self) -> Dict[str, Any]:
        """Get shedding statistics."""
        with self._lock:
            accept_rate = self._accepted_requests / self._total_requests \
                    ...
                    [
                        {
                            ...
                            'accept_rate': self._client_accepts.get(client_id, 0) / requests
                        }
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if not shedder.should_accept(context):
                raise Exception(f"Request shed due to high load (level={shedder._current_level.value})")
