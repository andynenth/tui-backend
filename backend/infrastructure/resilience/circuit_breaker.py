...
class CircuitBreaker(Generic[T]):
    ...
    async def _execute_call_async(self, func: Callable[..., Any], *args, **kwargs) -> T:
        """Execute asynchronous call with monitoring."""
        try:
            # Apply timeout if configured
            if self.config.timeout:
                result = await asyncio.wait_for(
                    func(*args, **kwargs), timeout=self.config.timeout.total_seconds()
                )
            else:
    ...
    def _transition_to(self, new_state: CircuitState) -> None:
        ...
        self._stats.state_changes.append((datetime.utcnow(), old_state, new_state))
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._state_lock:
            return {
                ...
                "state_changes": [
                    {"timestamp": ts.isoformat(), "from": old.value, "to": new.value}
