...
class TimeoutHandler:
    ...
    def execute(self, func: Callable[..., T], *args, **kwargs) -> T:
        ...
        if self.config.strategy == TimeoutStrategy.SIGNAL:
            ...
            raise ValueError(
                f"Unsupported strategy for sync execution: {self.config.strategy}"
            )
    ...
    async def execute_async(self, func: Callable[..., Any], *args, **kwargs) -> T:
        ...
        try:
            ...
            if self.config.raise_on_timeout:
                raise TimeoutError(self.config.timeout_seconds, func.__name__)
            return None

    def _execute_with_signal(self, func: Callable[..., T], *args, **kwargs) -> T:
    ...
    def _execute_with_thread(self, func: Callable[..., T], *args, **kwargs) -> T:
...
class AsyncTimeout:
    ...
    def __init__(
        self, timeout: Union[float, timedelta], operation: Optional[str] = None
    ):
...
def with_timeout(
    func: Callable[..., T], timeout: Union[float, timedelta], *args, **kwargs
) -> T:
    """
    Execute function with timeout.
...
async def with_timeout_async(
    func: Callable[..., Any], timeout: Union[float, timedelta], *args, **kwargs
) -> T:
    """
    Execute async function with timeout.
...
class TimeoutPool:
    ...
    def register(
        self, operation: str, timeout: Union[float, timedelta], **kwargs
    ) -> None:
        """Register timeout for operation."""
        if isinstance(timeout, (int, float)):
            timeout = timedelta(seconds=timeout)

        self._timeouts[operation] = TimeoutConfig(timeout=timeout, **kwargs)
    ...
    def execute(self, operation: str, func: Callable[..., T], *args, **kwargs) -> T:
...
def register_timeout(
    operation: str, timeout: Union[float, timedelta], **kwargs
) -> None:
    """Register timeout for operation in global pool."""
    _timeout_pool.register(operation, timeout, **kwargs)


def execute_with_timeout(operation: str, func: Callable[..., T], *args, **kwargs) -> T:
...
async def execute_with_timeout_async(
    operation: str, func: Callable[..., Any], *args, **kwargs
) -> T:
