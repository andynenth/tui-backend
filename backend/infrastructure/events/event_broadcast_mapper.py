...
class EventBroadcastMapper:
    ...
    def map_event(
        self, event: DomainEvent, context: Optional[Dict[str, Any]] = None
...
def map_room_update(
    event: DomainEvent, context: Optional[Dict[str, Any]]
) -> Dict[str, Any]:
...
def map_phase_change(
    event: "PhaseChanged", context: Optional[Dict[str, Any]]
) -> Dict[str, Any]:
...
def map_game_started(
    event: "GameStarted", context: Optional[Dict[str, Any]]
) -> Dict[str, Any]:
    """Map game started event."""
    return {"round_number": event.round_number, "players": event.player_names}


def map_turn_played(
    event: "PiecesPlayed", context: Optional[Dict[str, Any]]
) -> Dict[str, Any]:
...
def map_declaration(
    event: "DeclarationMade", context: Optional[Dict[str, Any]]
) -> Dict[str, Any]:
    """Map declaration event."""
    return {"player": event.player_name, "value": event.declared_count}


def map_score_update(
    event: "ScoresCalculated", context: Optional[Dict[str, Any]]
) -> Dict[str, Any]:
...
def map_error(
    event: "InvalidActionAttempted", context: Optional[Dict[str, Any]]
) -> Dict[str, Any]:
...
def map_pong(
    event: "ConnectionHeartbeat", context: Optional[Dict[str, Any]]
) -> Dict[str, Any]:
    """Map heartbeat to pong response."""
    data = {"timestamp": event.client_timestamp, "server_time": event.server_timestamp}
...
def map_room_created(
    event: "RoomCreated", context: Optional[Dict[str, Any]]
) -> Dict[str, Any]:
    """Map room created event to response."""
    return {"room_id": event.room_id, "host_name": event.host_name, "success": True}


def map_room_list(
    event: "RoomListUpdated", context: Optional[Dict[str, Any]]
) -> Dict[str, Any]:
    """Map room list update."""
    return {"rooms": event.rooms, "reason": event.reason}
...
def register_all_mappings():
    ...
    event_broadcast_mapper.register(
        ...
        "room",
        lambda e, c: {
            "old_host": e.old_host_name,
            "new_host": e.new_host_name,
            "reason": e.reason,
        },
    )
    ...
    event_broadcast_mapper.register(DeclarationMade, "declare", "room", map_declaration)
    event_broadcast_mapper.register(
        TurnCompleted,
        "turn_complete",
        "room",
    ...
    event_broadcast_mapper.register(ConnectionHeartbeat, "pong", "response", map_pong)
