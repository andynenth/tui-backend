...
class InMemoryEventBus(EventPublisher):
    ...
    def register_handler(self, handler: EventHandler) -> None:
        ...
        for event_type in handler.get_event_types():
            ...
            logger.debug(
                f"Registered handler {handler.__class__.__name__} for {event_type_name}"
            )
    ...
    def subscribe(
        self, event_type: Type[DomainEvent], handler_func: Callable, priority: int = 0
    ) -> None:
        """
        ...
        event_type_name = (
            event_type.__name__ if hasattr(event_type, "__name__") else str(event_type)
        )
    ...
    async def _handle_callable(
        self, handler_func: Callable, event: DomainEvent
    ) -> None:
        """
