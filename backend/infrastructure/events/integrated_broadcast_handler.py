...
class IntegratedBroadcastHandler:
    ...
    def __init__(self, room_manager: Optional['RoomManager'] = None):
        ...
        self._player_pieces_count: Dict[str, Dict[str, int]] = {}  # room_id -> player -> count
    ...
    def initialize(self):
        ...
        self._event_bus.subscribe(PlayerConnected, self._handle_connection_event, priority=50)
        self._event_bus.subscribe(PlayerDisconnected, self._handle_connection_event, priority=50)
        self._event_bus.subscribe(ConnectionHeartbeat, self._handle_connection_event, priority=50)
        self._event_bus.subscribe(RoomListUpdated, self._handle_lobby_event, priority=50)
    ...
    async def _handle_game_event(self, event: GameEvent):
        ...
        try:
            ...
            if not broadcast_info:
                logger.debug(f"No broadcast mapping for event type: {type(event).__name__}")
                return
            ...
            logger.error(f"Error handling game event {type(event).__name__}: {e}", exc_info=True)
    ...
    def _update_tracked_state(self, event: DomainEvent):
        ...
        if isinstance(event, PiecesPlayed):
            ...
            self._player_pieces_count[event.room_id][event.player_name] += len(event.pieces)
...
def get_broadcast_handler(room_manager: Optional['RoomManager'] = None) -> IntegratedBroadcastHandler:
