...
class WebSocketEventPublisher(EventPublisher):
    ...
    async def publish(self, event: DomainEvent) -> None:
        ...
        try:
            ...
            if not self._feature_flags.is_enabled(
                self._feature_flags.USE_DOMAIN_EVENTS, {"event_type": event.event_type}
            ):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "error_rate": (self._error_count / max(self._published_count, 1)),
        }
...
class CompositeEventPublisher(EventPublisher):
    ...
    async def publish(self, event: DomainEvent) -> None:
        ...
        """
        logger.info(
            f"[COMPOSITE_DEBUG] Publishing {event.__class__.__name__} to {len(self._publishers)} publishers"
        )
        ...
        tasks = [publisher.publish(event) for publisher in self._publishers]
    ...
    async def publish_batch(self, events: List[DomainEvent]) -> None:
        ...
        tasks = [publisher.publish_batch(events) for publisher in self._publishers]
...
class EventStorePublisher(EventPublisher):
    ...
    async def publish(self, event: DomainEvent) -> None:
        ...
        """
        if not self._feature_flags.is_enabled(self._feature_flags.USE_EVENT_SOURCING):
            return
