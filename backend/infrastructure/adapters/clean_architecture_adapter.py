...
class CleanArchitectureAdapter:
    ...
    async def handle_event(
        self, event_type: str, data: Dict[str, Any], context: Dict[str, Any]
    ...
    def _should_use_clean_architecture(
        ...
        if not self._feature_flags.is_enabled(
            self._feature_flags.USE_CLEAN_ARCHITECTURE, context
        ):
            return False
        ...
        if event_type in ["ping", "client_ready", "sync_state"]:
            return self._feature_flags.is_enabled(
                self._feature_flags.USE_CONNECTION_ADAPTERS, context
            )
        elif event_type in ["create_room", "join_room", "leave_room", "add_bot"]:
            return self._feature_flags.is_enabled(
                self._feature_flags.USE_ROOM_ADAPTERS, context
            )
        elif event_type in [
            "start_game",
            "declare",
            ...
            return self._feature_flags.is_enabled(
                self._feature_flags.USE_GAME_ADAPTERS, context
            )
    ...
    async def _handle_start_game(
        ...
        request = StartGameRequest(
            room_id=context["room_id"], requesting_player_id=context["player_id"]
        )
