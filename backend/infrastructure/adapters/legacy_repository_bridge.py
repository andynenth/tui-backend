...
class LegacyRepositoryBridge:
    ...
    async def sync_room_to_legacy(self, room_id: str) -> None:
        ...
        try:
            ...
            async with uow:
                ...
                if not clean_room:
                    logger.error(
                        f"[SYNC_DEBUG] Room {room_id} not found in clean architecture!"
                    )
                    return

                logger.info(
                    f"[SYNC_DEBUG] Found room {room_id} with {len(clean_room.slots)} slots"
                )
                ...
                if room_id not in shared_room_manager.rooms:
                    ...
                    logger.info(
                        f"[SYNC_DEBUG] Legacy manager now has rooms: {list(shared_room_manager.rooms.keys())}"
                    )
                else:
                    # Update existing room
                    shared_room_manager.rooms[room_id] = legacy_room
                    logger.info(
                        f"[SYNC_DEBUG] Updated existing room {room_id} in legacy manager"
                    )
    ...
    def _convert_to_legacy_room(self, clean_room: CleanRoom) -> Any:
        ...
        logger.info(
            f"[SYNC_DEBUG] Converting {len(clean_room.slots)} slots to legacy players"
        )
        for i, player in enumerate(clean_room.slots):
            if player is not None:
                logger.info(
                    f"[SYNC_DEBUG]   Slot {i}: {player.name} (bot={player.is_bot})"
                )
                legacy_player = LegacyPlayer(name=player.name, is_bot=player.is_bot)
        ...
        try:
            ...
            while len(legacy_players) < 4:
                ...
                bot_player = LegacyPlayer(
                    name=f"Bot {bot_pos + 1}", is_bot=True, position=bot_pos
                )
