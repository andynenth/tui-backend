...
class ReconnectionAdapter:
    ...
    def __init__(self, feature_flags: Optional["FeatureFlags"] = None):
        ...
        self._message_queue_service = MessageQueueService(
            uow=self._uow, event_publisher=event_publisher
        )

        # Create use cases
        self._disconnect_use_case = HandlePlayerDisconnectUseCase(
            uow=self._uow, event_publisher=event_publisher, metrics=metrics
        )
    ...
    async def handle_disconnect(
        self, room_id: str, player_name: str, activate_bot: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        try:
            request = HandlePlayerDisconnectRequest(
                room_id=room_id, player_name=player_name, activate_bot=activate_bot
            )
    ...
    async def handle_reconnect(
        ...
        try:
            request = HandlePlayerReconnectRequest(
                room_id=room_id, player_name=player_name, websocket_id=websocket_id
            )
    ...
    async def cleanup_disconnected_players(
        self, room_id: str, timeout_minutes: int = 10
    ) -> int:
        """
