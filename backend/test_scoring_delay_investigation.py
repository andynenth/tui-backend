...
async def test_scoring_delay_mechanism():
    ...
    print(f"🕐 After setup - display_delay_complete: {scoring_state.display_delay_complete}")
    ...
    for i in range(80):  # Check every 0.1 seconds for 8 seconds
        ...
        if state_machine.current_phase != GamePhase.SCORING:
            print(f"🔄 Phase changed to {state_machine.current_phase} at {elapsed:.2f}s")
            break
        
        # Check transition conditions
        if i % 10 == 0:  # Every second
            transition_phase = await scoring_state.check_transition_conditions()
            print(f"🕐 {elapsed:.1f}s - delay_complete: {scoring_state.display_delay_complete}, transition: {transition_phase}")
    ...
    print(f"   Delay completed at: {delay_completed_time:.2f}s" if delay_completed_time else "   Delay never completed!")
...
async def test_process_loop_frequency():
    ...
    # Analyze frequency
    if len(call_times) > 1:
        intervals = [call_times[i] - call_times[i-1] for i in range(1, min(len(call_times), 11))]
