...
async def test_health_monitor():
    ...
    try:
        from api.services.health_monitor import HealthMonitor, HealthStatus, HealthMetric
        ...
        print(f"  ✅ Health status: {health_status.status.value} with {len(health_status.metrics)} metrics")
...
async def test_recovery_manager():
    ...
    try:
        from api.services.recovery_manager import RecoveryManager, RecoveryAction, recovery_manager
        ...
        print(f"  ✅ Initialized with {len(manager.recovery_procedures)} recovery procedures")
        ...
        print(f"  ✅ Recovery status: {status['total_procedures']} procedures available")
        ...
        with patch('socket_manager._socket_manager') as mock_sm:
            ...
            success = await manager.trigger_recovery("stale_connections", 
                                                   context={"test": True})
...
async def test_health_endpoints():
    ...
    try:
        ...
        metrics.append(f"liap_health_status {0 if health_status.status.value == 'healthy' else 1}")
...
async def test_integration_scenarios():
    ...
    try:
        ...
        assert health_data.status in [HealthStatus.HEALTHY, HealthStatus.WARNING, 
                                    HealthStatus.CRITICAL, HealthStatus.UNKNOWN]
...
async def test_performance_monitoring():
    ...
    try:
        ...
        found_metrics = [metric for metric in resource_metrics 
                        if metric in health_status.metrics]
        ...
        try:
            ...
            print(f"  ✅ WebSocket stats available: {len(socket_stats.get('rooms', {}))} rooms")
