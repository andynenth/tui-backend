...
class SimpleAIGame:
    ...
    def run_game(self) -> Dict:
        ...
        logger.info(f"\n{'='*60}")
        logger.info(
            f"GAME OVER - Winner: {winner_names[0] if winner_names else 'None'} - {max(final_scores.values())} pts"
        )
    ...
    def _initialize_round(self):
        ...
        if self.game.round_number == 1:
            ...
            self.game.round_starter = (
                self.game.last_round_winner or self.game.players[0].name
            )
    ...
    def _run_declaration_phase(self):
        ...
        for position, player in enumerate(declaration_order):
            ...
            if position == 3:
                ...
                if total == 8:
                    logger.warning(
                        f"{player.name} cannot declare {declaration} (sum would be 8)"
                    )
    ...
    def _run_turn_phase(self):
        ...
        while not all(len(p.hand) == 0 for p in self.game.players):
            ...
            if turn_number == 1:
                ...
                current_starter = (
                    self.game.last_turn_winner.name
                    if self.game.last_turn_winner
                    else self.game.round_starter
                )
            ...
            for player in turn_order:
                ...
                try:
                    ...
                    if self.game.current_turn_plays and not (
                        player.name == current_starter
                    ):
                        ...
                        logger.debug(
                            f"Responder {player.name} must match play type: {required_play_type}"
                        )
                    ...
                    play_list = ai.choose_strategic_play_safe(
                        player.hand, context, verbose=self.verbose
                    )
                    ...
                    if selected:
                        logger.debug(
                            f"AI returned {len(selected)} pieces: {[p.kind for p in selected]}"
                        )
                        logger.debug(
                            f"Player hand contains: {[p.kind for p in player.hand]}"
                        )

                except ImportError:
                    # Fallback to basic AI - returns pieces directly
                    selected = ai.choose_best_play(
                        player.hand, required_count=required, verbose=self.verbose
                    )
                ...
                if self.ai_logger.should_log("decision"):
                    play_data = {
                        ...
                        "am_i_starter": (player.name == current_starter),
                        "last_turn_winner": self.game.last_turn_winner.name
                        if self.game.last_turn_winner
                        else None,
                ...
                for kind in selected_kinds:
                    ...
                    for i, hand_piece in enumerate(player.hand):
                        if (
                            hand_piece.kind == kind
                            and hand_piece not in pieces_to_remove
                        ):
                        ...
                        logger.error(
                            f"ERROR: {player.name} trying to play {kind} but it's not in hand!"
                        )
                        logger.error(f"Hand: {[p.kind for p in player.hand]}")
                        logger.error(
                            f"Already removing: {[p.kind for p in pieces_to_remove]}"
                        )
                        # Skip this piece
                        continue
                ...
                turn_play = TurnPlay(
                    player=player, pieces=selected, is_valid=play_valid
                )
                ...
                logger.info(
                    f"  {player.name} plays: {play_type} ({points} points) - {[p.name for p in selected]}"
                )
            ...
            self.ai_logger.log_turn_result(
                turn_number,
                {
                    ...
                    "winning_pieces": winner_data.get("winning_pieces")
                    if winner_data
                    else [],
                    "winning_type": winner_data.get("winning_type")
                    if winner_data
                    else None,
                    "winning_points": winner_data.get("winning_points")
                    if winner_data
                    else 0,
                    "pile_counts": dict(self.game.pile_counts),
                    "next_starter": self.game.last_turn_winner.name
                    if self.game.last_turn_winner
                    else current_starter,
                },
            )
    ...
    def _resolve_turn(self, turn_number: int):
        ...
        if winner_name:
            ...
            logger.info(
                f"  Turn {turn_number} winner: {winner_name} won {piles_won} piles (now has {winner.captured_piles} total)"
            )
    ...
    def _run_scoring_phase(self, round_number: int):
        ...
        round_summary = {"round_number": round_number, "player_stats": {}}
        ...
        for player in self.game.players:
            ...
            logger.info(
                f"{player.name}: Declared {player.declared}, Got {player.captured_piles}, "
        ...
        round_scores = [
            (p.name, round_summary["player_stats"][p.name]["score_gained"])
            for p in self.game.players
        ]
...
def main():
    ...
    parser.add_argument(
        "--games", type=int, default=1, help="Number of games to simulate (default: 1)"
    )
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Show detailed AI reasoning"
    )
