...
class TurnPhaseFlowTester:
    ...
    def assert_equal(self, actual, expected, message=""):
        ...
        if actual == expected:
            ...
            self.log_test(self.current_test, "FAIL", f"{message} - Expected: {expected}, Got: {actual}")
            return False
    ...
    def assert_in(self, item, collection, message=""):
        ...
        if item in collection:
            ...
            self.log_test(self.current_test, "FAIL", f"{message} - {item} not in {collection}")
            return False
    ...
    async def test_01_turn_phase_initialization(self, game, state_machine):
        ...
        self.log_test("01. Turn Phase Start", "RUNNING", "Setting first turn starter based on round starter")
        ...
        success &= self.assert_equal(
            game.turn_number, 
            1, 
            "Turn number should be 1 for first turn"
        )
        
        if success:
            self.log_test("01. Turn Phase Start", "PASS", "Turn phase initialized correctly with proper starter")
    ...
    async def test_02_starter_plays_pieces(self, game, state_machine):
        ...
        success = self.assert_equal(
            turn_state._get_current_player(), 
            "TestPlayer", 
            "Should be starter's turn"
        )
        ...
        success &= self.assert_true(
            ...
            "Starter's play should be accepted"
        )
        ...
        success &= self.assert_in(
            ...
            "Starter's play should be recorded"
        )
        ...
        if success:
            self.log_test("02. Starter Play", "PASS", "Starter played 2 pieces and set required count")
        
        return success

    async def test_03_others_must_match_count(self, game, state_machine):
        """Test: Others Play - Other players must match piece count"""
        self.log_test("03. Others Match Count", "RUNNING", "Testing other players must match piece count")
        ...
        await asyncio.sleep(0.2)
        success = self.assert_true(result1.get('success', False), "Bot1's matching play should be accepted")
        ...
        await asyncio.sleep(0.2)
        success &= self.assert_true(result2.get('success', False), "Bot2's matching play should be accepted")
        ...
        await asyncio.sleep(0.2)
        success &= self.assert_true(result3.get('success', False), "Bot3's matching play should be accepted")
        
        # Verify all players have played
        success &= self.assert_equal(
            len(turn_state.turn_plays), 
            4, 
            "All 4 players should have played"
        )
        
        # Verify turn is complete
        success &= self.assert_true(
            turn_state.turn_complete, 
            "Turn should be complete after all players played"
        )
        
        if success:
            self.log_test("03. Others Match Count", "PASS", "All players matched required piece count")
        
        return success

    async def test_04_determine_winner_and_piles(self, game, state_machine):
        """Test: Determine Winner and Winner Takes Piles"""
        self.log_test("04. Winner & Piles", "RUNNING", "Testing winner determination and pile distribution")
        
        turn_state = state_machine.current_state
        
        # Verify winner was determined
        success = self.assert_true(
            turn_state.winner is not None, 
        ...
        if success:
            self.log_test("04. Winner & Piles", "PASS", f"Winner {turn_state.winner} correctly awarded piles")
    ...
    async def test_05_next_turn_or_round_complete(self, game, state_machine):
        ...
        self.log_test("05. Next Turn/Round Complete", "RUNNING", "Testing turn continuation or round completion")
        ...
        if all_hands_empty:
            ...
            if success:
                self.log_test("05. Next Turn/Round Complete", "PASS", "Round complete - transitioning to scoring")
        else:
            ...
            success = self.assert_equal(
                game.turn_number, 
                2, 
                "Turn number should increment to 2"
            )
            ...
            success &= self.assert_equal(
                len(turn_state.turn_plays), 
                0, 
                "Turn plays should be reset for new turn"
            )
            ...
            success &= self.assert_false(
                turn_state.turn_complete, 
                "Turn complete flag should be reset"
            )
            
            if success:
                self.log_test("05. Next Turn/Round Complete", "PASS", f"Next turn started with {winner} as starter")
        
        return success

    async def test_06_complete_multiple_turns(self, game, state_machine):
        """Test: Complete multiple turns until round ends"""
        self.log_test("06. Multiple Turns", "RUNNING", "Testing complete turns until round ends")
        ...
        while turn_count < max_turns:
            ...
            for i, player in enumerate(game.players):
                ...
                if turn_state.required_piece_count is None:
                    ...
                    pieces_to_play = min(turn_state.required_piece_count, len(player.hand))
        ...
        success = self.assert_true(
            all_hands_empty, 
            f"All hands should be empty after {turn_count} turns"
        )
        ...
        if success:
            self.log_test("06. Multiple Turns", "PASS", f"Completed {turn_count} turns, all hands empty")
    ...
    async def run_complete_test(self):
        ...
        try:
            ...
            if all_passed:
                print("🎉 ALL TESTS PASSED! Turn phase flow matches TURN_PHASE_DIAGRAM.md")
            else:
