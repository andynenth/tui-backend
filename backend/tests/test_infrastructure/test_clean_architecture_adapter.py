...
def adapter(mock_legacy_handlers, mock_feature_flags):
    ...
    with patch('infrastructure.adapters.clean_architecture_adapter.get_feature_flags', return_value=mock_feature_flags):
        with patch('infrastructure.adapters.clean_architecture_adapter.get_metrics_collector'):
...
class TestCleanArchitectureAdapter:
    ...
    async def test_clean_architecture_when_enabled(self, adapter):
        ...
        with patch.object(adapter, '_handle_ping', return_value=mock_response.to_dict()):
            result = await adapter.handle_event(
                'ping',
                {'sequence': 1},
                {'player_id': 'player1'}
    ...
    async def test_feature_flag_granularity(self, adapter):
        ...
        should_use = adapter._should_use_clean_architecture('create_room', {'player_id': 'p1'})
    ...
    async def test_ping_handler_integration(self, adapter):
        ...
        with patch('infrastructure.adapters.clean_architecture_adapter.get_unit_of_work', return_value=mock_uow):
            with patch('infrastructure.adapters.clean_architecture_adapter.get_metrics_collector', return_value=mock_metrics):
                with patch('infrastructure.adapters.clean_architecture.adapter.HandlePingUseCase', return_value=mock_use_case):
                    result = await adapter._handle_ping(
                        {'sequence': 1},
                        {'player_id': 'player1', 'room_id': 'room1'}
                    )
    ...
    async def test_create_room_handler_integration(self, adapter):
        ...
        with patch('infrastructure.adapters.clean_architecture_adapter.get_unit_of_work', return_value=mock_uow):
            with patch('infrastructure.adapters.clean_architecture_adapter.get_event_publisher', return_value=mock_publisher):
                with patch('infrastructure.adapters.clean_architecture_adapter.CreateRoomUseCase', return_value=mock_use_case):
    ...
    def test_use_case_mapping(self, adapter):
        ...
        expected_events = [
            'ping', 'client_ready', 'sync_state',
            'create_room', 'join_room', 'leave_room', 'add_bot',
