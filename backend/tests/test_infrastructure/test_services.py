...
from infrastructure.services.websocket_notification_service import WebSocketNotificationService
...
class TestWebSocketNotificationService:
    ...
    async def test_notify_player(self):
        ...
        with patch('infrastructure.services.websocket_notification_service.broadcast') as mock_broadcast:
            mock_broadcast.return_value = asyncio.create_task(asyncio.sleep(0))
            
            await service.notify_player(
                'player1',
                'test_event',
                {'message': 'Hello'}
            )
    ...
    async def test_notify_players_multiple(self):
        ...
        with patch('infrastructure.services.websocket_notification_service.broadcast') as mock_broadcast:
            ...
            assert calls[0][0] == ('player1', 'announcement', {'text': 'Game starting soon'})
            assert calls[1][0] == ('player2', 'announcement', {'text': 'Game starting soon'})
            assert calls[2][0] == ('player3', 'announcement', {'text': 'Game starting soon'})
...
class TestSimpleBotService:
    ...
    def service(self, mock_bot_manager):
        ...
        with patch('infrastructure.services.simple_bot_service.shared_bot_manager', mock_bot_manager):
    ...
    async def test_get_bot_decision_declaration(self, service):
        ...
        decision = await service.get_bot_decision(
            'room1',
            'bot1',
            {},
            'declaration'
    ...
    async def test_trigger_bot_action(self, service, mock_bot_manager):
        ...
        await service.trigger_bot_action(
            'room1',
            'bot1',
            'declare'
        )
    ...
    async def test_replace_player_with_bot(self, service, mock_bot_manager):
        ...
        result = await service.replace_player_with_bot(
            'room1',
            'player1'
        )
