...
class TestCleanArchitectureIntegration:
    ...
    async def test_start_game_flow(self, uow, event_publisher):
        ...
        async with uow:
            room = Room(room_id=RoomId("room1"), room_code="TEST1", name="Game Room")
        ...
        start_request = StartGameRequest(room_id="room1", requesting_player_id="p1")
        ...
        published_events = event_publisher.get_published_events()
        game_started_events = [
            e for e in published_events if isinstance(e, GameStarted)
        ]
    ...
    async def test_unit_of_work_transaction_rollback(self, uow):
        ...
        async with uow:
            room = Room(
                room_id=RoomId("room1"), room_code="ROLL1", name="Rollback Test"
            )
            await uow.rooms.add(room)
            await uow.commit()
    ...
    async def test_repository_isolation(self, uow):
        ...
        async with uow:
            room = Room(room_id=RoomId("iso1"), room_code="ISO1", name="Isolation Test")
            await uow.rooms.add(room)
            await uow.commit()
    ...
    async def test_concurrent_room_operations(self, uow, event_publisher):
        ...
        async with uow:
            room = Room(
                room_id=RoomId("concurrent1"), room_code="CONC1", name="Concurrent Test"
            )
        ...
        # Join 1
        response1 = await join_use_case.execute(
            JoinRoomRequest(
                player_id="p1", player_name="Player 1", room_id="concurrent1"
            )
        )
        assert response1.success is True

        # Join 2
        response2 = await join_use_case.execute(
            JoinRoomRequest(
                player_id="p2", player_name="Player 2", room_id="concurrent1"
            )
        )
