...
from application.use_cases.game import StartGameUseCase, DeclareUseCase, PlayUseCase
...
class TestCompleteGameFlow:
    ...
    async def test_full_game_flow(self, setup):
        ...
        start_response = await start_use_case.execute(
            StartGameRequest(room_id=room_id, requesting_player_id="host123")
        )
    ...
    async def test_player_leaving_and_host_migration(self, setup):
        ...
        async with uow:
            room = Room(
                room_id=RoomId("migrate_test"), room_code="MIGR8", name="Migration Test"
            )
        ...
        leave_response = await leave_use_case.execute(
            LeaveRoomRequest(
                player_id="host", room_id="migrate_test", reason="Host disconnected"
            )
        )
    ...
    async def test_error_handling_in_flow(self, setup):
        ...
        async with uow:
            room = Room(
                room_id=RoomId("error_test"), room_code="ERR01", name="Error Test"
            )
        ...
        start_use_case = StartGameUseCase(uow, publisher)
        start_response = await start_use_case.execute(
            StartGameRequest(room_id="error_test", requesting_player_id="p1")
        )
