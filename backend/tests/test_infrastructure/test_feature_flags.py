...
class TestFeatureFlagManager:
    ...
    def test_initialization_from_config(self):
        ...
        config = {
            FeatureFlags.USE_CLEAN_ARCHITECTURE: {
                'enabled': True,
                'percentage': 50
            },
            'custom_flag': {
    ...
    def test_evaluation_context_creation(self):
        ...
        # Dict context
        ctx = manager._create_evaluation_context({'user_id': 'user1', 'room_id': 'room1'})
    ...
    def test_complex_evaluation_rules(self):
        ...
        with patch.object(manager, '_get_hash_for_context') as mock_hash:
            ...
            assert manager.is_enabled('complex_flag', {'user_id': 'regular_user'}) is True
            
            mock_hash.return_value = 70  # Above 60% threshold
            assert manager.is_enabled('complex_flag', {'user_id': 'regular_user'}) is False
    ...
    def test_environment_variable_override(self):
        ...
        with patch.dict('os.environ', {'FEATURE_FLAGS': json.dumps({
            'env_flag': {'enabled': True}
        })}):
            manager = FeatureFlagManager()
...
class TestFeatureFlags:
    ...
    def test_flag_naming_convention(self):
        ...
        flags = [
            attr for attr in dir(FeatureFlags)
