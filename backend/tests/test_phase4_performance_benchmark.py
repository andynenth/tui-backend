...
class TestPhase4PerformanceBenchmark:
    ...
    async def test_cache_hit_performance(self, benchmark_system):
        ...
        assert (
            stats["avg_ms"] < 100
        ), f"Average cache hit time {stats['avg_ms']:.2f}ms exceeds 100ms target"
        assert (
            stats["p95_ms"] < 100
        ), f"P95 cache hit time {stats['p95_ms']:.2f}ms exceeds 100ms target"
    ...
    async def test_game_state_update_performance(self, benchmark_system):
        ...
        await benchmark_system.store_game_event(
            ...
            {
                "players": [
                    {"player_name": f"Player{i}", "player_type": "ai"} for i in range(4)
                ],
        ...
        assert (
            stats["avg_ms"] < 100
        ), f"Average update time {stats['avg_ms']:.2f}ms exceeds 100ms target"
        assert (
            stats["p95_ms"] < 150
        ), f"P95 update time {stats['p95_ms']:.2f}ms exceeds 150ms target"
    ...
    async def test_concurrent_access_performance(self, benchmark_system):
        ...
        async def simulate_game_session(game_id: int):
            ...
            await benchmark_system.store_game_event(
                ...
                {
                    "players": [
                        {"player_name": f"P{i}", "player_type": "ai"} for i in range(4)
                    ]
                },
            )
            ...
            for phase in ["PREPARATION", "DECLARATION", "TURN", "SCORING"]:
                start = time.time()
                await benchmark_system.store_game_event(
                    room_id, "phase_change", {"new_phase": phase}
                )
                times.append(time.time() - start)
        ...
        assert (
            stats["avg_ms"] < 200
        ), f"Average concurrent time {stats['avg_ms']:.2f}ms exceeds 200ms target"
        assert (
            stats["p95_ms"] < 500
        ), f"P95 concurrent time {stats['p95_ms']:.2f}ms exceeds 500ms target"
    ...
    async def test_historical_write_performance(self, benchmark_system):
        ...
        assert (
            queue_stats["avg_ms"] < 10
        ), f"Average queue time {queue_stats['avg_ms']:.2f}ms exceeds 10ms target"
    ...
    async def test_cache_memory_efficiency(self, benchmark_system):
        ...
        for i in range(cache_size * 2):  # Double the capacity
            await benchmark_system.store_game_event(
                ...
                {
                    "players": [
                        {"player_name": f"P{j}", "player_type": "ai"} for j in range(4)
                    ],
                    "round_data": {
                        "round": 1,
                        "scores": {f"P{j}": 0 for j in range(4)},
                    },
        ...
        assert (
            cache_metrics["evictions"] >= cache_size
    ...
    async def test_recovery_performance(self, benchmark_system):
        ...
        if cache_times:
            ...
            assert (
                stats["avg_ms"] < 100
            ), f"Average recovery time {stats['avg_ms']:.2f}ms exceeds 100ms target"
    ...
    async def test_phase4_target_metrics(self, benchmark_system):
        ...
        print(
            f"  - Cache Size: {cache_metrics.get('size', 0)}/{cache_metrics.get('max_size', 0)}"
        )

        # Verify Phase 4 targets
        assert (
            final_stats["avg_ms"] < 100
        ), "Phase 4 target: <100ms average response time"
        assert final_stats["p95_ms"] < 200, "Phase 4 target: <200ms P95 response time"
        assert (
            cache_metrics.get("hit_rate", 0) > 0.7
        ), "Phase 4 target: >70% cache hit rate"
