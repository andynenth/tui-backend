...
class TestRealtimeSystemIntegration:
    ...
    def sample_game_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "phase_data": {"starter": "Alice", "hands_dealt": True},
    ...
    async def test_historical_event_queuing(self, realtime_system):
        ...
        await realtime_system.store_game_event(
            ...
            {"player_name": "Eve", "timestamp": "2024-01-01T12:05:00"},
        )
    ...
    async def test_game_recovery_with_cache(self, realtime_system, sample_game_state):
        ...
        # Recover game
        with patch(
            "backend.services.cached_game_recovery.CachedGameRecoveryService._reconstruct_game"
        ) as mock_reconstruct:
    ...
    async def test_cache_eviction(self, realtime_system):
        ...
        assert (
            state is None
            or realtime_system.cached_store._cache_reads
            < realtime_system.cached_store._db_reads
        )
    ...
    async def test_system_health_check(self, realtime_system, sample_game_state):
        ...
        # Populate some data
        await realtime_system.store_game_event(
            "room-1", "game_started", sample_game_state
        )
    ...
    async def test_graceful_shutdown(self, realtime_system):
        ...
        for i in range(3):
            await realtime_system.store_game_event(
                f"shutdown-test-{i}", "test_event", {"data": f"test_{i}"}
            )
