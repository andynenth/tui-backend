...
async def test_bot_redeal_sequential_timing():
    ...
    async def track_handle_action(action):
        if action.action_type in [ActionType.REDEAL_REQUEST, ActionType.REDEAL_RESPONSE]:
    ...
    assert len(decision_calls) == 3, f"Expected 3 bot decisions, got {len(decision_calls)}"
    ...
    assert all(c['accept'] for c in decision_calls), "All bots should accept redeal by default"
...
async def test_redeal_timing_matches_declaration():
    ...
    handler._bot_declare = AsyncMock(side_effect=lambda bot, pos: track_action(
    ...
    assert all(0.4 <= d <= 1.7 for d in all_delays), "All delays should be in 0.5-1.5s range"
