            ...
            with self.subTest(event=event_name):
                try:
                    ...
                    if should_exist and not has_event:
                        self.fail(
                            f"Expected event {event_name} not found in {module_name}"
                        )
                    elif not should_exist and has_event:
                        self.fail(
                            f"Unexpected event {event_name} found in {module_name} (should not exist)"
                        )
                    else:
                        self.assertTrue(
                            True, f"Event {event_name} existence check passed"
                        )
