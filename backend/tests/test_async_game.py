...
from engine.state_machine.async_game_adapter import AsyncGameAdapter, wrap_game_for_async
...
class TestAsyncGame:
    ...
    async def test_start_new_round_async(self, async_game):
        ...
        # Mock deal_pieces
        with patch.object(async_game, 'deal_pieces', new_callable=AsyncMock) as mock_deal:
...
class TestAsyncGameAdapter:
    ...
    async def test_adapter_with_async_game(self, async_game):
        ...
        # Mock async methods
        async_game.deal_pieces = AsyncMock(return_value={"success": True, "async": True})
