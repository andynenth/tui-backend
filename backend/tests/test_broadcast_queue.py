...
async def test_broadcast_with_queue():
    ...
    david_messages = await message_queue_manager.get_queued_messages(
        "test_room", "David"
    )
    print(f"3. David has {len(david_messages)} queued messages ✓")

    # Verify Alice and Charlie (connected) don't have queues
    alice_messages = await message_queue_manager.get_queued_messages(
        "test_room", "Alice"
    )
    charlie_messages = await message_queue_manager.get_queued_messages(
        "test_room", "Charlie"
    )
    print(
        f"4. Connected players have no queues: Alice={len(alice_messages)}, Charlie={len(charlie_messages)} ✓"
    )
