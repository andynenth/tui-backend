...
async def test_get_events_since_sequence(event_store):
    ...
    recent_events = await event_store.get_events_since("test_room", all_events[2].sequence)
...
async def test_replay_room_state(event_store):
    ...
    await event_store.store_event(room_id, "player_joined", {
        "player_name": "Alice",
        "player_data": {"is_bot": False}
    })
    await event_store.store_event(room_id, "player_joined", {
        "player_name": "Bob",
        "player_data": {"is_bot": False}
    })
    await event_store.store_event(room_id, "phase_change", {
        "old_phase": None,
        "new_phase": "declaration"
    })
    await event_store.store_event(room_id, "player_declared", {
        "player_name": "Alice",
        "value": 3
    })
    await event_store.store_event(room_id, "player_declared", {
...
async def test_phase_data_update_events(event_store):
    ...
    await event_store.store_event(room_id, "phase_data_update", {
        "phase": "turn",
        "updates": {
...
async def test_action_processed_events(event_store):
    ...
    await event_store.store_event(room_id, "action_processed", {
        "sequence_id": 1,
        "action_type": "DECLARE",
        "player_name": "Alice",
        "payload": {"value": 3}
...
async def test_turn_and_scoring_events(event_store):
    ...
    # Play some turns
    await event_store.store_event(room_id, "pieces_played", {
        "player_name": "Alice",
        "pieces": ["R5", "R6"]
    })
    
    await event_store.store_event(room_id, "turn_complete", {
        "winner": "Alice",
        "turn_number": 1
    })
    
    # Round scoring
    await event_store.store_event(room_id, "round_scoring", {
        "round_number": 1,
        "scores": {
            "Alice": 10,
            "Bob": -5,
...
async def test_game_complete_event(event_store):
    ...
    await event_store.store_event(room_id, "game_complete", {
        "final_scores": {
            "Alice": 55,
            "Bob": 45,
            "Charlie": 30,
...
async def test_export_room_history(event_store):
    ...
    await event_store.store_event(room_id, "player_declared", {"player_name": "Alice", "value": 3})
