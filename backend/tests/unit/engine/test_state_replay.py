...
async def test_full_game_replay(game_state_machine, temp_event_store):
    ...
    await state_machine._store_phase_change_event(GamePhase.PREPARATION, GamePhase.DECLARATION)
    
    # Simulate player declarations
    for player in game.players[:2]:  # Only human players declare
        await state_machine.store_game_event("player_declared", {
    ...
    # Simulate a turn
    await state_machine.store_game_event("pieces_played", {
        "player_name": "Alice",
        "pieces": ["R5", "R6"]
    }, "Alice")
    
    await state_machine.store_game_event("turn_complete", {
        "winner": "Alice",
        "turn_number": 1
    })
...
async def test_action_processing_persistence(game_state_machine, temp_event_store):
    ...
    action_events = await temp_event_store.get_events_by_type(room_id, "action_processed")
...
async def test_replay_with_complex_state(game_state_machine, temp_event_store):
    ...
    for round_num in range(1, 4):
        ...
        # Store round start
        await state_machine.store_game_event("round_started", {
        ...
        for turn in range(1, 3):
            await state_machine.store_game_event("turn_complete", {
                "turn_number": turn,
                "winner": game.players[turn % 4].name
            })
        
        # Store round scoring
        scores = {p.name: 10 * round_num for p in game.players}
        await state_machine.store_game_event("round_scoring", {
            "round_number": round_num,
            "scores": scores
        })
...
async def test_event_sequence_integrity(game_state_machine, temp_event_store):
    ...
    for i in range(event_count):
        await state_machine.store_game_event(f"test_event_{i % 5}", {
            "index": i,
            "data": f"test_data_{i}"
        })
...
async def test_concurrent_state_updates(game_state_machine, temp_event_store):
    ...
    update_events = await temp_event_store.get_events_by_type(room_id, "phase_data_update")
...
async def test_game_completion_replay(game_state_machine, temp_event_store):
    ...
    for round_num in range(1, 6):
        ...
        # Simulate round
        await state_machine.store_game_event("round_complete", {
            "round_number": round_num,
            "scores": {
                "Alice": 15,
                "Bob": 10,
    ...
    # End game
    await state_machine.store_game_event("game_complete", {
        "final_scores": {
            "Alice": 75,
            "Bob": 50,
            "Charlie": 25,
