...
from tests.utilities.helpers.test_helpers import create_strong_hand, create_test_hand_realistic
...
async def test_realistic_integration():
    ...
    try:
        ...
        for player in room.game.players:
            ...
            print(
                f"✅ {player.name} assigned strong hand (max: {max(p.point for p in player.hand)} points)"
            )
        ...
        if room.game_state_machine.current_phase == GamePhase.TURN:
            ...
            starter_name = (
                getattr(room.game, "current_player", None) or room.game.players[0].name
            )
        ...
        for player in room.game.players:
            ...
            print(
                f"   {player.name}: Max piece = {max_points} points ({'STRONG' if max_points > 9 else 'WEAK'})"
            )
