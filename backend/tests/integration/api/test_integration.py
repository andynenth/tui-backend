...
async def test_complete_integration():
    ...
    async def test_broadcast(event_type, event_data):
        ...
        print(
            f"📡 Broadcast: {event_type} -> {list(event_data.keys()) if isinstance(event_data, dict) else event_data}"
        )
    ...
    try:
        ...
        if room.game_state_machine.current_phase == GamePhase.DECLARATION:
            ...
            print(
                f"✅ Phase after declarations: {room.game_state_machine.current_phase}"
            )
