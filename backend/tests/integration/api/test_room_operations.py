        ...
        try:
            async with websockets.connect(f"{BASE_URL}/ws/lobby") as websocket:
                ...
                await websocket.send(
                    json.dumps(
                        {"event": "client_ready", "data": {"player_name": "TestPlayer"}}
                    )
                )
                ...
                if data.get("event") == "room_list_update":
                    self.log_result(
                        "Lobby Connection", True, "Connected and received room list"
                    )
                else:
                    self.log_result(
                        "Lobby Connection",
                        False,
                        f"Unexpected response: {data.get('event')}",
                    )
    ...
    async def test_room_creation(self):
        ...
        try:
            async with websockets.connect(f"{BASE_URL}/ws/lobby") as websocket:
                ...
                await websocket.send(
                    json.dumps(
                        {"event": "client_ready", "data": {"player_name": "Host"}}
                    )
                )
                ...
                await websocket.send(
                    json.dumps(
                        {"event": "create_room", "data": {"player_name": "Host"}}
                    )
                )
                ...
                if data.get("event") == "room_created":
                    ...
                    self.log_result(
                        "Room Creation", True, f"Created room {self.room_id}"
                    )
                    ...
                    if room:
                        self.log_result(
                            "Room Verification", True, "Room exists in AsyncRoomManager"
                        )
                    else:
                        self.log_result(
                            "Room Verification", False, "Room not found in manager"
                        )
                else:
                    self.log_result(
                        "Room Creation", False, f"Unexpected response: {data}"
                    )
    ...
    async def test_room_join(self):
        ...
        try:
            async with websockets.connect(f"{BASE_URL}/ws/lobby") as websocket:
                ...
                await websocket.send(
                    json.dumps(
                        {"event": "client_ready", "data": {"player_name": "Player2"}}
                    )
                )
                ...
                if data.get("event") == "room_joined":
                    self.log_result(
                        "Room Join", True, f"Player2 joined room {self.room_id}"
                    )
                else:
    ...
    async def test_bot_operations(self):
        ...
        try:
            async with websockets.connect(f"{BASE_URL}/ws/{self.room_id}") as websocket:
                ...
                # Add bot to slot 3
                await websocket.send(
                    json.dumps({"event": "add_bot", "data": {"slot_id": "3"}})
                )
                ...
                await websocket.send(
                    json.dumps({"event": "remove_player", "data": {"slot_id": "3"}})
                )
                ...
                if room:
                    ...
                    self.log_result(
                        "Bot Operations", False, "Room not found after bot operations"
                    )
        ...
        try:
            ...
            async with websockets.connect(f"{BASE_URL}/ws/{self.room_id}") as websocket:
                ...
                for slot in [3, 4]:  # Assuming Host is slot 1, Player2 is slot 2
                    await websocket.send(
                        json.dumps({"event": "add_bot", "data": {"slot_id": str(slot)}})
                    )
                ...
                if data.get("event") == "game_started":
                    ...
                    self.log_result(
                        "Game Start", False, f"Unexpected response: {data.get('event')}"
                    )
    ...
    async def test_cleanup(self):
        ...
        try:
            ...
            async with websockets.connect(f"{BASE_URL}/ws/{self.room_id}") as websocket:
                ...
                # Leave room
                await websocket.send(json.dumps({"event": "leave_room", "data": {}}))
            ...
            if room is None:
                ...
                self.log_result(
                    "Room Cleanup", False, "Room still exists after host left"
                )
