...
async def test_websocket_room_operations():
    ...
    async with websockets.connect("ws://localhost:5050/ws/lobby") as websocket:
        ...
        await websocket.send(json.dumps({"event": "get_rooms", "data": {}}))
        ...
        await websocket.send(
            json.dumps({"event": "create_room", "data": {"player_name": "TestPlayer1"}})
        )
    ...
    async with websockets.connect("ws://localhost:5050/ws/lobby") as websocket2:
        ...
        await websocket2.send(
            json.dumps(
                {
                    "event": "join_room",
                    "data": {"room_id": room_id, "player_name": "TestPlayer2"},
                }
            )
        )
    ...
    async with websockets.connect(f"ws://localhost:5050/ws/{room_id}") as room_ws:
        ...
        # Add a bot
        await room_ws.send(json.dumps({"event": "add_bot", "data": {"slot_id": 3}}))
        ...
        # Start the game
        await room_ws.send(json.dumps({"event": "start_game", "data": {}}))
