...
async def test_websocket_connection():
    ...
    try:
        ...
        async with websockets.connect(uri) as websocket:
            ...
            client_ready = {"event": "client_ready", "data": {"room_id": "2C2507"}}
            ...
            try:
                while True:
                    ...
                    if data.get("event") == "phase_change":
                        ...
                        print(
                            f"   Phase: {data.get('data', {}).get('phase', 'unknown')}"
                        )
                        print(
                            f"   Simultaneous mode: {phase_data.get('simultaneous_mode', False)}"
                        )
                        print(f"   Weak players: {phase_data.get('weak_players', [])}")
                        print(
                            f"   Weak players awaiting: {phase_data.get('weak_players_awaiting', [])}"
                        )
                        print(
                            f"   Decisions received: {phase_data.get('decisions_received', 0)}"
                        )
                        print(
                            f"   Decisions needed: {phase_data.get('decisions_needed', 0)}"
                        )
