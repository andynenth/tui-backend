...
class TestWebSocketIntegration:
    ...
    @pytest.mark.asyncio
    async def test_connection_disconnect_flow(
        self, websocket_components, mock_websocket
    ):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    ...
                    mock_cm.get_connection_by_websocket_id.return_value = (
                        mock_connection
                    )
                    ...
                    returned_id = await server.handle_disconnect(
                        mock_websocket, room_id, mock_websocket._ws_id
                    )
    ...
    async def test_disconnect_handler_pregame(self, websocket_components):
        ...
        with patch(
            "application.websocket.disconnect_handler.get_unit_of_work"
        ) as mock_get_uow:
            with patch(
                "application.websocket.disconnect_handler.broadcast"
            ) as mock_broadcast:
                mock_uow = AsyncMock()
                mock_get_uow.return_value = mock_uow
    ...
    async def test_disconnect_handler_ingame(self, websocket_components):
        ...
        with patch(
            ...
            with patch(
                ...
                with patch(
                    "application.websocket.disconnect_handler.get_bot_service"
                ) as mock_get_bot:
                    mock_uow = AsyncMock()
                    mock_get_uow.return_value = mock_uow
                    ...
                    call_args = mock_broadcast.call_args_list
                    assert any(
                        args[0][1] == "player_replaced_by_bot" for args in call_args
                    )
