...
class RateLimitTester:
        ...
        try:
            ...
            async with websockets.connect(f"{self.base_url}/ws/lobby") as ws:
                ...
                if data.get("event") == "room_created":
                    ...
                    # Connect to room
                    async with websockets.connect(f"{self.base_url}/ws/{room_id}") as room_ws:
                        print(f"Connected to room {room_id}")
                        
                        # Simulate normal game events
                        events = [
                            {"event": "player_ready", "data": {"player_name": "TestPlayer1"}},
                        ...
                        for event in events:
                            ...
                            print(f"Event: {event['event']}, Response: {json.loads(response)['event']}")
        ...
        try:
            async with websockets.connect(f"{self.base_url}/ws/test_room") as ws:
                ...
                for i in range(20):
                    ...
                    if data.get("event") == "error" and "rate_limit" in str(data.get("data", {})):
                        ...
                        print(f"Ping after rate limit: {json.loads(ping_response)['event']}")
    ...
    async def test_concurrent_connections(self):
        ...
        try:
            ...
            connections = []
            for i in range(5):
                ws = await websockets.connect(f"{self.base_url}/ws/concurrent_test_room")
            ...
            for i, ws in enumerate(connections):
                async def send_events(ws_conn, conn_id):
                    for j in range(3):
                        event = {
                            "event": "play",
                            "data": {"player_name": f"Player{conn_id}", "pieces": [1, 2]}
                        }
    ...
    async def test_reconnection_after_rate_limit(self):
        ...
        try:
            ...
            try:
                async with websockets.connect(f"{self.base_url}/ws/reconnect_test") as ws2:
    ...
    async def run_all_tests(self):
        ...
        for result in self.results:
            ...
            print(f"{status_color}[{result['status']}]{reset_color} {result['test']}: {result['message']}")
