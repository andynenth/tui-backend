...
class TestLayerBoundaries:
    ...
    def test_infrastructure_no_application_imports(self, project_root):
        ...
        for file_path in infrastructure_files:
            ...
            app_imports = [
                ...
                if imp.startswith("application.") and 
                not imp.startswith("application.interfaces")
            ]
    ...
    def test_application_no_api_imports(self, project_root):
        ...
        application_files = get_python_files_in_layer(
    ...
    def test_domain_no_external_imports(self, project_root):
        ...
        domain_files = get_python_files_in_layer(
            project_root / "domain"
        )
        
        # Also check engine directory (contains domain logic)
        domain_files.extend(get_python_files_in_layer(
            project_root / "engine"
        ))
        
        violations = []
        allowed_stdlib = {
            "abc", "dataclasses", "enum", "typing", "datetime",
            "uuid", "random", "math", "collections", "itertools",
            "functools", "copy", "json", "re", "logging", "os", "sys"
        }
        ...
        for file_path in domain_files:
            ...
            for imp in imports:
                ...
                if (not imp.startswith('.') and 
                    root_module not in allowed_stdlib and
                    not imp.startswith('__')):
        ...
        for v in violations:
            filtered_imports = [
                imp for imp in v['imports']
                if not any(
                    imp.startswith(prefix) for prefix in [
...
class TestInterfaceUsage:
    ...
    def test_infrastructure_implements_interfaces(self, project_root):
        ...
        impl_file = project_root / "infrastructure" / "websocket" / "contract_implementations.py"
    ...
    def test_application_implements_interfaces(self, project_root):
        ...
        impl_file = project_root / "application" / "websocket" / "contract_implementations.py"
...
class TestPackageStructure:
    ...
    def test_interfaces_directories(self, project_root):
        ...
        for location in interface_locations:
            ...
            assert interface_dir.exists(), f"Interface directory '{location}' should exist"
                    ...
                    if line.strip().startswith('def ') and not line.strip().startswith('def __'):
                        ...
                        if i + 1 < len(lines):
                            ...
                            if next_line and not (
                                ...
                                if any("@abstractmethod" in lines[j] for j in range(max(0, i-5), i)):
                                    continue
