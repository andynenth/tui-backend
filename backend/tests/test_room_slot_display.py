...
async def test_room_creation_and_slots():
    ...
    try:
        ...
        async with websockets.connect(LOBBY_URL) as lobby_ws:
            ...
            await lobby_ws.send(json.dumps({
                "event": "client_ready",
                "data": {
                    "version": "1.0.0",
            ...
            if data.get("event") == "room_created":
                ...
                for player in room_info.get('players', []):
                    print(f"   - Slot {player.get('seat_position')}: {player.get('name')} ({'Bot' if player.get('is_bot') else 'Human'})")
            else:
                ...
                if data.get("event") == "client_ready_ack":
                    ...
                    if room_state:
                        ...
                        for player in room_state.get('players', []):
                            ...
                            print(f"     - Slot {player.get('seat_position')}: {player.get('name')}{bot_tag}{host_tag}")
                        ...
                        if len(players) == 4:
                            ...
                            if players[0].get('name') == player_name and not players[0].get('is_bot'):
