...
class GameFlowTester:
    ...
    async def simulate_message(
        self, action: str, data: Dict[str, Any], room_id: Optional[str] = None
    ):
        ...
        if response and response.get("event") == "room_created":
            ...
            self.room_states[room_id] = {
                "room_id": room_id,
                "players": [
                    {"name": data.get("player_name"), "slot": 0, "is_host": True}
                ],
                "started": False,
            }
...
class TestCompleteGameFlows:
    ...
    async def test_full_game_flow_with_4_players(self):
        ...
        # Step 1: Create room
        response = await tester.simulate_message(
        ...
        await tester.simulate_message(
            "join_room", {"room_id": room_id, "player_name": "Bob"}
        )
        await tester.simulate_message(
            "join_room", {"room_id": room_id, "player_name": "Charlie"}
        )
        await tester.simulate_message(
            "join_room", {"room_id": room_id, "player_name": "David"}
        )
    ...
    async def test_weak_hand_redeal_flow(self):
        ...
        for player in ["Bob", "Charlie", "David"]:
            await tester.simulate_message(
                "join_room", {"room_id": room_id, "player_name": player}
            )
    ...
    async def test_room_closure_scenarios(self):
        ...
        response = await tester.simulate_message(
            "create_room", {"player_name": "Charlie"}
        )
...
class TestErrorScenarios:
    ...
    async def test_invalid_actions_in_wrong_phase(self):
        ...
        error_response = await tester.simulate_message(
            "declare", {"player_name": "Alice", "value": 3}
        )
        ...
        error_response = await tester.simulate_message(
            "play", {"player_name": "Alice", "indices": [0, 1]}
        )
        assert (
            error_response.get("event") == "error"
            or error_response.get("event") == "play_rejected"
        )
    ...
    async def test_concurrent_action_handling(self):
        ...
        tasks = [
            tester.simulate_message(
                "join_room", {"room_id": room_id, "player_name": f"Player{i}"}
            )
        ...
        successful_joins = sum(
            1
            for r in responses
            if isinstance(r, dict) and r.get("event") == "room_joined"
        )
...
class TestPerformanceScenarios:
    ...
    async def test_response_timing_requirements(self):
        ...
        flow_record = {
            ...
            "messages_sent": tester.messages_sent,
            "events_received": tester.events_received,
            "timing_measurements": {"ping_response_ms": duration},
        }
