...
class IntegrationTestHarness:
    ...
    async def run_game_flow_with_validation(
        self, flow_name: str, flow_steps: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        for i, step in enumerate(flow_steps):
            ...
            if "action" in step:
                ...
                if not contract_validation["valid"]:
                    ...
                    print(
                        f"  ❌ Contract validation failed: {contract_validation['error']}"
                    )
                else:
    ...
    async def execute_flow_step(
        self, step: Dict[str, Any], game_state: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def validate_against_contract(
        ...
        return {"valid": True, "action": action, "contract": contract.name}

    def update_game_state(
        self, game_state: Dict[str, Any], step_result: Dict[str, Any]
    ):
        ...
        if response.get("event") == "room_created":
            ...
            game_state["players"] = [
                {"name": response["data"]["host_name"], "is_host": True}
            ]

        elif response.get("event") == "room_joined":
            game_state["players"].append(
                {"name": response["data"]["player_name"], "is_host": False}
            )
...
class TestIntegratedGameFlows:
    ...
    async def test_integrated_4_player_game(self):
        ...
        flow_steps = [
            ...
            {"description": "Alice starts game", "action": "start_game", "data": {}},
            {
        ...
        result = await harness.run_game_flow_with_validation(
            "4_player_game_integrated", flow_steps
        )
        ...
        failed_validations = [
            v for v in result["contract_validations"] if not v["valid"]
        ]
        assert (
            len(failed_validations) == 0
        ), f"Contract validations failed: {failed_validations}"
    ...
    async def test_error_handling_integration(self):
        ...
        for step in error_flow_steps:
            ...
            if "expected_error" in step:
                assert (
                    result.get("response", {}).get("event") == "error"
    ...
    async def test_performance_integrated(self):
        ...
        ), f"Average ping response time {avg_response_time}ms exceeds 100ms"
...
def generate_integration_test_report():
    ...
    summary = {
        ...
        "total_contract_validations": sum(
            len(r.get("contract_validations", [])) for r in results
        ),
