...
class TestFullGameFlow:
    ...
    def test_compact_format(self):
        ...
        response = client.get(
            f"/api/rooms/{room_id}/play-history?format=compact&include_hands=false"
        )
        assert response.status_code == 200
...
class TestPerformance:
    ...
    def test_compact_format_performance(self):
        ...
        if data_compact["rounds"]:
            ...
            assert (
                compact_hands == {}
            ), f"Compact format should have empty hands_dealt but got {compact_hands}"

            # Full format should have hands (we set up players with hands)
            assert (
                len(full_hands) > 0
            ), "Full format should include hands when players have cards"
        ...
        if full_hands and not compact_hands:
            assert (
                size_compact < size_full
            ), f"Compact response ({size_compact}) should be smaller than full ({size_full}) when hands are excluded"
