...
def test_no_duplicate_colors():
    ...
    for i, name in enumerate(player_names):
        ...
        if slot < 4:
            ...
            # Check no duplicate
            assert player.avatar_color not in assigned_colors, f"Duplicate color {player.avatar_color} assigned to {name}"
            assigned_colors.append(player.avatar_color)
    
    print(f"\n✅ All assigned colors: {assigned_colors}")
    print(f"✅ Unique colors: {set(assigned_colors)}")
    assert len(assigned_colors) == len(set(assigned_colors)), "Duplicate colors detected!"
...
def test_color_reassignment_on_rejoin():
    ...
    assert len(current_colors) == len(set(current_colors)), "Duplicate colors after rejoin!"
...
def test_bots_have_no_color():
    ...
    for i in range(4):
        ...
        if player:
            if player.is_bot:
                assert player.avatar_color is None, f"Bot {player.name} should not have color"
                print(f"✅ Bot {player.name} has no color (None)")
            else:
                assert player.avatar_color is not None, f"Human {player.name} should have color"
