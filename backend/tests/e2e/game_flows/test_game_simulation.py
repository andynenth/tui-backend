...
async def simulate_game_with_replay():
    ...
    try:
        ...
        logger.info(
            f"Current phase: {current_phase.value if current_phase else 'Unknown'}"
        )
        ...
        for i, event in enumerate(all_events[:10]):  # Show first 10 events
            logger.info(
                f"  {i+1}. [{event.sequence}] {event.event_type} - {event.player_id or 'System'}"
            )
        ...
        for event in history["timeline"][-5:]:
            logger.info(
                f"  Seq {event['sequence']}: {event['type']} by {event['player'] or 'System'}"
            )
