...
class TestAsyncPerformance:
    ...
    async def test_concurrent_bot_decisions(self):
        ...
        bot_hands = {}
        for i in range(4):
            hand = [MagicMock(spec=Piece, point=j + 5, color="RED") for j in range(8)]
    ...
    @pytest.mark.asyncio
    async def test_async_vs_sync_room_performance(
        self, async_room_manager, compat_room_manager
    ):
    ...
    async def test_concurrent_game_operations(self):
        ...
        for i in range(4):
            players = [
                MagicMock(spec=Player, name=f"P{j}", is_bot=False) for j in range(4)
            ]
