...
class TestRoundStartPhaseTransitions:
    ...
    def game(self):
        ...
        game = Game()
        game.players = [
            Player(f"Player{i}", is_bot=(i > 1)) for i in range(1, 5)
        ]
        game.round_number = 1
        return game
    ...
    async def test_preparation_to_round_start_no_weak_hands(self, state_machine):
        ...
        assert next_phase == GamePhase.ROUND_START, "Should transition to ROUND_START, not DECLARATION"
    ...
    async def test_bot_manager_notifications(self, state_machine):
        ...
        with patch('engine.state_machine.game_state_machine.BotManager') as mock_bot_manager:
    ...
    async def test_multiple_redeals_maintain_correct_flow(self, state_machine):
        ...
        with patch.object(prep_state, '_deal_cards', new_callable=AsyncMock) as mock_deal:
