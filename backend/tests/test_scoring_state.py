...
class TestScoringState:
    ...
    @pytest.mark.asyncio
    async def test_calculate_round_scores_no_multiplier(self, scoring_state, mock_players):
    ...
    @pytest.mark.asyncio
    async def test_calculate_round_scores_with_multiplier(self, scoring_state, mock_players):
        ...
        scoring_state.state_machine.game.players = mock_players[:2]  # Only use first 2 players
    ...
    async def test_cleanup_phase_game_over(self, scoring_state):
        ...
        assert scoring_state.state_machine.game.round_scores == {"Alice": {"final_score": 10}}
    ...
    async def test_validate_action_disconnect_reconnect(self, scoring_state):
        ...
        disconnect_action = GameAction("Alice", ActionType.PLAYER_DISCONNECT, {"player_name": "Alice"})
        reconnect_action = GameAction("Alice", ActionType.PLAYER_RECONNECT, {"player_name": "Alice"})
    ...
    async def test_handle_player_reconnect(self, scoring_state, mock_players):
        ...
        action = GameAction("Charlie", ActionType.PLAYER_RECONNECT, {"player_name": "Charlie"})
