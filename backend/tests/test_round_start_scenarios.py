...
class TestRoundStartScenarios:
    ...
    @pytest.mark.asyncio
    async def test_scenario_1_round1_red_general(self, create_game_with_hands, mock_broadcast):
        """
    ...
    @pytest.mark.asyncio
    async def test_scenario_2_round2_previous_winner(self, create_game_with_hands, mock_broadcast):
        """
    ...
    @pytest.mark.asyncio
    async def test_scenario_3_redeal_acceptance(self, create_game_with_hands, mock_broadcast):
        """
    ...
    @pytest.mark.asyncio
    async def test_scenario_4_timer_functionality(self, create_game_with_hands, mock_broadcast):
        """
        ...
        for elapsed, should_transition in [(0, False), (2, False), (4.9, False), (5.1, True)]:
            # Mock the time
            round_start_state.start_time = time.time() - elapsed
            next_phase = await round_start_state.check_transition_conditions()
            
            if should_transition:
                assert next_phase == GamePhase.DECLARATION, f"Should transition after {elapsed} seconds"
            else:
                assert next_phase is None, f"Should not transition after {elapsed} seconds"
    ...
    async def test_scenario_6_backend_state_sync(self, create_game_with_hands):
        ...
        for data in received_data[1:]:
            assert data["phase_data"]["current_starter"] == first_data["phase_data"]["current_starter"]
            assert data["phase_data"]["starter_reason"] == first_data["phase_data"]["starter_reason"]
            assert data["round"] == first_data["round"]

    @pytest.mark.asyncio
    async def test_edge_case_no_general_red_round1(self, create_game_with_hands, mock_broadcast):
        """
    ...
    @pytest.mark.asyncio
    async def test_multiple_broadcasts_during_round_start(self, create_game_with_hands, mock_broadcast):
        """
