...
class TestRoomCreationVisibility:
    ...
    async def test_newly_created_room_appears_in_lobby(self, mock_dependencies):
        ...
        with patch(
            "application.use_cases.lobby.get_room_list.PropertyMapper"
        ) as mock_mapper:
    ...
    async def test_room_creation_event_broadcast(self, mock_dependencies):
        ...
        event = RoomCreated(room_id="EVENT001", host_name="EventHost", total_slots=4)
...
class TestPlayerCountUpdates:
    ...
    @pytest.mark.asyncio
    async def test_player_count_increases_on_join(
        self, mock_dependencies, sample_rooms
    ):
    ...
    @pytest.mark.asyncio
    async def test_player_count_decreases_on_leave(
        self, mock_dependencies, sample_rooms
    ):
...
class TestRoomStateChanges:
    ...
    async def test_full_room_filtering(self, mock_dependencies, sample_rooms):
        ...
        get_room_list_use_case = GetRoomListUseCase(mock_uow, mock_metrics)
        request = GetRoomListRequest(player_id="observer", include_full=False)
        response = await get_room_list_use_case.execute(request)
...
class TestEdgeCases:
    ...
    async def test_pagination_with_many_rooms(self, mock_dependencies):
        ...
        # Create many rooms
        many_rooms = [
            Room(room_id=f"ROOM{i:03d}", host_name=f"Host{i}") for i in range(25)
        ]
        ...
        # First page
        request = GetRoomListRequest(player_id="observer", page=1, page_size=10)
        response = await get_room_list_use_case.execute(request)
