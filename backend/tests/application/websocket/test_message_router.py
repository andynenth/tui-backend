...
class TestMessageRouter:
    ...
    async def test_route_message_missing_event(self, message_router, mock_websocket):
        ...
        response = await message_router.route_message(mock_websocket, message, room_id)
    ...
    @pytest.mark.asyncio
    async def test_route_message_unsupported_event(
        self, message_router, mock_websocket
    ):
    ...
    async def test_route_message_supported_event(self, message_router, mock_websocket):
        ...
        with patch.object(message_router, "_route_to_adapter") as mock_route:
            ...
            response = await message_router.route_message(
                mock_websocket, message, room_id
            )

            assert response is not None
            assert response["event"] == "pong"
            mock_route.assert_called_once_with(mock_websocket, message, room_id)
    ...
    async def test_route_to_adapter_lobby(self, message_router, mock_websocket):
        ...
        with patch.object(
            message_router.adapter_wrapper, "try_handle_with_adapter"
        ) as mock_adapter:
            ...
            mock_adapter.assert_called_once_with(
                mock_websocket, message, room_id, None  # No room state for lobby
            )
    ...
    async def test_route_to_adapter_with_room(self, message_router, mock_websocket):
        ...
        with patch.object(message_router, "_get_room_state") as mock_get_state:
            with patch.object(
                ...
                mock_adapter.assert_called_once_with(
                    mock_websocket, message, room_id, mock_room_state
                )
    ...
    async def test_get_room_state(self, message_router, mock_websocket):
        ...
        with patch(
            "application.websocket.message_router.get_unit_of_work"
        ) as mock_get_uow:
            mock_uow = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_check_message_queue_no_connection(
        self, message_router, mock_websocket
    ):
    ...
    @pytest.mark.asyncio
    async def test_check_message_queue_with_messages(
        self, message_router, mock_websocket
    ):
        ...
        with patch(
            "application.websocket.message_router.connection_manager"
        ) as mock_cm:
            with patch(
                "application.websocket.message_router.message_queue_manager"
            ) as mock_queue:
    ...
    @pytest.mark.asyncio
    async def test_handle_room_validation_not_exists(
        self, message_router, mock_websocket
    ):
