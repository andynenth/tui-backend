...
class TestLobbyDispatcherEvents:
    ...
    async def test_get_rooms_returns_room_list_update_event(
        ...
        response = await dispatcher.dispatch("get_rooms", {}, mock_context)
    ...
    async def test_request_room_list_alias(self, dispatcher, mock_context):
        ...
        # Mock the use case
        mock_response = GetRoomListResponse(
            rooms=[], player_current_room=None, page=1, page_size=20, total_items=0
        )
        ...
        response1 = await dispatcher.dispatch("get_rooms", {}, mock_context)

        response2 = await dispatcher.dispatch("request_room_list", {}, mock_context)
    ...
    async def test_room_list_with_filters(self, dispatcher, mock_context):
        ...
        # Mock response
        mock_response = GetRoomListResponse(
            rooms=[], player_current_room=None, page=2, page_size=10, total_items=25
        )
    ...
    async def test_room_creation_event_format(self, dispatcher, mock_context):
        ...
        response = await dispatcher.dispatch(
            "create_room", {"player_name": "TestPlayer"}, mock_context
        )
    ...
    @pytest.mark.asyncio
    async def test_error_handling_for_invalid_filters(self, dispatcher, mock_context):
        ...
        response = await dispatcher.dispatch("get_rooms", {"page": -1}, mock_context)

        assert response["event"] == "error"
        assert "Failed to handle get_rooms" in response["data"]["message"]

    @pytest.mark.asyncio
    async def test_sorting_parameters_passed_correctly(self, dispatcher, mock_context):
        ...
        await dispatcher.dispatch(
            "get_rooms", {"sort_by": "player_count", "sort_order": "desc"}, mock_context
        )
