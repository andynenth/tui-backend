...
def mock_dependencies():
    ...
    with patch(
        "application.websocket.use_case_dispatcher.get_unit_of_work"
    ) as mock_uow, patch(
        "application.websocket.use_case_dispatcher.get_event_publisher"
    ) as mock_publisher, patch(
        "application.websocket.use_case_dispatcher.get_metrics_collector"
    ) as mock_metrics, patch(
        "application.websocket.use_case_dispatcher.get_bot_service"
    ) as mock_bot, patch(
        "application.websocket.use_case_dispatcher.get_game_service"
    ) as mock_game, patch(
        "application.websocket.use_case_dispatcher.get_game_state_store"
    ) as mock_store:
...
class TestUseCaseDispatcher:
    ...
    async def test_dispatch_unknown_event(self, dispatcher, dispatch_context):
        ...
        result = await dispatcher.dispatch("unknown_event", {}, dispatch_context)
    ...
    async def test_dispatch_error_handling(self, dispatcher, dispatch_context):
        ...
        dispatcher.event_handlers["ping"] = AsyncMock(
            side_effect=Exception("Test error")
        )

        result = await dispatcher.dispatch("ping", {}, dispatch_context)
    ...
    async def test_handle_ping(self, dispatcher, dispatch_context):
        ...
        result = await dispatcher.dispatch(
            "ping", {"timestamp": time.time(), "sequence": 42}, dispatch_context
        )
    ...
    async def test_handle_client_ready(self, dispatcher, dispatch_context):
        ...
        result = await dispatcher.dispatch(
            "client_ready", {"version": "1.0.0"}, dispatch_context
        )
    ...
    async def test_handle_create_room(self, dispatcher, dispatch_context):
        ...
        mock_response = CreateRoomResponse(
            success=True, room_info=room_info, join_code="ABC123"
        )
        dispatcher.create_room_use_case.execute = AsyncMock(return_value=mock_response)

        # Mock legacy visibility
        with patch(
            "application.websocket.use_case_dispatcher.ensure_room_visible_to_legacy",
    ...
    @pytest.mark.asyncio
    async def test_handle_create_room_validation_error(
        self, dispatcher, dispatch_context
    ):
        """Test create_room with missing player name"""
        result = await dispatcher.dispatch(
            "create_room", {}, dispatch_context  # No player_name
        )
    ...
    async def test_handle_join_room(self, dispatcher, dispatch_context):
        ...
        mock_response = JoinRoomResponse(
            success=True, room_info=room_info, player_id="player_456"
        )
        dispatcher.join_room_use_case.execute = AsyncMock(return_value=mock_response)

        result = await dispatcher.dispatch(
            "join_room",
            {"player_name": "New Player", "room_id": "ROOM123"},
            dispatch_context,
        )
    ...
    async def test_handle_join_room_failure(self, dispatcher, dispatch_context):
        ...
        mock_response = JoinRoomResponse(success=False, error_message="Room is full")
        dispatcher.join_room_use_case.execute = AsyncMock(return_value=mock_response)
    ...
    async def test_handle_request_room_list(self, dispatcher, dispatch_context):
        ...
        mock_response = GetRoomListResponse(rooms=rooms, total_count=2)
        dispatcher.get_room_list_use_case.execute = AsyncMock(
            return_value=mock_response
        )

        result = await dispatcher.dispatch(
            "request_room_list", {"include_in_progress": True}, dispatch_context
        )
    ...
    async def test_handle_play_pieces_alias(self, dispatcher, dispatch_context):
        ...
        await dispatcher.dispatch(
            "play_pieces", {"pieces": [1, 2, 3]}, dispatch_context
        )
    ...
    async def test_handle_redeal_decision(self, dispatcher, dispatch_context):
        ...
        dispatcher._handle_accept_redeal = AsyncMock(
            return_value={"event": "redeal_vote_cast"}
        )

        await dispatcher.dispatch(
            "redeal_decision", {"decision": "accept"}, dispatch_context
        )

        dispatcher._handle_accept_redeal.assert_called_once()

        # Test decline decision
        dispatcher._handle_decline_redeal = AsyncMock(
            return_value={"event": "redeal_vote_cast"}
        )

        await dispatcher.dispatch(
            "redeal_decision", {"decision": "decline"}, dispatch_context
        )
        ...
        result = await dispatcher.dispatch(
            "redeal_decision", {"decision": "invalid"}, dispatch_context
        )
    ...
    def test_format_room_info(self, dispatcher):
        ...
        assert (
            formatted["players"][0]["name"] == "Test Player"
