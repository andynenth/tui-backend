...
async def test_handle_ping_use_case():
    ...
    request = HandlePingRequest(
        player_id="player1", room_id="room1", sequence_number=42
    )
    ...
    metrics.increment.assert_called_once_with(
        "ping.received", tags={"room_id": "room1"}
    )
...
async def test_create_room_use_case():
    ...
    request = CreateRoomRequest(
        host_player_id="player1", host_player_name="Player One", room_name="Test Room"
    )
...
async def test_use_case_error_handling():
    ...
    request = CreateRoomRequest(host_player_id="player1", host_player_name="Player One")
