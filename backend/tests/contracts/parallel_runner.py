...
class TestSuite:
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "failed_tests": [
                {"name": r.test_name, "error": r.error, "comparison": r.comparison}
...
class ParallelContractRunner:
    ...
    async def run_single_test(
        ...
        try:
            ...
            current_behavior = await self._capture_behavior(
                self.current_handler, test_message, room_state, "current"
            )
            ...
            if self.new_handler:
                new_behavior = await self._capture_behavior(
                    self.new_handler, test_message, room_state, "new"
                )
    ...
    async def run_contract_suite(
        self,
        test_cases: List[
            Tuple[WebSocketContract, Dict[str, Any], Optional[Dict[str, Any]]]
        ],
    ) -> TestSuite:
        """
    ...
    async def _capture_behavior(
        ...
        try:
            ...
            response = {"event": "error", "data": {"message": str(e)}}
    ...
    def save_test_results(
        self, suite: TestSuite, output_path: str = "test_results.json"
    ):
    ...
    def generate_compatibility_report(self, suite: TestSuite) -> str:
            ...
            for failed in summary["failed_tests"]:
                ...
                if failed.get("comparison") and failed["comparison"].get("differences"):
                    ...
                    for diff in failed["comparison"]["differences"]:
                        report.append(
                            f"   - {diff['type']}: {diff.get('details', {}).get('error', 'Unknown')}"
                        )
            report.append("")
        ...
        if summary["success_rate"] == "100.0%":
            ...
            report.append(
                "   The refactored system has behavioral differences that need addressing."
            )
