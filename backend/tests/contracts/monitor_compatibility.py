...
class ContractTestMonitor:
    ...
    def get_golden_master_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "coverage": (
                (len(masters) / expected_count * 100) if expected_count > 0 else 0
            ),
    ...
    def generate_dashboard(self) -> Layout:
        ...
        layout["body"].split_row(
            Layout(name="status", ratio=1), Layout(name="details", ratio=2)
        )
        ...
        layout["body"]["details"].split_column(
            Layout(name="coverage"), Layout(name="problems")
        )
...
🧪 Last Test Run: {self._get_last_test_time()}
    ...
    def _create_coverage_panel(self) -> Panel:
        ...
        coverage_color = (
            "green"
            if golden_status["coverage"] >= 100
            else "yellow" if golden_status["coverage"] >= 80 else "red"
        )
        table.add_row(
            "Golden Master Coverage",
        ...
        table.add_row("Contract Definitions", "21/21", "[green]✓[/green]")
        ...
        if "summary" in latest:
            test_coverage = (
                ...
                if latest["summary"]["total"] > 0
                else 0
            )
            coverage_color = (
                "green"
                if test_coverage >= 100
                else "yellow" if test_coverage >= 80 else "red"
            )
            table.add_row(
                "Test Pass Rate",
                f"{test_coverage:.1f}%",
    ...
    def _create_problems_panel(self) -> Panel:
        ...
        if not problems:
            ...
            for i, problem in enumerate(problems, 1):
                test_name = (
                    problem["test"].split("::")[-1]
                    if "::" in problem["test"]
                    else problem["test"]
                )
                lines.append(f"{i}. {test_name}")
                lines.append(
                    f"   Status: [{problem['outcome']}]{problem['outcome']}[/{problem['outcome']}]"
                )

            content = "\n".join(lines)

        return Panel(
            content, title="Problem Areas", border_style="red" if problems else "green"
        )
...
def generate_compatibility_report():
    ...
    console.print(Panel.fit("[bold]Frontend Compatibility Report[/bold]", style="cyan"))
    ...
    status_table.add_row(
        "Golden Masters", f"{golden_status['found']}/{golden_status['expected']}"
    )
    status_table.add_row(
        "Overall Status",
        (
    ...
    if compatibility < 100 or golden_status["status"] != "ok":
        ...
        if golden_status["missing"] > 0:
            console.print(
                f"  • Capture {golden_status['missing']} missing golden masters"
            )
            console.print(
                "    Run: [cyan]python tests/contracts/capture_golden_masters.py[/cyan]"
            )
    ...
    report_data = {
        ...
        "problems": monitor.get_problem_areas(),
        "ready_for_refactoring": compatibility >= 100
        and golden_status["status"] == "ok",
    }
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--report", action="store_true", help="Generate compatibility report"
    )
    parser.add_argument(
