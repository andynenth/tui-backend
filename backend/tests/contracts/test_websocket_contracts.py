...
from tests.contracts.parallel_runner import ParallelContractRunner, TestSuite
...
class TestWebSocketContracts:
    ...
    async def test_create_room_contract(self, parallel_runner):
        ...
        test_cases = [
            ...
            (
                ...
                {
                    "action": "create_room",
                    "data": {"player_name": "VeryLongPlayerName123"},
                },
                None,
            ),
            ...
            (
                CREATE_ROOM_CONTRACT,
                {"action": "create_room", "data": {"player_name": "Player One"}},
                None,
            ),
        ]
        ...
        ), f"Contract tests failed: {suite.get_summary()}"
    ...
    async def test_join_room_contract(self, parallel_runner):
        ...
        test_cases = [
            ...
            (
                ...
                {
                    "action": "join_room",
                    "data": {"room_id": "FULL01", "player_name": "Charlie"},
                },
                {
                    "room_id": "FULL01",
                    "players": [
    ...
    async def test_game_action_contracts(self, parallel_runner):
        ...
        test_cases = [
            ...
            (
                DECLARE_CONTRACT,
                {"action": "declare", "data": {"player_name": "Alice", "value": 3}},
                declaration_state,
            ),
            # Valid play
            (
    ...
    async def test_capture_golden_masters(self, golden_master_capture, current_handler):
        ...
        test_messages = [
            {"action": "create_room", "data": {"player_name": "Alice"}},
            {"action": "ping", "data": {"timestamp": 1234567890}},
            {"action": "declare", "data": {"player_name": "Bob", "value": 2}},
        ]
        ...
        for message in test_messages:
            ...
            loaded = golden_master_capture.load_golden_master(
                record.message_name, record.test_id
            )
    ...
    async def test_timing_comparison(self, parallel_runner):
        ...
        if results[0].new_behavior:  # Only if new system exists
            avg_current_time = (
                sum(r.current_behavior.timing["duration_ms"] for r in results)
                / iterations
            )
            avg_new_time = (
                sum(r.new_behavior.timing["duration_ms"] for r in results) / iterations
            )
            ...
            ), f"New system is {performance_ratio:.2f}x slower"
...
async def mock_current_handler(websocket, message, room_state, broadcast):
    ...
    if action == "create_room":
        ...
        await broadcast(
            "lobby",
            "room_list_update",
            {"rooms": [], "timestamp": 1234567890.123, "reason": "new_room_created"},
        )
