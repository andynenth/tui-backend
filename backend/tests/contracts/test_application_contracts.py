...
class TestMessageHandlerContract:
    ...
    async def test_handle_message(self, handler):
        ...
        handler.dispatcher.dispatch = AsyncMock(return_value={
            "event": "response",
            "data": {"result": "success"}
        })
...
class TestMessageRouterContract:
    ...
    def router(self):
        ...
        with patch('application.websocket.contract_implementations.UseCaseMessageHandler'):
    ...
    async def test_route(self, router):
        ...
        mock_handler.handle_message = AsyncMock(return_value=WebSocketResponse(
            event="response",
            data={"result": "success"}
        ))
...
class TestEventPublisherContract:
    ...
    async def test_publish_event_to_room(self, publisher):
        ...
        with patch('application.websocket.contract_implementations.broadcast') as mock_broadcast:
    ...
    async def test_publish_to_room(self, publisher):
        ...
        with patch('application.websocket.contract_implementations.broadcast') as mock_broadcast:
            ...
            await publisher.publish_to_room(
                "room1",
                "test_event",
                {"data": "value"}
            )
    ...
    async def test_publish_to_player(self, publisher):
        ...
        await publisher.publish_to_player(
            "player1",
            "test_event",
            {"data": "value"}
        )
...
class TestMessageValidatorContract:
    ...
    def test_validate_message_legacy(self, validator):
        ...
        with patch.object(validator, 'should_bypass_validation', return_value=False):
            with patch('application.websocket.contract_implementations.validate_websocket_message') as mock_validate:
    ...
    def test_validate_event(self, validator):
        ...
        with patch('application.websocket.contract_implementations.websocket_config') as mock_config:
            ...
            # Test legacy event
            with patch('api.validation.websocket_validators.WebSocketMessageValidator') as mock_validator:
