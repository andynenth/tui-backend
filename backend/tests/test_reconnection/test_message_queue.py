...
async def test_queue_message_overflow():
    ...
    # Fill the queue
    for i in range(5):
        queue.add_message(QueuedMessage(
...
async def test_queue_critical_message_replaces_non_critical():
    ...
    async with uow:
        ...
        assert not any(msg.event_type == "non_critical_0" for msg in updated_queue.messages)
...
async def test_message_queue_service_delivery():
    ...
    delivery_calls = [call for call in event_publisher.publish_batch.call_args_list
