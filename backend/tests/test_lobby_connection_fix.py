    ...
    try:
        async with websockets.connect(WS_URL) as websocket:
            ...
            await websocket.send(
                json.dumps({"event": "client_ready", "data": {"version": "1.0.0"}})
            )
            ...
            if data.get("event") == "room_list":
                print(
                    f"✅ Room list received with {len(data.get('data', {}).get('rooms', []))} rooms"
                )
            else:
                print(f"❌ Unexpected response: {data}")
            ...
            await websocket.send(
                json.dumps({"event": "ping", "data": {"sequence_number": sequence}})
            )
