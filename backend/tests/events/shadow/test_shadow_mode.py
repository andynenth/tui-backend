...
class TestShadowMode:
    ...
    async def test_shadow_mode_runs_both_adapters(self, handler, websocket, config):
        ...
        async def mock_legacy(ws, msg):
            ...
            direct_called = True
            return {"event": "pong", "data": {"server_time": 1234567890}}

        # Patch the event adapter to track calls
        with patch(
            "api.adapters.connection_adapters_event.PingAdapterEvent.handle"
        ) as mock_event:

            async def mock_event_handle(ws, msg, room_state=None):
                nonlocal event_called
                event_called = True
                return {"event": "pong", "data": {"server_time": 1234567891}}

            mock_event.side_effect = mock_event_handle

            # Configure handler
            with patch(
                "api.adapters.adapter_event_config.adapter_event_config", config
            ):
                message = {"action": "ping", "data": {"timestamp": 123}}
                result = await handler.handle_message(websocket, message, mock_legacy)
    ...
    async def test_shadow_mode_matching_results(self, handler, websocket, config):
        ...
        response = {
            "event": "room_created",
            "data": {"room_id": "room123", "host_name": "Alice", "success": True},
        }
        ...
        with patch(
            "api.adapters.room_adapters_event.CreateRoomAdapterEvent.handle"
        ) as mock_event:
    ...
    async def test_shadow_mode_error_handling(self, handler, websocket, config):
        ...
        with patch(
            "api.adapters.room_adapters_event.JoinRoomAdapterEvent.handle"
        ) as mock_event:
            ...
            with patch(
                ...
                with pytest.raises(Exception) as exc_info:
                    await handler.handle_message(websocket, message, mock_legacy_error)
    ...
    async def test_shadow_mode_performance_tracking(self, handler, websocket, config):
        ...
        with patch(
            "api.adapters.game_adapters_event.PlayAdapterEvent.handle"
        ) as mock_event:
            ...
            with patch(
                ...
                await handler.handle_message(websocket, message, mock_legacy_slow)
...
class TestShadowModeIntegration:
    ...
    async def test_real_adapter_shadow_comparison(self):
        ...
        with patch("api.adapters.adapter_event_config.adapter_event_config", config):
            ...
            result = await handler.handle_message(websocket, message, legacy_handler)
