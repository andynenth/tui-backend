...
from infrastructure.events.in_memory_event_bus import get_event_bus, reset_event_bus
...
class EventSystemValidator:
    ...
    async def validate_event_bus(self):
        ...
        event = RoomCreated(
            room_id="test_room", host_name="Test Host", metadata=metadata
        )
        ...
        print(
            f"   ✓ Event publishing and subscription working"
            if success
            else "   ✗ Event bus failed"
        )
    ...
    async def validate_event_mappings(self):
        ...
        test_cases = [
            {
                "event": RoomCreated(
                    room_id="room123", host_name="Alice", metadata=EventMetadata()
                ),
        ...
        for test in test_cases:
            ...
            if result:
                passed = (
                    result["event_name"] == test["expected_event"]
                    and result["target_type"] == test["expected_target"]
                )
                status = "✓" if passed else "✗"
                print(
                    f"   {status} {type(test['event']).__name__} → {test['expected_event']}"
                )
    ...
    async def validate_adapter_events(self):
        ...
        try:
            ...
            message = {"action": "create_room", "data": {"player_name": "Test Player"}}
            ...
            success = (
                len(published) == 1
                and isinstance(published[0], RoomCreated)
                and response["event"] == "room_created"
            )
            ...
            print(
                f"   ✓ Adapters publish events correctly"
                if success
                else "   ✗ Adapter events failed"
            )
    ...
    async def validate_state_machine_events(self):
        ...
        try:
            ...
            print(
                f"   ✓ State machine publishes events"
                if success
                else "   ✗ State events failed"
            )
    ...
    async def validate_broadcast_integration(self):
        ...
        async def mock_broadcast(room_id, event_name, data):
            broadcasts.append({"room_id": room_id, "event": event_name, "data": data})
        ...
        with patch(
            "backend.infrastructure.events.integrated_broadcast_handler.broadcast",
            mock_broadcast,
        ):
            bus = get_event_bus()
        ...
        # Validate
        success = len(broadcasts) > 0 and any(
            b["event"] == "game_started" for b in broadcasts
        )
        ...
        print(
            f"   ✓ Events trigger broadcasts"
            if success
            else "   ✗ Broadcast integration failed"
        )
