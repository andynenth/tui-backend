...
from api.adapters.connection_adapters_event import (
    PingAdapterEvent,
    ClientReadyAdapterEvent,
    AckAdapterEvent,
    SyncRequestAdapterEvent,
)
...
from api.adapters.room_adapters import (
    ...
    _handle_join_room,
    _handle_leave_room,
    _handle_get_room_state,
    _handle_add_bot,
    _handle_remove_player,
)
from api.adapters.room_adapters_event import (
    CreateRoomAdapterEvent,
    JoinRoomAdapterEvent,
    LeaveRoomAdapterEvent,
    GetRoomStateAdapterEvent,
    AddBotAdapterEvent,
    RemovePlayerAdapterEvent,
)
...
class TestConnectionAdapterContracts:
    ...
    async def test_ping_adapter_contract(self, websocket):
        ...
        assert (
            direct_response["data"]["timestamp"] == event_response["data"]["timestamp"]
        )
    ...
    async def test_client_ready_adapter_contract(self, websocket):
        ...
        room_state = {
            "room_id": "room123",
            "players": [{"name": "Alice", "slot": "P1"}, {"name": "Bob", "slot": "P2"}],
            "host_name": "Alice",
        }
    ...
    async def test_ack_adapter_contract(self, websocket):
        ...
        message = {"action": "ack", "data": {"sequence": 42, "client_id": "client123"}}
    ...
    async def test_sync_request_adapter_contract(self, websocket):
        ...
        message = {"action": "sync_request", "data": {"client_id": "client123"}}
        room_state = {
            "room_id": "room123",
            "game_state": {"phase": "TURN", "current_player": "Alice"},
        }
        ...
        assert direct_response["event"] == event_response["event"]
        assert (
            direct_response["data"]["room_state"]
            == event_response["data"]["room_state"]
        )
        assert (
            direct_response["data"]["client_id"] == event_response["data"]["client_id"]
        )
...
class TestRoomAdapterContracts:
    ...
    async def test_create_room_adapter_contract(self, websocket):
        ...
        assert direct_response["event"] == event_response["event"]
        assert (
            direct_response["data"]["host_name"] == event_response["data"]["host_name"]
        )
    ...
    async def test_create_room_error_contract(self, websocket):
        ...
        message = {"action": "create_room", "data": {}}  # Missing player_name
    ...
    async def test_join_room_adapter_contract(self, websocket):
        ...
        assert direct_response["data"]["room_id"] == event_response["data"]["room_id"]
        assert (
            direct_response["data"]["player_name"]
            == event_response["data"]["player_name"]
        )
        assert direct_response["data"]["success"] == event_response["data"]["success"]
    ...
    async def test_add_bot_adapter_contract(self, websocket):
        ...
        assert direct_response["event"] == event_response["event"]
        assert (
            direct_response["data"]["difficulty"]
            == event_response["data"]["difficulty"]
        )
        assert direct_response["data"]["success"] == event_response["data"]["success"]
