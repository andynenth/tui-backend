...
class TestStateChangeEventPublisher:
    ...
    async def test_turn_winner_event_published(self, publisher, state_machine):
        ...
        await publisher.on_phase_data_update(
            state_machine,
            GamePhase.TURN,
            {"turn_winner": "Alice", "winning_pieces": ["K", "K", "K"]},
            {},
            "Alice won the turn",
        )
        ...
        winner_events = [
            e for e in self.published_events if isinstance(e, TurnWinnerDetermined)
        ]
    ...
    async def test_scoring_event_published(self, publisher, state_machine):
        ...
        await publisher.on_phase_data_update(
            state_machine,
            GamePhase.SCORING,
            {"scores": {"Alice": 10, "Bob": -5, "Carol": 0}, "round_number": 1},
            {},
            "Scores calculated",
        )
        ...
        score_events = [
            e for e in self.published_events if isinstance(e, ScoresCalculated)
        ]
    ...
    async def test_round_transition_events(self, publisher, state_machine):
        ...
        await publisher.on_phase_data_update(
            ...
            {"round_number": 2, "starter_player": "Bob"},
            {"round_number": 1},
            "New round started",
        )
        ...
        started_events = [
            e for e in self.published_events if isinstance(e, RoundStarted)
        ]
    ...
    async def test_game_ended_event(self, publisher, state_machine):
        ...
        await publisher.on_phase_data_update(
            ...
            GamePhase.SCORING,
            {
                "game_over": True,
                "winner": "Alice",
                "final_scores": {"Alice": 55, "Bob": 45, "Carol": 30},
                "total_rounds": 10,
            },
            {},
            "Game ended",
...
class TestStateMachineIntegration:
    ...
    async def test_update_phase_data_publishes_events(self, test_state):
        ...
        with patch("engine.state_machine.base_state.broadcast", new_callable=AsyncMock):
            ...
            await test_state.update_phase_data(
                {"current_player": "Alice", "turn_number": 1}, "Alice's turn started"
            )
    ...
    async def test_broadcast_custom_event_publishes_events(self, test_state):
        ...
        with patch("engine.state_machine.base_state.broadcast", new_callable=AsyncMock):
            ...
            await test_state.broadcast_custom_event(
                "special_bonus",
                {"player": "Alice", "bonus": 10},
                "Special bonus awarded",
            )

        # Should have published CustomGameEvent
        custom_events = [
            e for e in self.published_events if isinstance(e, CustomGameEvent)
        ]
    ...
    async def test_event_publishing_error_handling(self, test_state):
        ...
        try:
            # Mock broadcast
            with patch(
                "engine.state_machine.base_state.broadcast", new_callable=AsyncMock
            ):
                # Update should still work despite event error
                await test_state.update_phase_data({"test": "data"}, "Test update")
