...
from domain.events.all_events import RoomCreated, PlayerJoinedRoom, PiecesPlayed
...
class TestEventSystemPerformance:
    ...
    async def test_event_handling_scalability(self):
        ...
        for count in handler_counts:
            ...
            results.append({"handlers": count, "avg_time_ms": avg_time_ms})
        ...
        for result in results:
            ...
            print(
                f"  {result['handlers']:<10} {result['avg_time_ms']:<15.3f} {per_handler_us:<20.1f}"
            )
    ...
    async def test_event_bus_memory_usage(self):
        ...
        for i in range(event_count):
            event = RoomCreated(
                room_id=f"room{i}", host_name=f"Host{i}", metadata=metadata
            )
            await bus.publish(event)
