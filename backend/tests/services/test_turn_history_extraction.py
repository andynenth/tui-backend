...
class TestTurnHistoryExtraction:
    ...
    def test_single_turn_extraction(self):
        ...
        turn_plays = [
            TurnPlay(player=self.players[0], pieces=[Piece("SOLDIER_RED")], is_valid=True),
            TurnPlay(player=self.players[1], pieces=[Piece("SOLDIER_BLACK")], is_valid=True),
            TurnPlay(player=self.players[2], pieces=[Piece("ELEPHANT_RED")], is_valid=True),
            TurnPlay(player=self.players[3], pieces=[Piece("SOLDIER_BLACK")], is_valid=True),
        ]
    ...
    def test_multiple_turns_extraction(self):
        ...
        turn2_plays = [
            TurnPlay(player=self.players[2], pieces=[Piece("SOLDIER_RED"), Piece("SOLDIER_BLACK")], is_valid=True),
            TurnPlay(player=self.players[3], pieces=[Piece("SOLDIER_RED"), Piece("SOLDIER_BLACK")], is_valid=True),
            TurnPlay(player=self.players[0], pieces=[Piece("SOLDIER_RED"), Piece("SOLDIER_BLACK")], is_valid=True),
            TurnPlay(player=self.players[1], pieces=[Piece("SOLDIER_RED"), Piece("SOLDIER_BLACK")], is_valid=True),
        ]
        ...
        turn3_plays = [
            TurnPlay(player=self.players[2], pieces=[Piece("CHARIOT_BLACK"), Piece("HORSE_RED"), Piece("CANNON_BLACK")], is_valid=True),
            TurnPlay(player=self.players[3], pieces=[Piece("CHARIOT_RED"), Piece("HORSE_BLACK"), Piece("CANNON_RED")], is_valid=True),
            TurnPlay(player=self.players[0], pieces=[Piece("CHARIOT_BLACK"), Piece("HORSE_RED"), Piece("CANNON_BLACK")], is_valid=True),
            TurnPlay(player=self.players[1], pieces=[Piece("CHARIOT_RED"), Piece("HORSE_BLACK"), Piece("CANNON_RED")], is_valid=True),
        ]
    ...
    def test_hand_state_tracking(self):
        ...
        for i, player in enumerate(self.players):
            ...
            pieces_to_play = [p for p in player.hand if p.kind.startswith("SOLDIER")][0:1]
            turn_plays.append(TurnPlay(player=player, pieces=pieces_to_play, is_valid=True))
        ...
        self.game.turn_history_this_round.append({
            ...
            'hands_after_play': {
                player.name: [p for p in initial_hands[player.name] if p not in turn_plays[i].pieces]
                for i, player in enumerate(self.players)
            }
        })
        ...
        assert not any(p.kind == played_piece.kind and p.point == played_piece.point for p in bot1_play.hand_after)
    ...
    def test_play_type_classification(self):
        ...
        play_types_data = [
            ...
            # Three of a kind
            ([Piece("SOLDIER_BLACK"), Piece("SOLDIER_BLACK"), Piece("SOLDIER_BLACK")], "THREE_OF_A_KIND"),
            # Straight
            ([Piece("CHARIOT_RED"), Piece("HORSE_RED"), Piece("CANNON_RED")], "STRAIGHT"),
        ]
        ...
        for turn_num, (pieces, expected_type) in enumerate(play_types_data, 1):
            ...
            for player in self.players:
                ...
                if player == self.players[0]:
                    turn_plays.append(TurnPlay(player=player, pieces=pieces, is_valid=True))
                else:
                    # Others play same number of soldiers
                    match_pieces = [Piece("SOLDIER_BLACK") for _ in range(len(pieces))]
                    turn_plays.append(TurnPlay(player=player, pieces=match_pieces, is_valid=True))
