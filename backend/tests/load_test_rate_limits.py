...
class TestMetrics:
    ...
    endpoint_stats: Dict[str, Dict[str, int]] = field(default_factory=lambda: defaultdict(lambda: {
        "total": 0, "success": 0, "rate_limited": 0, "failed": 0
    }))
...
class WebSocketLoadTestClient(LoadTestClient):
    ...
    async def _pattern_normal_game(self, websocket):
        ...
        while self.active and (time.time() - start_time) < 60:  # 1 minute of gameplay
            ...
            # Play pieces
            await self._send_event(websocket, "play_pieces", {
                "player_name": player_name,
                "pieces": [random.randint(1, 13) for _ in range(random.randint(1, 3))]
            })
    ...
    async def _pattern_rapid_declarations(self, websocket):
        ...
        for i in range(20):
            await self._send_event(websocket, "declare", {
                "player_name": player_name,
                "pile_count": random.randint(0, 8)
            })
...
class LoadTestRunner:
    ...
    async def run(self):
        ...
        if not self.config.api_only:
            ...
            ws_clients_start = 0 if self.config.ws_only else self.config.num_clients // 2
            ws_clients_count = self.config.num_clients if self.config.ws_only else self.config.num_clients - ws_clients_start
            
            for i in range(ws_clients_count):
                client = WebSocketLoadTestClient(ws_clients_start + i, self.config, self.metrics)
                self.clients.append(client)
                tasks.append(asyncio.create_task(client.run()))
    ...
    def _print_results(self):
        ...
        print(f"Successful: {self.metrics.successful_requests} ({self._percentage(self.metrics.successful_requests, self.metrics.total_requests)}%)")
        print(f"Rate Limited: {self.metrics.rate_limited_requests} ({self._percentage(self.metrics.rate_limited_requests, self.metrics.total_requests)}%)")
        print(f"Failed: {self.metrics.failed_requests} ({self._percentage(self.metrics.failed_requests, self.metrics.total_requests)}%)")
        print(f"Warnings Received: {self.metrics.warnings_received}")
        
        # Performance metrics
        if self.metrics.response_times:
            print("\n--- Performance Metrics ---")
            print(f"Avg Response Time: {statistics.mean(self.metrics.response_times)*1000:.1f}ms")
            ...
            print(f"P95 Response Time: {self._percentile(self.metrics.response_times, 95)*1000:.1f}ms")
            print(f"P99 Response Time: {self._percentile(self.metrics.response_times, 99)*1000:.1f}ms")
        ...
        for endpoint, stats in sorted(self.metrics.endpoint_stats.items()):
            if stats["total"] > 0:
                ...
                print(f"  Success: {stats['success']} ({self._percentage(stats['success'], stats['total'])}%)")
                print(f"  Rate Limited: {stats['rate_limited']} ({self._percentage(stats['rate_limited'], stats['total'])}%)")
                print(f"  Failed: {stats['failed']} ({self._percentage(stats['failed'], stats['total'])}%)")
        ...
        if self.metrics.cpu_usage:
            ...
            print(f"Avg Memory Usage: {statistics.mean(self.metrics.memory_usage):.1f}MB")
        ...
        if self.metrics.rate_limited_requests > 0:
            ...
            print(f"Rate limit enforcement rate: {self._percentage(self.metrics.rate_limited_requests, self.metrics.total_requests)}%")
        else:
...
async def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="Load test rate limiting implementation")
    ...
    parser.add_argument("--aggressive", action="store_true", help="Use aggressive test patterns")
    parser.add_argument("--base-url", default="http://localhost:5050", help="Base URL for API")
    parser.add_argument("--ws-base-url", default="ws://localhost:5050", help="Base URL for WebSocket")
