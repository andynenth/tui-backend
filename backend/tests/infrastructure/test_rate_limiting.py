...
class TestTokenBucket:
    ...
    async def test_cost_based_consumption(self):
        ...
        config = RateLimitConfig(
            algorithm=RateLimitAlgorithm.TOKEN_BUCKET, capacity=100, refill_rate=1.0
        )
        limiter = TokenBucketRateLimiter(config)
...
class TestDistributedRateLimiting:
    ...
    async def test_memory_store_operations(self):
        ...
        total = await store.get_window_sum(
            "window1", now - timedelta(seconds=1), now + timedelta(seconds=2)
        )
        assert total == 3
    ...
    async def test_composite_rate_limiter(self):
        ...
        second_config = RateLimitConfig(
            algorithm=RateLimitAlgorithm.TOKEN_BUCKET, capacity=10, refill_rate=10.0
        )
        ...
        composite = CompositeRateLimiter(
            [("per_second", second_limiter), ("per_minute", minute_limiter)]
        )
...
class TestGameRateLimiting:
    ...
    async def test_game_action_costs(self):
        ...
        limiter = create_game_rate_limiter(player_limit=100, adaptive=False)

        player_id = "player1"
        room_id = "room1"

        # Cheap action
        result = await limiter.check_action(player_id, room_id, "send_message")
        assert result.allowed is True

        # Expensive action
        result = await limiter.check_action(player_id, room_id, "create_room")
        assert result.allowed is True
    ...
    async def test_room_level_limiting(self):
        ...
        limiter = create_game_rate_limiter(
            player_limit=100, room_limit=50, adaptive=False
        )
    ...
    async def test_adaptive_rate_limiting(self):
        ...
        result = await limiter.check_action(player_id, room_id, "play_piece", data)
        assert result.allowed is True
...
class TestRateLimitIntegration:
    ...
    async def test_rate_limit_decorator(self):
        ...
        @RateLimitDecorator(limiter, lambda *args: "test_key")
    ...
    async def test_factory_functions(self):
        ...
        tb_limiter = create_rate_limiter(
            algorithm=RateLimitAlgorithm.TOKEN_BUCKET, capacity=100
        )
...
class TestRateLimitingPerformance:
    ...
    async def test_high_concurrency(self):
        ...
        limiter = create_rate_limiter(
            algorithm=RateLimitAlgorithm.TOKEN_BUCKET, capacity=1000, refill_rate=100
        )
        ...
        for i, result in enumerate(results):
            user_id = f"user{i // 10}"
            success_by_user[user_id] = success_by_user.get(user_id, 0) + (
                1 if result else 0
            )
    ...
    async def test_memory_efficiency(self):
        ...
        optimized = OptimizedTokenBucket(
            RateLimitConfig(
                algorithm=RateLimitAlgorithm.TOKEN_BUCKET, capacity=100, refill_rate=10
            ),
