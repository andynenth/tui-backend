...
class TestConnectionManager:
    ...
    async def test_connection_lifecycle(self, connection_manager, mock_websocket):
        ...
        connection = await connection_manager.connect(
            mock_websocket, "test-connection-1"
        )
        ...
        # Authenticate
        await connection_manager.authenticate("test-connection-1", "player-123")

        connection = await connection_manager.registry.get_connection(
            "test-connection-1"
        )
        ...
        # Join room
        await connection_manager.join_room("test-connection-1", "room-456")
...
class TestWebSocketRepositories:
    ...
    async def test_room_repository_broadcasting(
        self, connection_manager, subscription_manager, mock_websocket
    ):
        """Test room repository with broadcasting."""
        # Create repository
        repo = WebSocketRoomRepository(connection_manager, subscription_manager)

        # Subscribe to room changes
        connection = await connection_manager.connect(mock_websocket, "test-conn")
        await subscription_manager.subscribe("test-conn", "room", "room-1")
    ...
    async def test_subscription_filtering(self, subscription_manager):
        ...
        sub_id = await subscription_manager.subscribe(
            "conn-1", "room", filter_func=only_created
        )
...
class TestEventPropagation:
    ...
    async def test_event_broadcasting(
        self, connection_manager, event_propagator, mock_websocket
    ):
...
class TestMiddleware:
    ...
    async def test_middleware_chain(self, connection_manager, mock_websocket):
        ...
        # Handle connection
        connection = await infra.handle_connection(mock_websocket, "test-conn")
    ...
    async def test_rate_limiting_middleware(self, mock_websocket):
        ...
        connection = ConnectionInfo(
            connection_id="test-conn", websocket=mock_websocket, player_id="player-1"
        )
    ...
    async def test_error_handling_middleware(self, mock_websocket):
        ...
        connection = ConnectionInfo(connection_id="test-conn", websocket=mock_websocket)
...
class TestStateSynchronization:
    ...
    async def test_full_sync_strategy(
        ...
        # Sync state
        state = {"version": 5, "data": {"field": "value"}}

        await sync.sync_state(connection, "room", "room-1", state)
    ...
    async def test_delta_sync_strategy(
        ...
        await sync.sync_state(connection, "room", "room-1", state, client_version=1)
    ...
    async def test_state_synchronizer(
        ...
        synchronizer = StateSynchronizer(connection_manager, event_propagator)
        ...
        # Subscribe to sync
        await synchronizer.subscribe_to_sync("test-conn", "room", "room-1")

        # Sync state
        state = {"version": 1, "data": {}}
        await synchronizer.sync_state("room", "room-1", state)
...
class TestConnectionRecovery:
    ...
    async def test_recovery_token_generation(self, connection_manager):
        ...
        handler = ReconnectionHandler(connection_manager, RecoveryConfig())
    ...
    @pytest.mark.asyncio
    async def test_successful_recovery(self, connection_manager, mock_websocket):
        ...
        new_conn = ConnectionInfo(connection_id="new-conn", websocket=mock_websocket)
    ...
    async def test_recovery_manager(
        ...
        recovery_manager = ConnectionRecoveryManager(
            connection_manager, state_sync, event_propagator
        )
        ...
        connection = await recovery_manager.handle_connection(
            ws, "new-conn", recovery_token=None  # No recovery
        )
...
class TestWebSocketIntegration:
    ...
    async def test_full_game_flow(self, connection_manager, subscription_manager):
        ...
        room_repo = WebSocketRoomRepository(connection_manager, subscription_manager)
        ...
        for i in range(2):
            ...
            await subscription_manager.subscribe(f"player-{i}", "room")
        ...
        await event_bus.emit_to_room("game-room", "game_started", {"game_id": "game-1"})
