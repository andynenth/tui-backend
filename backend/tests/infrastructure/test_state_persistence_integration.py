...
class TestGameStatePersistence:
    ...
    async def game_setup(self):
        ...
        room = Room("TEST123")
        players = [Player(f"player{i}", f"Player {i}") for i in range(4)]
    ...
    @pytest.mark.asyncio
    async def test_persist_game_state_transitions(
        self, game_setup, persistence_manager
    ):
        ...
        # Save initial state
        await persistence_manager.save_state(f"game_{game.id}", initial_state)

        # Start game (transition to PREPARATION)
        action = GameAction(
            type=ActionType.START_GAME, actor_id=players[0].id, payload={}
        )
        ...
        await persistence_manager.save_state(f"game_{game.id}", prep_state)
    ...
    async def test_recover_game_state(self, game_setup, persistence_manager):
            ...
            if result.success:
                ...
                if persistence_manager.transition_logger and result.transition:
                    await persistence_manager.handle_transition(
                        f"game_{game.id}", result.transition
                    )
        ...
        recovered = await persistence_manager.recover_state(
            f"game_{game.id}", RecoveryOptions(mode=RecoveryMode.LATEST)
        )
    ...
    async def test_performance_metrics(self, game_setup, persistence_manager):
        ...
        for i in range(10):
            ...
            # Save
            await persistence_manager.save_state(f"game_{game.id}", state, force=True)
    ...
    async def test_concurrent_game_persistence(self, persistence_manager):
        ...
        for i in range(5):
            ...
            games.append({"game": game, "state_machine": state_machine, "room": room})

        # Save all games concurrently
        save_tasks = []
        for game_data in games:
            state = await game_data["state_machine"].get_state_data()
            task = persistence_manager.save_state(f"game_{game_data['game'].id}", state)
    ...
    @pytest.mark.asyncio
    async def test_state_machine_recovery_after_error(
        self, game_setup, persistence_manager
    ):
        ...
        await persistence_manager.save_state(
            f"game_{game.id}", checkpoint_state, force=True
        )
        ...
        recovery_options = RecoveryOptions(
            mode=RecoveryMode.LATEST, validate_state=True
        )

        recovered = await persistence_manager.recover_state(
            f"game_{game.id}", recovery_options
        )
