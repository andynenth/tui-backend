...
class TestGameAggregate(EventSourcedAggregate):
    ...
    def create(self, room_id: str, players: List[str]) -> None:
        ...
        self.raise_event(
            GameCreatedEvent(
                room_id=room_id, players=players, config={"max_rounds": 20}
            )
        )
...
class TestHybridEventStore:
    ...
    async def test_version_filtering(self, event_store):
        ...
        filtered_events = await event_store.get_events(
            game_id, from_version=3, to_version=7
        )
    ...
    async def test_get_all_events_by_type(self, event_store):
        ...
        created_events = await event_store.get_all_events(
        ...
        started_events = await event_store.get_all_events(
