...
class TestCachePatterns:
    ...
    async def test_cache_stampede_prevention(self):
        ...
        tasks = [
            stampede.get_or_load("key1", slow_loader)
            for _ in range(5)
        ]
...
class TestInvalidationStrategies:
    ...
    async def test_immediate_invalidation(self):
        ...
        # Trigger event
        invalidated = await invalidator.handle_event(
            "user_updated",
            {"user_id": "1"}
        )
...
class TestCacheWarming:
    ...
    async def test_frequently_accessed_warming(self):
        ...
        # Simulate access log
        access_log = [
            "user:1", "user:1", "user:1",  # 3 times
            "user:2", "user:2",             # 2 times
            "user:3",                       # 1 time
            "post:1", "post:1"              # 2 times
        ]
        ...
        # Warm cache
        warmed = await warm_frequently_accessed(
            cache,
            access_log,
            load_data,
            top_n=2
