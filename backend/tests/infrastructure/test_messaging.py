...
class TestMessage:
    ...
    def test_message_serialization(self):
        ...
        message = Message(payload={"key": "value"}, priority=MessagePriority.HIGH)
...
class TestRetryPolicy:
    ...
    def test_no_exponential_backoff(self):
        ...
        policy = RetryPolicy(
            initial_delay=timedelta(seconds=5), exponential_backoff=False
        )
...
class TestInMemoryQueue:
    ...
    async def test_concurrent_operations(self):
        ...
        await asyncio.gather(producer(10), producer(10), consumer(20))
...
class TestPriorityQueue:
    ...
    async def test_priority_ordering(self):
        ...
        messages = [
            ...
            (
                Message(payload="critical", priority=MessagePriority.CRITICAL),
                MessagePriority.CRITICAL,
            ),
            (
                Message(payload="normal", priority=MessagePriority.NORMAL),
                MessagePriority.NORMAL,
            ),
            (
                Message(payload="high", priority=MessagePriority.HIGH),
...
class TestBoundedQueue:
    ...
    async def test_drop_oldest_overflow(self):
        ...
        queue = BoundedInMemoryQueue(
            "test", max_size=3, overflow_strategy="drop_oldest"
        )
...
class TestDeadLetterQueue:
    ...
    async def test_add_to_dlq(self):
        ...
        message_id = await dlq.add(
            message, DeadLetterReason.PROCESSING_FAILED, error, "original_queue"
        )
    ...
    async def test_dlq_handler(self):
        ...
        dlq_handler = DeadLetterHandler(handler, dlq, DeadLetterPolicy(max_retries=2))
    ...
    async def test_retryable_dlq(self):
        ...
        dlq = RetryableDeadLetterQueue(
            "test_rdlq",
            DeadLetterPolicy(max_retries=3, retry_delay=timedelta(milliseconds=100)),
        )
        ...
        await dlq.add(
            message, DeadLetterReason.PROCESSING_FAILED, original_queue="original"
        )
...
class TestMessageSerialization:
    ...
    def test_json_serialization(self):
        ...
        # Create message
        message = Message(
            payload={"key": "value", "number": 42}, priority=MessagePriority.HIGH
        )
...
class TestMessageHandlers:
    ...
    async def test_batch_handler(self):
        ...
        handler = BatchMessageHandler(
            batch_func, batch_size=3, batch_timeout=timedelta(milliseconds=100)
        )
    ...
    async def test_timeout_handler(self):
        ...
        timeout_handler = TimeoutHandler(handler, timeout=timedelta(milliseconds=100))
...
class TestGameIntegration:
    ...
    async def test_game_event_handler(self):
        ...
        start_event = GameEvent(
            event_type=GameEventType.GAME_STARTED, room_id="room123"
        )
        ...
        player_event = GameEvent(
            event_type=GameEventType.PLAYER_CONNECTED, room_id="room123"
        )
    ...
    async def test_game_task_processor(self):
        ...
        for i in range(5):
            await processor.submit_task(
                "custom_task", {"index": i}, priority=MessagePriority.NORMAL
            )
