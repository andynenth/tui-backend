...
class TestGameMetrics:
    ...
    async def test_game_lifecycle_metrics(self):
        ...
        await collector.record_game_start(
            game_id="game123", room_id="room456", player_count=4, bot_count=2
        )
    ...
    async def test_phase_duration_metrics(self):
        ...
        await collector.record_phase_duration(
            game_id="game123", phase="PREPARATION", duration_seconds=30.5
        )

        await collector.record_phase_duration(
            game_id="game123", phase="DECLARATION", duration_seconds=45.2
        )
...
class TestTracing:
    ...
    async def test_state_transition_tracing(self):
        ...
        async with tracer.trace_state_transition(
            game_id="game123", from_state="PREPARATION", to_state="DECLARATION"
        ) as span:
    ...
    async def test_action_processing_tracing(self):
        ...
        async with tracer.trace_action_processing(
            game_id="game123", action_type="play_piece", player_id="player1"
        ) as span:
            assert span is not None
            # Process action
            await asyncio.sleep(0.01)
...
class TestEventStream:
    ...
    async def test_event_filtering(self):
        ...
        filter = EventFilter(event_types={EventType.GAME_STARTED, EventType.GAME_ENDED})
        subscriber = await stream.subscribe("filtered", filter)

        # Publish various events
        await stream.publish_event(SystemEvent(event_type=EventType.GAME_STARTED))
        await stream.publish_event(SystemEvent(event_type=EventType.PLAYER_JOINED))
        await stream.publish_event(SystemEvent(event_type=EventType.GAME_ENDED))
...
class TestVisualization:
    ...
    def test_state_node_updates(self):
        ...
        # Update state
        viz.update_state_node("PREPARATION", active_games_delta=5, duration_sample=30.0)
    ...
    def test_room_visualization(self):
        ...
        viz.update_room_visualization(
            room_id="room123", game_id="game456", current_state="TURN", player_count=4
        )
...
class TestEnterpriseMonitor:
    ...
    async def test_monitor_game_lifecycle(self):
        ...
        await monitor.record_game_start(
            game_id="game123", room_id="room456", player_count=4, bot_count=0
        )
