...
class TestSnapshots:
    ...
    async def test_filesystem_snapshot(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            store = FileSystemSnapshotStore(
                Path(temp_dir), SnapshotConfig(compression_enabled=True)
            )
    ...
    async def test_snapshot_manager(self):
        ...
        stores = [InMemorySnapshotStore(), InMemorySnapshotStore()]
    ...
    async def test_filesystem_transition_log(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            log = FileSystemTransitionLog(
                Path(temp_dir), max_file_size_mb=1, max_files_per_state_machine=5
            )
...
class TestRecovery:
    ...
    async def test_snapshot_recovery(self):
        ...
        result = await recovery.recover(
            "game1", RecoveryOptions(mode=RecoveryMode.LATEST)
        )
        ...
        result = await recovery.recover(
            "game1",
            RecoveryOptions(mode=RecoveryMode.SNAPSHOT, snapshot_id=snapshot_ids[0]),
        )
        assert result.is_successful
    ...
    async def test_event_sourced_recovery(self):
        ...
        transitions = [
            ...
            StateTransition(
                "playing", "scoring", "score", {"scores": [10, 20, 15, 25]}
            ),
        ]
    ...
    async def test_hybrid_recovery(self):
        ...
        event_recovery = EventSourcedRecovery(
            [transition_log], lambda x: {"current_state": "idle"}
        )
...
class TestEventSourcing:
    ...
    async def test_event_store(self):
        ...
        create_event = await event_store.create_state_machine(
            "game1", initial_state, actor_id="system"
        )
        ...
        update_event = await event_store.update_state(
            "game1", {"players": ["alice", "bob"]}, actor_id="system"
        )
    ...
    async def test_event_sourced_state_machine(self):
        ...
        # Save new state
        state = {"current_state": "playing", "round": 1, "scores": [0, 0, 0, 0]}
    ...
    async def test_custom_projection(self):
                ...
                if event.event_type == StateEventType.TRANSITIONED:
                    if event.event_data.get("to_state") == "game_over":
                        ...
                        state["final_stats"] = {
                            "duration": 0,
                            "winner": None,
                            "final_scores": state.get("state_data", {}).get(
                                "scores", []
                            ),
                        }
        ...
        store = InMemoryEventStore()
        event_store = StateMachineEventStore(store, projections=[GameStateProjection()])

        # Create and transition state
        await event_store.create_state_machine("game1", {"current_state": "playing"})

        transition = StateTransition(
            "playing", "game_over", "end_game", {"scores": [100, 90, 80, 70]}
        )
...
class TestVersioning:
    ...
    async def test_version_manager(self):
        ...
        version_manager = StateVersionManager(
            base_storage, migration_runner, conflict_resolver
        )

        # Save versioned state
        state = {"current_state": "active", "data": {"value": 42}}
    ...
    async def test_migration_runner(self):
        ...
        migrated = await runner.migrate(
            state, StateVersion(1, 0, 0), StateVersion(2, 0, 0)
        )
...
class TestPersistenceManager:
    ...
    async def test_persistence_manager_hybrid(self):
        ...
        config = PersistenceConfig(
            strategy=PersistenceStrategy.HYBRID, cache_enabled=True, cache_size=10
        )

        # Create manager
        manager = StatePersistenceManager(
            config, snapshot_stores, transition_logs, event_store
        )
        ...
        # Handle transition
        transition = StateTransition("playing", "scoring", "end_round", {"round": 1})
