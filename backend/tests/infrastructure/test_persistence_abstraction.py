...
class TestMemoryAdapter:
    ...
    async def test_basic_operations(self):
        ...
        config = PersistenceConfig(
            backend=PersistenceBackend.MEMORY, options={"max_items": 100}
        )
    ...
    async def test_batch_operations(self):
        ...
        # Save many
        entities = {
            str(i): TestEntity(id=str(i), name=f"test{i}", value=i) for i in range(10)
        }
    ...
    async def test_lru_eviction(self):
        ...
        config = PersistenceConfig(
            backend=PersistenceBackend.MEMORY, options={"max_items": 3}
        )
        adapter = MemoryAdapter[TestEntity](config)
    ...
    async def test_query_operations(self):
        ...
        sorted_results = await adapter.query_sorted(
            {}, "value", ascending=False, limit=3  # No filter
        )
...
class TestHybridRepository:
    ...
    async def test_memory_first_access(self):
        ...
        repo = HybridRepository(
            memory_adapter=memory_adapter, persistent_adapter=persistent_adapter
        )
...
class TestRepositoryFactory:
    ...
    def test_custom_configuration(self):
        ...
        config = RepositoryConfig(
            ...
            memory_config=PersistenceConfig(
                backend=PersistenceBackend.MEMORY, options={"max_items": 50}
            ),
        )
    ...
    def test_adapter_registration(self):
        ...
        factory.register_adapter(PersistenceBackend.POSTGRESQL, MockPersistentAdapter)

        # Create repository with custom backend
        config = RepositoryConfig(
            strategy=RepositoryStrategy.PERSISTENT_ONLY,
            persistent_config=PersistenceConfig(backend=PersistenceBackend.POSTGRESQL),
        )
