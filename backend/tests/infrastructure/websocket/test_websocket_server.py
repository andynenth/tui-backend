...
class TestWebSocketServer:
    ...
    async def test_send_error(self, websocket_server, mock_websocket):
        ...
        await websocket_server.send_error(
            mock_websocket,
            "Test error",
            "test_error"
        )
    ...
    async def test_check_rate_limit_not_limited(self, websocket_server, mock_websocket):
        ...
        with patch('infrastructure.websocket.websocket_server.check_websocket_rate_limit') as mock_check:
    ...
    async def test_check_rate_limit_limited(self, websocket_server, mock_websocket):
        ...
        with patch('infrastructure.websocket.websocket_server.check_websocket_rate_limit') as mock_check:
            with patch('infrastructure.websocket.websocket_server.send_rate_limit_error') as mock_send_error:
    ...
    @pytest.mark.asyncio
    async def test_handle_disconnect_no_connection_id(self, websocket_server, mock_websocket):
        """Test handling disconnection when no connection ID found."""
        room_id = "test_room"
        
        with patch('infrastructure.websocket.websocket_server.get_connection_id_for_websocket') as mock_get_id:
