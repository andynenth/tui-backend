...
async def measure_writes_without_buffer():
    ...
    for i in range(4):
        await store.store_event("TEST001", "player_declared", {"player": f"p{i}", "value": i})
        write_count += 1
...
async def measure_writes_with_buffer():
    ...
    # Declarations (4 players)
    for i in range(4):
        await store.store_event_buffered("TEST002", "player_declared", {"player": f"p{i}", "value": i})
        write_count += 1
...
async def main():
    ...
    print(f"   Buffer flushes: {with_buffer_results['buffer_metrics']['total_flushes']}")
    
    # Calculate improvement
    print("\n3. Performance Improvement")
    print("=" * 50)
    
    write_reduction = (1 - (with_buffer_results['actual_writes'] / no_buffer_results['actual_writes'])) * 100
    ...
    print(f"   I/O operations saved: {no_buffer_results['actual_writes'] - with_buffer_results['actual_writes']}")
    ...
    if no_buffer_results['actual_writes'] == with_buffer_results['actual_writes']:
        ...
        print(f"   ⚠️  Event count mismatch: {no_buffer_results['actual_writes']} vs {with_buffer_results['actual_writes']}")
