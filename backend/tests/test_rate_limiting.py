...
from api.middleware.rate_limit import (
    RateLimiter, RateLimitRule, RateLimitStats,
    get_rate_limiter, RateLimitMiddleware
)
from api.middleware.websocket_rate_limit import (
    WebSocketRateLimiter, get_websocket_rate_limiter,
    check_websocket_rate_limit, send_rate_limit_error,
...
class TestRateLimiter:
    ...
    async def test_stats_tracking(self, rate_limiter, basic_rule):
        ...
        for i in range(3):
            for j in range(5):
                await rate_limiter.check_rate_limit(f"client_{i}", basic_rule, "test_route")
...
class TestIntegrationScenarios:
    ...
    async def test_abuse_scenario_protection(self):
        ...
        blocked_at = None
        for i in range(10):
            allowed, _ = await check_websocket_rate_limit(mock_ws, "lobby", "create_room")
            if not allowed and blocked_at is None:
                blocked_at = i + 1
        
        assert blocked_at is not None and blocked_at <= 6, "Rapid room creation should be blocked"
        ...
        assert blocked_at is not None and blocked_at <= 6, "Declaration spam should be blocked"
    ...
    async def test_multiple_clients_fair_usage(self):
        ...
        for client in clients:
            for _ in range(3):
                allowed, _ = await check_websocket_rate_limit(client, room_id, "declare")
