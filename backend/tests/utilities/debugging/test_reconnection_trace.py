...
class ReconnectionTrace:
    ...
    async def run_trace(self):
        ...
        self.log(
            "1. SETUP: Create room and game",
            {"room_id": "test_room", "players": ["Alice", "Bob", "Charlie", "David"]},
        )
        ...
        self.log(
            "3. VERIFY: All players connected",
            {
                "alice_connected": alice_conn is not None,
                "bob_connected": bob_conn is not None,
                "alice_status": (
                    alice_conn.connection_status.value if alice_conn else None
                ),
        ...
        self.log(
            "5. VERIFY: Disconnect handled",
            {
                ...
                "disconnect_time": (
                    connection.disconnect_time.isoformat()
                    if connection and connection.disconnect_time
                    else "None"
                ),
            },
        )
        ...
        for event_type, data in events:
            await message_queue_manager.queue_message(
                "test_room", "Bob", event_type, data
            )
            self.log(
                f"   - Queued: {event_type}",
                {"critical": event_type in message_queue_manager.CRITICAL_EVENTS},
            )
        ...
        can_reconnect = await self.connection_manager.check_reconnection(
            "test_room", "Bob"
        )
        ...
        self.log(
            "12. RETRIEVE: Get queued messages",
            {
        ...
        self.log(
            "13. RESTORE: Player control restored",
            {"is_bot": bob_player.is_bot, "is_connected": bob_player.is_connected},
        )
        ...
        self.log(
            "14. CLEANUP: Queue cleared",
            {"queues_remaining": final_status["total_queues"]},
        )

        # Step 12: Verify final state
        bob_connection = await self.connection_manager.get_connection(
            "test_room", "Bob"
        )
        ...
        self.log(
            "15. FINAL STATE",
            {
                ...
                "bob_status": (
                    bob_connection.connection_status.value if bob_connection else None
                ),
        ...
        print(
            f"✓ Bot activated on disconnect: {self.trace_log[6]['data'].get('is_bot', False)}"
        )
        print(f"✓ Messages queued: {len(queued_messages)} messages")
        print(
            f"✓ Critical events preserved: {bob_queue.get('critical_count', 0)} critical"
        )
        print(
            f"✓ Player state restored: is_bot={bob_player.is_bot}, connected={bob_player.is_connected}"
        )
...
async def test_edge_cases():
    ...
    print(
        f"  Same player name: {new_conn.player_name == 'Player1' if new_conn else False} ✓"
    )
    ...
    print(
        f"  Critical messages preserved: {queue_info.get('critical_count', 0)} critical messages"
    )
