...
async def test_duplicate_action_prevention():
    ...
    events = [
        bot_manager.handle_game_event("test_room", "phase_change", {
            "phase": "turn",
    ...
    print("✅ SUCCESS: Duplicate action prevention working - logs show duplicates were blocked")
    return True
...
async def test_sequence_tracking():
    ...
    if not should_skip_1 and should_skip_2:
        ...
        print(f"❌ FAILURE: Sequence tracking broken - first: {should_skip_1}, second: {should_skip_2}")
        return False
...
async def test_action_hash_deduplication():
    ...
    if not is_dup_1 and is_dup_2 and not is_dup_3:
        print("✅ SUCCESS: Action hash deduplication working - first allowed, second blocked, third allowed after timeout")
        return True
    else:
        print(f"❌ FAILURE: Action hash deduplication broken - first: {is_dup_1}, second: {is_dup_2}, third: {is_dup_3}")
        return False
