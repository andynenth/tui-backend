...
class GameStateSync:
    ...
    @staticmethod
    def _get_phase_specific_data(
        phase: GamePhase, phase_data: dict, game, player_name: str
    ) -> Dict[str, Any]:
    ...
    @staticmethod
    def _get_valid_declaration_options(
        game, player_name: str, phase_data: dict
    ) -> List[int]:
    ...
    def _get_recent_events(
        game, player_name: str, limit: int = 10
    ) -> List[Dict[str, Any]]:
    ...
    @staticmethod
    async def get_reconnection_summary(
        game, player_name: str, disconnect_time: datetime
    ) -> Dict[str, Any]:
        """
        ...
        if player:
            ...
            position = next(
                i for i, (name, _) in enumerate(scores, 1) if name == player_name
            )
