...
class WebSocketMigrationHelper:
    ...
    def __init__(
        self, room_manager: Union[RoomManager, AsyncRoomManager, AsyncCompatRoomManager]
    ):
        """
        ...
        self.is_async = isinstance(
            room_manager, (AsyncRoomManager, AsyncCompatRoomManager)
        )

        logger.info(
            f"WebSocketMigrationHelper initialized with {'async' if self.is_async else 'sync'} manager"
        )
    ...
    async def create_room(self, host_name: str) -> str:
        ...
        if self.is_async:
            ...
            return await loop.run_in_executor(
                None, self.room_manager.create_room, host_name
            )

    async def get_room(
        self, room_id: str
    ) -> Optional[Union[Room, AsyncRoom, AsyncCompatRoom]]:
...
async def handle_create_room_async(
    helper: WebSocketMigrationHelper, player_name: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"type": "room_created", "room_id": room_id, "room": summary}
...
async def handle_join_room_async(
    helper: WebSocketMigrationHelper, room_id: str, player_name: str
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        return {"type": "joined_room", "slot": slot, "room": summary}
...
async def handle_exit_room_async(
    ...
    try:
        ...
        if is_host:
            ...
            return {"type": "room_deleted", "reason": "host_left"}
        ...
        return {"type": "player_left", "player_name": player_name, "room": summary}

    except Exception as e:
        logger.error(f"Error exiting room: {e}")
        return {"type": "error", "message": "Failed to exit room"}


async def handle_start_game_async(
    helper: WebSocketMigrationHelper, room_id: str, broadcast_callback
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if hasattr(room, "start_game") and asyncio.iscoroutinefunction(room.start_game):
            ...
            return {"type": "error", "message": "Room does not support game start"}

        if result.get("success"):
            return {"type": "game_started", "room_id": room_id, **result}
        else:
            return {
                "type": "error",
...
def is_async_room(room: Any) -> bool:
    ...
    return isinstance(room, (AsyncRoom, AsyncCompatRoom)) or (
        hasattr(room, "join_room") and asyncio.iscoroutinefunction(room.join_room)
    )
