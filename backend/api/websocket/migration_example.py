        ...
        while True:
            ...
            if data["type"] == "create_room":
                ...
                await websocket.send_json(
                    {"type": "room_created", "room_id": room_id, "room": room.summary()}
                )
                ...
                if room:
                    try:
                        ...
                        await websocket.send_json({"type": "joined", "slot": slot})
    ...
    try:
        while True:
            ...
            if data["type"] == "create_room":
                ...
                result = await handle_create_room_async(helper, data["player_name"])
                await websocket.send_json(result)

            elif data["type"] == "join_room":
                # ✅ Async join operation
                result = await handle_join_room_async(
                    helper, data["room_id"], data["player_name"]
                )
                await websocket.send_json(result)
                ...
                result = await handle_start_game_async(
                    ...
                    lambda event_type, data: websocket.send_json(
                        {"type": "game_event", "event": event_type, "data": data}
                    ),
                )
                await websocket.send_json(result)
    ...
    async def handle_player_action(room_id: str, player_name: str, action: dict):
        ...
        if not room:
            await connection_manager.send_to_player(
                room_id, player_name, {"type": "error", "message": "Room not found"}
            )
            return
...
async def robust_error_handling(websocket: WebSocket, helper: WebSocketMigrationHelper):
    ...
    try:
        ...
        try:
            ...
            slot = await asyncio.wait_for(room.join_room(player_name), timeout=5.0)
            ...
            await websocket.send_json(
                {"type": "error", "code": "GAME_ERROR", "message": str(e)}
            )
    ...
    try:
        ...
        while True:
            ...
            if data["type"] == "authenticate":
                ...
                await connection_manager.add_connection(room_id, player_name, websocket)

                # Send room state
                room_summary = (
                    await room.summary() if hasattr(room, "summary") else room.summary()
                )
                await websocket.send_json(
                    {"type": "authenticated", "room": room_summary}
                )
        ...
        if player_name:
            ...
            if is_host:
                ...
                await connection_manager.broadcast_to_room(
                    room_id, {"type": "room_closed", "reason": "host_disconnected"}
                )
