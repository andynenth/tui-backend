...
async def get_events_since_sequence(
    ...
    try:
        ...
        logger.error(
            f"Error retrieving events since sequence {seq} for room {room_id}: {e}"
        )
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_turn_plays(
    room_id: str,
    turn_number: Optional[int] = Query(
        None, description="Specific turn number to retrieve"
    ),
    ...
    try:
        ...
        return {
            ...
            "filter": {"turn_number": turn_number, "player_name": player_name},
            "turns": turn_list,
        }
...
async def validate_room_events(room_id: str):
    ...
    try:
        ...
        if not validation["valid"]:
            ...
            duplicates = {
                seq: count for seq, count in sequence_counts.items() if count > 1
            }
...
async def get_debug_logs(
    ...
    logger_filter: Optional[str] = Query(
        None, description="Filter by logger name (substring match)"
    ),
    since_minutes: Optional[int] = Query(
        None, description="Only logs from last N minutes"
    ),
    search: Optional[str] = Query(
        None, description="Search in log messages (case-insensitive)"
    ),
):
    """
