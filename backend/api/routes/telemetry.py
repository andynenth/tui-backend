...
async def receive_telemetry(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        background_tasks.add_task(
            process_telemetry_data, payload, client_ip, received_at
        )
...
async def process_telemetry_data(
    payload: TelemetryPayload, client_ip: str, received_at: datetime
):
    ...
    try:
        ...
        device_type = detect_device_type(
            session_events[0].get("userAgent", "") if session_events else ""
        )
        initial_route = None
...
async def check_alert_conditions(
    event_type: str, event: Dict[str, Any], connection: Dict[str, Any], user_agent: str
):
...
async def get_bundle_load_stats(
    ...
    group_by: Optional[str] = Query(
        None, description="Group by: device, connection, location"
    ),
):
    ...
    try:
        ...
        summary = {
            ...
            "mobile_percentage": round(mobile / total_sessions * 100, 1)
            if total_sessions > 0
            else 0.0,
        ...
        return {"period": f"last_{hours}_hours", "summary": summary, "groups": groups}
            ...
            errors.append(
                {
                    ...
                    "last_seen": last_seen,
                    "severity": "critical"
                    if event_type in ["bundle_error", "load_failed"]
                    else "warning",
                }
            )
...
async def get_performance_trends(
    ...
    try:
        ...
        if interval == "1h":
            ...
            time_format = (
                "%Y-%m-%d " + str((datetime.utcnow().hour // 6) * 6).zfill(2) + ":00:00"
            )
        else:  # 24h
