...
async def telemetry_monitoring_websocket(websocket: WebSocket):
    ...
    try:
        ...
        logger.info(f"Telemetry monitor connected. Total monitors: {len(connected_monitors)}")
...
async def broadcast_telemetry_event(event_data: Dict[str, Any]):
    ...
    safe_event = {
        ...
        "event": {
            ...
            "error": event_data.get("error") if event_data.get("event") in [
                "bundle_error", "load_timeout", "load_failed", "javascript_error"
            ] else None,
    ...
    try:
        ...
        if stats:
            ...
            overview = {
                ...
                "mobile_percentage": round(mobile / total * 100, 1) if total > 0 else 0.0,
...
async def get_recent_events(limit: int = 20) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        for row in cursor.fetchall():
            session_id, timestamp, event_type, data_json, user_agent, conn_type, received_at = row
    ...
    try:
        ...
        if stats:
            ...
            return {
                ...
                "monitors": {
                    "connected": len(connected_monitors)
                }
            }
        else:
            return {
