    ...
    try:
        while True:
            ...
            if event_name == "ack":
                ...
                print(f"DEBUG_WS_SYNC: Received sync_request from client in room {room_id}, reason: {reason}")
                
                # For game rooms, send current game state
                if room_id != "lobby":
                    room = room_manager.get_room(room_id)
                    if room and room.started and room.game_state_machine:
                        ...
                        if current_phase:
                            ...
                            await registered_ws.send_json({
                                ...
                                "data": {
                                    ...
                                    "sync_response": True,
                                    "reason": f"State sync after {reason}"
                                }
                            })
                            print(f"DEBUG_WS_SYNC: Sent game state sync to client in room {room_id}")
                
                # Also handle socket manager sync for message delivery
                from backend.socket_manager import socket_manager
