...
async def get_play_history_metrics():
    ...
    play_history_endpoints = {
        k: v for k, v in general_metrics["endpoints"].items() if "play-history" in k
    }
...
async def get_time_series_metrics(
    ...
    minutes: int = Query(
        5, description="Number of minutes of data to retrieve", ge=1, le=60
    ),
):
    """
...
async def get_performance_health():
    ...
    for endpoint, stats in metrics["endpoints"].items():
        if stats["p95_response_time_ms"] > 2000:  # > 2s p95
            ...
            issues.append(
                f"{endpoint}: Very slow p95 response time ({stats['p95_response_time_ms']}ms)"
            )
        elif stats["p95_response_time_ms"] > 1000:  # > 1s p95
            health_status = "YELLOW" if health_status == "GREEN" else health_status
            issues.append(
                f"{endpoint}: Slow p95 response time ({stats['p95_response_time_ms']}ms)"
            )
    ...
    for cache_type, cache_stats in metrics["cache"].items():
        if cache_stats["hit_rate"] < 0.5:  # < 50% hit rate
            health_status = "YELLOW" if health_status == "GREEN" else health_status
            issues.append(
                f"{cache_type} cache: Low hit rate ({cache_stats['hit_rate']:.2%})"
            )
...
async def get_alerts(
    minutes: int = Query(
        60, description="Number of minutes of alerts to retrieve", ge=1, le=1440
    ),
    ...
    alerts = alert_service.get_recent_alerts(
        minutes=minutes, severity=severity, alert_type=alert_type
    )

    return {
        "alerts": alerts,
        "count": len(alerts),
        "filter": {"minutes": minutes, "severity": severity, "alert_type": alert_type},
    }
...
async def test_alert(
    severity: str = Query(
        "warning", description="Alert severity", pattern="^(warning|critical)$"
    ),
    message: str = Query("Test alert message", description="Alert message"),
    duration_ms: Optional[float] = Query(
        None, description="Simulated duration for response time alerts"
    ),
):
    """
