...
async def get_event_metrics(event_name: str) -> Dict[str, Any]:
    ...
    if metrics is None:
        raise HTTPException(
            status_code=404,
            detail=f"No metrics found for event '{event_name}'"
        )
...
async def list_event_metrics() -> Dict[str, Any]:
    ...
    return {
        ...
        "total_event_types": len(summary["events"])
    }
...
async def get_time_series_data(
    minutes: int = Query(default=5, ge=1, le=60, description="Number of minutes of data to retrieve")
) -> Dict[str, Any]:
    """
...
async def get_metrics_health() -> Dict[str, Any]:
    ...
    return {
        ...
        "metrics": {
            ...
            "avg_success_rate": sum(m["success_rate"] * m["count"] for m in summary["events"].values()) / 
