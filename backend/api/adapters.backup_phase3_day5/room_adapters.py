...
async def ensure_room_visible_to_legacy(room_id: str) -> None:
    ...
    try:
        ...
        from infrastructure.adapters.legacy_repository_bridge import (
            ensure_room_visible_to_legacy as sync_func,
        )

        logger.info(
            f"[ROOM_CREATE_DEBUG] Legacy bridge imported successfully for room {room_id}"
        )
        await sync_func(room_id)
    except ImportError as e:
        logger.error(
            f"[ROOM_CREATE_DEBUG] Failed to import legacy bridge: {e}", exc_info=True
        )
        logger.warning(
            f"[ROOM_CREATE_DEBUG] Using no-op sync for room {room_id} - import failed"
        )
    except Exception as e:
        logger.error(
            f"[ROOM_CREATE_DEBUG] Error during import/sync: {e}", exc_info=True
        )
...
async def _handle_create_room(
    ...
    logger.debug(
        f"[ROOM_CREATE_DEBUG] Starting room creation for player: {player_name}"
    )
    ...
    if not player_name:
        return {
            "event": "error",
            "data": {"message": "Player name is required", "type": "validation_error"},
        }
    ...
    try:
        ...
        logger.debug(
            f"[ROOM_CREATE_DEBUG] Executing CreateRoomUseCase for player: {player_name}"
        )
                    ...
                    "players": [
                        {
                            ...
                            "avatar_color": getattr(
                                p, "avatar_color", None
                            ),  # Include avatar color if available
                        }
        ...
        logger.info(
            f"[ROOM_CREATE_DEBUG] About to sync room {response_dto.room_info.room_id} to legacy"
        )
        logger.info(
            f"[ROOM_CREATE_DEBUG] Room has {len(response_dto.room_info.players)} players"
        )
        for p in response_dto.room_info.players:
            logger.info(
                f"[ROOM_CREATE_DEBUG]   - {p.player_name} (bot={p.is_bot}, seat={p.seat_position})"
            )
...
async def _handle_join_room(
    ...
    try:
        ...
        # Execute the use case
        logger.debug(
            f"[ROOM_JOIN_DEBUG] Executing JoinRoomUseCase for player: {player_name}"
        )
        response_dto = await use_case.execute(request)
        ...
        # Log for monitoring
        logger.info(
            f"Player {player_name} joined room: {response_dto.room_info.room_id}"
        )
...
async def _handle_leave_room(
    ...
    if not room_id:
        return {
            "event": "error",
            "data": {"message": "Room ID is required", "type": "validation_error"},
        }
    ...
    try:
        ...
        # Execute the use case
        logger.debug(
            f"[ROOM_LEAVE_DEBUG] Executing LeaveRoomUseCase for player: {player_name}"
        )
        response_dto = await use_case.execute(request)
...
async def _handle_get_room_state(
    ...
    try:
        ...
        request = GetRoomStateRequest(
            room_id=room_id, requesting_player_id=data.get("player_id")
        )
        ...
        # Execute the use case
        logger.debug(
            f"[ROOM_STATE_DEBUG] Executing GetRoomStateUseCase for room: {room_id}"
        )
        response_dto = await use_case.execute(request)

        # Log the players found
        logger.info(
            f"[ROOM_STATE_DEBUG] Found {len(response_dto.room_info.players)} players in room {room_id}:"
        )
        for p in response_dto.room_info.players:
            logger.info(
                f"[ROOM_STATE_DEBUG]   - {p.player_name} (bot={p.is_bot}, seat={p.seat_position})"
            )
...
async def _handle_remove_player(
    ...
    try:
        ...
        # Execute the use case
        logger.debug(
            f"[PLAYER_REMOVE_DEBUG] Executing RemovePlayerUseCase for player: {target_player_id}"
        )
        response_dto = await use_case.execute(request)
        ...
        # Log for monitoring
        logger.info(
            f"Player {target_player_id} removed from room {room_id} by {requesting_player_id}"
        )
