...
async def test_async_overhead():
    ...
    print(f"Sync function:      +{(sync_func_time-sync_time)/iterations*1000000:.2f} μs overhead")
    print(f"Async function:     +{(async_time-sync_time)/iterations*1000000:.2f} μs overhead")
    print(f"Dict lookup:        +{(dict_lookup_time-sync_time)/iterations*1000000:.2f} μs overhead")
    print(f"Adapter pattern:    +{(adapter_time-sync_time)/iterations*1000000:.2f} μs overhead")
    
    # Conclusion
    print("\n💡 Key Findings:")
    print(f"- Async function calls add ~{(async_time-sync_func_time)/iterations*1000000:.1f} μs per call")
    print(f"- Dict lookups add ~{(dict_lookup_time-sync_func_time-sync_time)/iterations*1000000:.1f} μs per call")
    print(f"- For 40,000 messages, async overhead alone is ~{(async_time-sync_func_time)*40:.0f}ms")
