...
async def handle_websocket_message_integrated(
    ...
    # Room adapters - delegate to room handler
        ...
        return await handle_room_messages(
            websocket, message, legacy_handler, room_state, broadcast_func
        )
...
class IntegratedAdapterSystem:
    ...
    def __init__(self, legacy_handler: Callable):
        ...
        logger.info(
            f"IntegratedAdapterSystem initialized with {len(self.enabled_adapters)} adapters"
        )
    ...
    async def handle_message(
        ...
        if action not in self.enabled_adapters:
            if self._adapter_only_mode:
                logger.warning(
                    f"ADAPTER-ONLY MODE: Action {action} not in enabled adapters"
                )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "phases": {
                "connection": all(
                    a in self.enabled_adapters
                    for a in ["ping", "client_ready", "ack", "sync_request"]
                ),
