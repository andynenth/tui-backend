...
class CreateRoomAdapterEvent:
    ...
    async def handle(
        ...
        return {
            "event": "room_created",
            "data": {"room_id": room_id, "host_name": player_name, "success": True},
        }
    ...
    async def handle(
        ...
        if room_id:
            ...
            event = PlayerLeftRoom(
                room_id=room_id, player_name=player_name, metadata=metadata
            )
...
class GetRoomStateAdapterEvent:
    ...
    async def handle(
        ...
        if room_state:
            return {"event": "room_state", "data": room_state}

        return {
            "event": "room_state",
            "data": {"slots": [], "host_name": None, "game_active": False},
        }
