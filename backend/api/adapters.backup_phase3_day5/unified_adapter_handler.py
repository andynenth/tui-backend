...
class UnifiedAdapterHandler:
    ...
    def __init__(self, room_manager=None, game_manager=None):
        ...
        self.adapter_map = {
            ...
            "client_ready": lambda: self._get_adapter(
                "client_ready",
                lambda: get_client_ready_adapter_event(self.room_manager),
            ),
            "ack": lambda: self._get_adapter("ack", get_ack_adapter_event),
            "sync_request": lambda: self._get_adapter(
                "sync_request",
                lambda: get_sync_request_adapter_event(
                    self.room_manager, self.game_manager
                ),
            ),
            # Room adapters
            "create_room": lambda: self._get_adapter(
                "create_room", lambda: get_create_room_adapter_event(self.room_manager)
            ),
            "join_room": lambda: self._get_adapter(
                "join_room", lambda: get_join_room_adapter_event(self.room_manager)
            ),
            "leave_room": lambda: self._get_adapter(
                "leave_room", lambda: get_leave_room_adapter_event(self.room_manager)
            ),
            "get_room_state": lambda: self._get_adapter(
                "get_room_state",
                lambda: get_room_state_adapter_event(self.room_manager),
            ),
            "add_bot": lambda: self._get_adapter(
                "add_bot", lambda: get_add_bot_adapter_event(self.room_manager)
            ),
            "remove_player": lambda: self._get_adapter(
                "remove_player",
                lambda: get_remove_player_adapter_event(self.room_manager),
            ),
            # Game adapters
            "start_game": lambda: self._get_adapter(
                "start_game",
                lambda: get_start_game_adapter_event(
                    self.room_manager, self.game_manager
                ),
            ),
            "declare": lambda: self._get_adapter(
                "declare", lambda: get_declare_adapter_event(self.game_manager)
            ),
            "play": lambda: self._get_adapter(
                "play", lambda: get_play_adapter_event(self.game_manager)
            ),
            "play_pieces": lambda: self._get_adapter(
                "play",  # Alias to play
                lambda: get_play_adapter_event(self.game_manager),
            ),
            "request_redeal": lambda: self._get_adapter(
                "request_redeal",
                lambda: get_request_redeal_adapter_event(self.game_manager),
            ),
            "accept_redeal": lambda: self._get_adapter(
                "accept_redeal",
                lambda: get_accept_redeal_adapter_event(self.game_manager),
            ),
            "decline_redeal": lambda: self._get_adapter(
                "decline_redeal",
                lambda: get_decline_redeal_adapter_event(self.game_manager),
            ),
            "player_ready": lambda: self._get_adapter(
                "player_ready",
                lambda: get_player_ready_adapter_event(self.room_manager),
            ),
            # Lobby adapters
            "request_room_list": lambda: self._get_adapter(
                "request_room_list",
                lambda: get_request_room_list_adapter_event(self.room_manager),
            ),
            "get_rooms": lambda: self._get_adapter(
                "get_rooms", lambda: get_rooms_adapter_event(self.room_manager)
            ),
        }
