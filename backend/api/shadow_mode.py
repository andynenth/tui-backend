...
class ShadowModeOrchestrator:
    ...
    async def handle_message(
        ...
        current_task = asyncio.create_task(
            self._run_handler(
                self.current_handler, websocket, message, room_state, "current"
            )
        )
        ...
        if self._should_shadow(message):
            shadow_task = asyncio.create_task(
                self._run_handler(
                    self.shadow_handler, websocket, message, room_state, "shadow"
                )
            )
        ...
        if shadow_task:
            try:
                ...
                shadow_result = await asyncio.wait_for(
                    shadow_task, timeout=self.config.timeout_ms / 1000
                )
    ...
    async def _run_handler(
        ...
        async def capture_broadcast(room_id: str, event: str, data: Dict[str, Any]):
            broadcasts.append({"room_id": room_id, "event": event, "data": data})
    ...
    async def _compare_results(
        ...
        if current_result["response"] and shadow_result["response"]:
            response_comparison = self.comparator.compare_responses(
                current_result["response"], shadow_result["response"]
            )
        ...
        broadcast_comparison = self.comparator.compare_broadcasts(
            current_result["broadcasts"], shadow_result["broadcasts"]
        )
            ...
            comparison.differences.append(
                {
                    ...
                    "details": {
                        ...
                        "ratio": shadow_result["duration_ms"]
        ...
        if self.config.comparison_storage_enabled:
            ...
            if len(self.comparisons) > self.config.max_stored_comparisons:
                self.comparisons = self.comparisons[
                    -self.config.max_stored_comparisons :
                ]

        # Log mismatches
        if not comparison.match and self.config.log_mismatches:
            logger.warning(
                f"Shadow mismatch for {comparison.request_id}: {comparison.differences}"
            )
...
class ShadowModeMetrics:
    ...
    def get_summary(self) -> Dict[str, Any]:
        """Get metrics summary"""
        match_rate = (
            (self.matches / self.shadowed_requests * 100)
            if self.shadowed_requests > 0
            else 0
        )
...
async def handle_with_shadow(
    websocket, message: Dict[str, Any], room_state: Optional[Dict[str, Any]] = None
) -> Dict[str, Any]:
