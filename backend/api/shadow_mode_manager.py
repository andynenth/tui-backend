...
class ShadowModeManager:
    ...
    def start_shadow_mode(
        ...
        session_info = {
            ...
            "config": {"sample_rate": sample_rate, "state": state.value},
        }
    ...
    def adjust_sample_rate(self, new_rate: float) -> Dict[str, Any]:
        ...
        return {"status": "adjusted", "old_rate": old_rate, "new_rate": new_rate}
    ...
    def analyze_mismatches(self, time_window_minutes: int = 60) -> Dict[str, Any]:
        """Analyze recent mismatches to identify patterns"""
        cutoff_time = datetime.now() - timedelta(minutes=time_window_minutes)

        recent_comparisons = [
            c for c in shadow_mode.comparisons if c.timestamp > cutoff_time
        ]
        ...
        for comp in recent_comparisons:
            if not comp.match:
                ...
                if action not in mismatch_by_action:
                    mismatch_by_action[action] = {"count": 0, "difference_types": {}}
                ...
                for diff in comp.differences:
                    ...
                    mismatch_by_action[action]["difference_types"][diff_type] = (
                        mismatch_by_action[action]["difference_types"].get(diff_type, 0)
                        + 1
                    )

        # Find most problematic actions
        problematic_actions = sorted(
            mismatch_by_action.items(), key=lambda x: x[1]["count"], reverse=True
        )[:5]
        ...
        return {
            ...
            "mismatch_rate": (
                f"{sum(1 for c in recent_comparisons if not c.match) / len(recent_comparisons) * 100:.1f}%"
        ...
        return [
            {
                ...
                "performance_ratio": (
                    c.shadow_duration_ms / c.current_duration_ms
                    if c.current_duration_ms > 0
                    else None
                ),
            }
...
class ShadowModeMonitor:
    ...
    async def start_monitoring(
        self, report_interval_seconds: int = 60, alert_threshold: float = 0.95
    ):
    ...
    async def _send_alert(self, alert: Dict[str, Any]):
        ...
        # Log alert
        self.manager._log_event({"event": "alert", "alert": alert})
...
def create_shadow_mode_cli():
    ...
    start_parser.add_argument(
        "--sample-rate", type=float, default=0.1, help="Sample rate (0.0-1.0)"
    )
    start_parser.add_argument(
        "--state",
        choices=["monitoring", "validating"],
        default="monitoring",
        help="Shadow mode state",
    )
    ...
    analyze_parser.add_argument(
        "--window", type=int, default=60, help="Time window in minutes"
    )
