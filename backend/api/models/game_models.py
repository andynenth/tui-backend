...
class Piece(BaseModel):
    ...
    value: int = Field(
        ...
        ge=1,
        le=14,
        description="Piece value (1-14, where 1=Ace, 11=Jack, 12=Queen, 13=King, 14=Red General)",
    )
    color: PieceColor = Field(..., description="Piece color")
    suit: PieceSuit = Field(..., description="Piece suit")

    class Config:
        schema_extra = {"example": {"value": 12, "color": "red", "suit": "hearts"}}
...
class Player(BaseModel):
    ...
    slot_id: Optional[int] = Field(
        None, ge=1, le=4, description="Player's slot position (1-4)"
    )
    ...
    class Config:
        schema_extra = {
            "example": {
                ...
                "status": "waiting",
                "players": [
                    {"name": "Alice", "id": "player_123", "is_bot": False, "slot_id": 1}
                ],
                "max_players": 4,
...
class GameState(BaseModel):
    ...
    declarations: Dict[str, int] = Field(
        {}, description="Player declarations (player_name -> pile_count)"
    )
    scores: Dict[str, int] = Field(
        {}, description="Player scores (player_name -> total_score)"
    )
