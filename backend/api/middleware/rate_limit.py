...
class RateLimiter:
    ...
    async def _cleanup_old_data(self):
        ...
        async with self._lock:
            ...
            expired_blocks = [
                client_id for client_id, blocked_until in self.blocked_until.items()
    ...
    def _get_client_identifier(self, request: Optional[Request] = None, 
    ...
    async def check_rate_limit(self, 
                             identifier: str,
                             rule: RateLimitRule,
                             route: Optional[str] = None) -> Tuple[bool, Optional[Dict[str, Any]]]:
        """
        ...
        async with self._lock:
            ...
            if request_count >= max_requests:
                ...
                if recent_requests > rule.requests * 2:
                    ...
                    self.blocked_until[identifier] = current_time + rule.block_duration_seconds
    ...
    async def check_websocket_rate_limit(self,
                                       ...
                                       client_id: str,
                                       event_type: str,
                                       rule: RateLimitRule) -> Tuple[bool, Optional[Dict[str, Any]]]:
        """
...
if CONFIG_AVAILABLE:
    ...
    DEFAULT_RULES = {
        ...
        "/api/health/detailed": RateLimitRule(requests=30, window_seconds=60),  # 30 req/min
        ...
        "ws_declare": RateLimitRule(requests=10, window_seconds=60),  # 10 declarations/min
        ...
        # Global fallback
        "global": RateLimitRule(requests=100, window_seconds=60, burst_multiplier=2.0),  # 100 req/min with 2x burst
    }
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if not rule:
            rule = self.rules.get("global", RateLimitRule(requests=100, window_seconds=60))
            
        # Get client identifier
        identifier = self.rate_limiter._get_client_identifier(request)
        
        # Check rate limit
        allowed, rate_info = await self.rate_limiter.check_rate_limit(identifier, rule, path)
        ...
        if not allowed:
            ...
            if ERROR_CODES_AVAILABLE:
                ...
                response_data = {
                    ...
                    "retry_after": rate_info.get("retry_after", 60),
                    "message": rate_info.get("reason", "Too many requests. Please try again later.")
                }
...
def create_rate_limiter(rules: Optional[Dict[str, RateLimitRule]] = None) -> RateLimitMiddleware:
    """
