...
WEBSOCKET_RATE_LIMITS = {
    ...
    "create_room": SimpleRateLimitRule(
...
class WebSocketRateLimiter:
    ...
    def __init__(self, rules: Dict[str, SimpleRateLimitRule] = None):
        ...
        self.connections: Dict[str, Dict[str, Any]] = defaultdict(
            lambda: {
    ...
    def _cleanup_old_requests(
        self, connection_data: Dict[str, Any], current_time: float
    ):
    ...
    def _is_blocked(
        self, connection_data: Dict[str, Any], event_type: str, current_time: float
    ) -> bool:
    ...
    async def check_rate_limit(
        self, websocket: WebSocket, event_type: str
    ) -> tuple[bool, str]:
        """
        ...
        if self._is_blocked(connection_data, event_type, current_time):
            remaining_block = (
                connection_data["blocked_until"][event_type] - current_time
            )
            return (
                False,
                f"Rate limit exceeded. Blocked for {int(remaining_block)} more seconds.",
            )
        ...
        if len(recent_requests) >= effective_limit:
            ...
            if rule.block_duration_seconds > 0:
                connection_data["blocked_until"][event_type] = (
            ...
            return (
                False,
                f"Rate limit exceeded: {len(recent_requests)}/{rule.requests} requests in {rule.window_seconds}s",
            )
    ...
    async def should_process_event(
        self, websocket: WebSocket, event_type: str, priority: EventPriority = None
    ) -> bool:
        """
...
async def check_websocket_rate_limit(
    websocket: WebSocket, event_type: str
) -> tuple[bool, str]:
    """
...
async def send_rate_limit_error(
    websocket: WebSocket, event_type: str, message: str = None
):
    """
