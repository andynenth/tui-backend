...
class PlayerActivityTracker:
    ...
    async def record_action(self, room_id: str, player_name: str, 
    ...
    async def get_diagnostics(self, limit: int = 20, 
                            hang_type: Optional[str] = None,
                            player_id: Optional[str] = None) -> List[HangDiagnostic]:
    ...
    async def _check_for_hang(self, activity: PlayerActivity, 
                             now: float) -> Optional[HangDiagnostic]:
    ...
    async def _create_diagnostic(self, activity: PlayerActivity, 
                               hang_type: str, duration: float) -> HangDiagnostic:
