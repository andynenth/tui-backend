...
class EventStore:
    ...
    def _init_database(self):
        ...
        conn.execute('CREATE INDEX IF NOT EXISTS idx_room_sequence ON game_events(room_id, sequence)')
        conn.execute('CREATE INDEX IF NOT EXISTS idx_room_timestamp ON game_events(room_id, timestamp)')
        conn.execute('CREATE INDEX IF NOT EXISTS idx_created_at ON game_events(created_at)')
    ...
    async def store_event(
        ...
        async with self._lock:
            ...
            logger.debug(f"Stored event: {event.event_type} for room {room_id} (seq: {sequence})")
            return event

    async def get_events_since(self, room_id: str, since_sequence: int) -> List[GameEvent]:
        """
        ...
        logger.info(f"Retrieved {len(events)} events for room {room_id} since sequence {since_sequence}")
        return events
    ...
    def _apply_event_to_state(self, state: Dict[str, Any], event: GameEvent) -> Dict[str, Any]:
        ...
        try:
            if event.event_type == 'phase_change':
                ...
                if player_name:
                    new_state['players'][player_name] = event.payload.get('player_data', {})
                ...
                new_state['round_number'] = event.payload.get('round_number', state['round_number'])
    ...
    async def cleanup_old_events(self, older_than_hours: int = 24) -> int:
        ...
        conn = sqlite3.connect(self.db_path)
        cursor = conn.execute('DELETE FROM game_events WHERE timestamp < ?', (cutoff_timestamp,))
        deleted_count = cursor.rowcount
        conn.commit()
        conn.close()
        
        logger.info(f"Cleaned up {deleted_count} events older than {older_than_hours} hours")
    ...
    async def get_event_stats(self) -> Dict[str, Any]:
        ...
        cursor = conn.execute('SELECT COUNT(*) FROM game_events WHERE timestamp > ?', (cutoff_time,))
