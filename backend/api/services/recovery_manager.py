...
class RecoveryManager:
    ...
    def __init__(self):
        ...
        self.procedure_attempts: Dict[str, List[float]] = {}  # procedure_name -> timestamps
    ...
    async def start_monitoring(self):
        ...
        if LOGGING_AVAILABLE and game_logger:
            game_logger.log_game_event("recovery_monitoring_started",
        ...
        try:
            ...
            if "websocket" in health.metrics:
                ...
                if isinstance(ws_metric.value, dict):
                    ...
                    if pending > 100:
                        await self.trigger_recovery("high_pending_messages", 
            ...
            if "memory" in health.metrics:
                ...
                if memory_metric.value > 90:
                    await self.trigger_recovery("memory_pressure", 
                                              context={"memory_usage": memory_metric.value})
            ...
            if memory_critical and cpu_critical:
                await self.trigger_recovery("system_overload", 
                                          context={"memory": health.metrics["memory"].value,
            ...
            if "database" in health.metrics:
                ...
                if db_metric.status == HealthStatus.CRITICAL:
                    await self.trigger_recovery("database_connection_loss",
    ...
    async def trigger_recovery(self, procedure_name: str, context: Dict[str, Any] = None) -> bool:
        """Trigger a specific recovery procedure"""
        if procedure_name not in self.recovery_procedures:
            if LOGGING_AVAILABLE and game_logger:
                game_logger.log_error(Exception(f"Unknown recovery procedure: {procedure_name}"),
                                    "trigger_recovery")
            return False
        ...
        if len(recent_attempts) >= procedure.max_attempts:
            if LOGGING_AVAILABLE and game_logger:
                game_logger.log_game_event("recovery_max_attempts_reached",
        ...
        if LOGGING_AVAILABLE and game_logger:
            game_logger.log_game_event("recovery_procedure_executed",
                                     procedure=procedure_name,
    ...
    async def _execute_recovery_action(self, procedure: RecoveryProcedure, context: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if action == RecoveryAction.CLEANUP_CONNECTIONS:
                ...
                if LOGGING_AVAILABLE and game_logger:
                    game_logger.log_error(Exception(f"Unknown recovery action: {action}"),
    ...
    async def _cleanup_stale_connections(self, context: Dict[str, Any]) -> bool:
        ...
        try:
                ...
                for ws in list(connections):
                    try:
                        ...
                        if hasattr(ws, 'ping'):
                            ...
                            if hasattr(ws, 'client_state') and hasattr(ws.client_state, 'name'):
            ...
            if LOGGING_AVAILABLE and game_logger:
                game_logger.log_websocket_event("system", "cleanup_stale_connections",
    ...
    async def _clear_pending_messages(self, context: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if LOGGING_AVAILABLE and game_logger:
                game_logger.log_websocket_event("system", "clear_pending_messages",
                                              cleared_count=cleared_count)
            
            print(f"🔧 RECOVERY_MANAGER: Cleared {cleared_count} expired pending messages")
            return True
    ...
    async def _restart_room(self, context: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            await broadcast(room_id, "room_restarted", {
                "reason": "State recovery",
                "players": players
            })
            
            if LOGGING_AVAILABLE and game_logger:
                game_logger.log_game_event("room_restarted", room_id=room_id, 
    ...
    async def _force_client_sync(self, context: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if room_id == "all":
                ...
                rooms_to_sync = [room_id] if room_id in socket_manager.room_connections else []
            ...
            for target_room in rooms_to_sync:
                ...
                for ws in connections:
                    try:
                        await socket_manager.request_client_sync(target_room, ws, "recovery")
                        sync_count += 1
                    except Exception:
                        continue
            
            if LOGGING_AVAILABLE and game_logger:
                game_logger.log_websocket_event("system", "force_client_sync",
    ...
    def _get_recent_attempts(self, procedure_name: str, time_window_seconds: int) -> List[float]:
    ...
    def get_recovery_status(self) -> Dict[str, Any]:
        ...
        recent_attempts = [attempt for attempt in self.recovery_history 
...
__all__ = ['RecoveryManager', 'RecoveryAction', 'RecoveryProcedure', 'RecoveryAttempt', 'recovery_manager']