...
class SimpleMaintenanceScheduler:
    ...
    def start(self):
        ...
        logger.info(f"Maintenance scheduler started. Daily cleanup at {cleanup_hour}:00")
    ...
    async def daily_cleanup(self):
        ...
        try:
            ...
            for days_ago in range(retention_days, retention_days + 7):  # Archive up to a week of missed days
                target_date = datetime.now() - timedelta(days=days_ago)
                await self.archive_day(target_date)
            
            # 2. Remove events older than retention period from main DB
            deleted_count = await self.event_store.cleanup_old_events(hours=retention_days * 24)
    ...
    def vacuum_database(self):
        ...
        try:
            ...
            logger.info(f"Database vacuumed. Size: {original_size:.1f}MB -> {new_size:.1f}MB (saved {saved:.1f}MB)")
        except Exception as e:
    ...
    def log_maintenance_summary(self, events_deleted: int, archives_deleted: int, duration: float):
        """Log a summary of maintenance activities."""
        db_size = os.path.getsize(self.db_path) / 1024 / 1024  # MB
        archive_files = glob.glob(os.path.join(self.archives_dir, '*.gz'))
        archive_size = sum(os.path.getsize(f) for f in archive_files) / 1024 / 1024  # MB
    ...
    def get_status(self) -> dict:
        ...
        archive_files = glob.glob(os.path.join(self.archives_dir, '*.gz'))
        archive_size = sum(os.path.getsize(f) for f in archive_files) / 1024 / 1024 if archive_files else 0
        ...
        if self.scheduler.running:
            ...
            if job:
                next_cleanup = job.next_run_time.isoformat() if job.next_run_time else None
