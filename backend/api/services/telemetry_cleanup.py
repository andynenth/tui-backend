...
class TelemetryCleanupService:
    ...
    async def _cleanup_events(self, cursor) -> int:
        ...
        for event_type, retention_days in self.retention_policies.items():
            ...
            if cleaned > 0:
                logger.debug(
                    f"Cleaned {cleaned} {event_type} events older than {retention_days} days"
                )
        ...
        if max_cleaned > 0:
            logger.info(
                f"Cleaned {max_cleaned} events older than {self.max_retention_days} days (max retention)"
            )
    ...
    def sanitize_telemetry_data(data: Dict) -> Dict:
        ...
        if "clientIp" in sanitized:
            sanitized["clientIp"] = TelemetryCleanupService._hash_ip(
                sanitized["clientIp"]
            )
        ...
        if "stack" in sanitized:
            sanitized["stack"] = TelemetryCleanupService._sanitize_stack_trace(
                sanitized["stack"]
            )

        # Sanitize user agent (keep browser/version info only)
        if "userAgent" in sanitized:
            sanitized["userAgent"] = TelemetryCleanupService._sanitize_user_agent(
                sanitized["userAgent"]
            )
    ...
    def _sanitize_user_agent(user_agent: str) -> str:
        ...
        return (
            f"{browser}/{version} ({platform})"
            if version
            else f"{browser} ({platform})"
        )
...
class TelemetryPrivacyService:
    ...
    def export_user_data(session_id: str, db_path: Path) -> Dict:
        ...
        try:
            ...
            return {
                ...
                "session_data": dict(
                    zip([col[0] for col in cursor.description], session_data)
                )
                if session_data
                else None,
                "events": [
                    {"timestamp": event[0], "type": event[1], "data": event[2]}
                    for event in events
                ],
    ...
    def delete_user_data(session_id: str, db_path: Path) -> bool:
        ...
        try:
            ...
            # Delete events
            cursor.execute(
                "DELETE FROM telemetry_events WHERE session_id = ?", (session_id,)
            )
            events_deleted = cursor.rowcount

            # Delete session
            cursor.execute(
                "DELETE FROM telemetry_sessions WHERE session_id = ?", (session_id,)
            )
            ...
            logger.info(
                f"Deleted user data: {events_deleted} events, {session_deleted} session"
            )
            return True
