...
class ReconnectionService:
    ...
    async def handle_disconnect(
        ...
        async with self._uow:
            ...
            if not player:
                raise ApplicationException(
                    f"Player {player_name} not in room", code="PLAYER_NOT_FOUND"
                )
        ...
        async with self._uow:
            ...
            for conn in connections:
                status = {
                    ...
                    "last_activity": (
                        conn.last_activity.isoformat() if conn.last_activity else None
                    ),
                }
        ...
        async with self._uow:
            ...
            for player in room.players[
                ...
                if (
                    ...
                    and (cutoff_time - player.disconnect_time).total_seconds()
                    > timeout_minutes * 60
                ):
