...
class GameApplicationService(ApplicationService):
    ...
    def __init__(
        ...
        self._request_redeal_use_case = RequestRedealUseCase(
            unit_of_work, event_publisher, metrics
        )
        self._accept_redeal_use_case = AcceptRedealUseCase(
            unit_of_work, event_publisher, metrics
        )
        self._decline_redeal_use_case = DeclineRedealUseCase(
            unit_of_work, event_publisher, metrics
        )
        self._handle_redeal_use_case = HandleRedealDecisionUseCase(
            unit_of_work, event_publisher, metrics
        )
        self._mark_ready_use_case = MarkPlayerReadyUseCase(
            unit_of_work, event_publisher, metrics
        )
        self._leave_game_use_case = LeaveGameUseCase(
            unit_of_work, event_publisher, bot_service, metrics
        )

    async def start_game_with_bots(
        self, room_id: str, requesting_player_id: str, user_id: Optional[str] = None
    ) -> StartGameResponse:
        """
        ...
        try:
            ...
            if response.success:
                ...
                await self._handle_bot_redeal_responses(
                    response.game_id, response.weak_hands_detected
                )
            ...
            self._logger.error(f"Failed to start game with bots: {e}", exception=e)
            raise ApplicationException(
                f"Failed to start game: {str(e)}", code="GAME_START_FAILED"
            )
    ...
    async def complete_game_turn(
        ...
        play_request = PlayRequest(
            game_id=game_id, player_id=player_id, pieces=piece_infos, user_id=user_id
        )
    ...
    async def handle_redeal_timeout(
        self, game_id: str, redeal_id: str
    ) -> Dict[str, Any]:
        """
        ...
        request = HandleRedealDecisionRequest(
            game_id=game_id, redeal_id=redeal_id, force_decision=True
        )
    ...
    async def get_game_statistics(self, game_id: str) -> Dict[str, Any]:
        ...
        async with self._uow:
            ...
            if not game:
                raise ApplicationException(
                    f"Game {game_id} not found", code="GAME_NOT_FOUND"
                )

            # Calculate statistics
            total_turns = sum(getattr(p, "turns_played", 0) for p in game.players)
            total_pieces_played = sum(
                getattr(p, "pieces_played", 0) for p in game.players
            )
    ...
    async def _handle_bot_redeal_responses(
        self, game_id: str, weak_hands: List[str]
    ) -> None:
            ...
            for player in game.players:
                if player.is_bot and not hasattr(player, "declaration"):
                    ...
                    declare_request = DeclareRequest(
                        game_id=game_id, player_id=player.id, pile_count=pile_count
                    )
    ...
    def _get_current_leader(self, game) -> Dict[str, Any]:
        ...
        return {
            ...
            "score": leader.score,
            "lead_margin": (
                leader.score - sorted([p.score for p in game.players], reverse=True)[1]
    ...
    def _calculate_declaration_accuracy(self, player) -> float:
        ...
        accurate = sum(
            1 for d in player.declaration_history if d["declared"] == d["actual"]
        )
    ...
    def _create_game_state_for_bot(self, game) -> Dict[str, Any]:
        ...
        return {
            ...
            "player_hands": {p.id: len(p.hand) for p in game.players},
