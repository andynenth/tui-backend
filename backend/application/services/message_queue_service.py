        ...
        async with self._uow:
            ...
            if queue.add_message(message):
                ...
                overflow_event = MessageQueueOverflow(
                    ...
                    player_name=player_name,
                    dropped_count=1,
                    retained_critical_count=sum(1 for msg in queue.messages if msg.is_critical),
        ...
        async with self._uow:
            ...
            if messages:
                event = QueuedMessagesDelivered(
                    ...
                    ).total_seconds() if queue.messages else 0,
                    critical_message_count=sum(1 for msg in queue.messages if msg.is_critical)
                )
                await self._event_publisher.publish_batch([event])
            ...
            for player in room.players:
                ...
                if queue:
                    player_stats = {
                        ...
                        "queue_size": len(queue.messages),
                        "critical_messages": sum(1 for msg in queue.messages if msg.is_critical),
                        "oldest_message_age": (
                            datetime.utcnow() - queue.messages[0].timestamp
                        ).total_seconds() if queue.messages else 0
                    }
            ...
            for player in room.players:
                ...
                if queue:
                    ...
                    queue.messages = [
                        ...
                        if (cutoff_time - msg.timestamp).total_seconds() < max_age_minutes * 60
                    ]
