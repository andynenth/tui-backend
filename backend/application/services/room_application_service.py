...
class RoomApplicationService(ApplicationService):
    ...
    def __init__(
        ...
        self._create_room_use_case = CreateRoomUseCase(unit_of_work, event_publisher, metrics)
        self._join_room_use_case = JoinRoomUseCase(unit_of_work, event_publisher, metrics)
        self._leave_room_use_case = LeaveRoomUseCase(unit_of_work, event_publisher, metrics)
        self._get_room_state_use_case = GetRoomStateUseCase(unit_of_work, metrics)
        self._add_bot_use_case = AddBotUseCase(unit_of_work, event_publisher, bot_service, metrics)
        self._remove_player_use_case = RemovePlayerUseCase(unit_of_work, event_publisher, metrics)
    ...
    async def create_room_and_fill_bots(
        ...
        try:
            ...
            self._logger.error(
                f"Failed to create room with bots: {e}",
                exception=e
            )
            raise
    ...
    async def cleanup_inactive_rooms(
        ...
        return {
            "rooms_cleaned": len(cleaned_rooms),
    ...
    async def get_room_analytics(
        ...
        async with self._uow:
            ...
            connected_players = sum(
                1 for slot in room.slots
                if slot and getattr(slot, 'is_connected', True)
            )
            
            # Room age
            from datetime import datetime
            room_age_minutes = (datetime.utcnow() - room.created_at).total_seconds() / 60
    ...
    async def balance_room_with_bots(
        ...
        async with self._uow:
            ...
            if room.current_game:
                raise ApplicationException(
                    "Cannot balance room during active game",
            ...
            # Remove bots if needed
                ...
                for i in range(min(bots_to_remove, len(bot_slots))):
                    ...
                    try:
                        ...
                        remove_response = await self._remove_player_use_case.execute(remove_request)
