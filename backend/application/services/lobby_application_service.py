...
class LobbyApplicationService(ApplicationService):
    ...
    async def get_recommended_rooms(
        ...
        list_request = GetRoomListRequest(
            player_id=player_id, include_full=False, include_in_game=False, page_size=50
        )
        ...
        for room_summary in list_response.rooms:
            score = await self._calculate_room_match_score(
                room_summary, player_stats, preferences
            )
            ...
            try:
                details_response = await self._get_room_details_use_case.execute(
                    details_request
                )
    ...
    async def get_lobby_statistics(self) -> Dict[str, Any]:
        ...
        async with self._uow:
            ...
            stats = {
                ...
                "average_players_per_room": (
                    round(total_players / total_rooms, 2) if total_rooms > 0 else 0
                ),
    ...
    async def get_trending_rooms(
        ...
        async with self._uow:
            ...
            for room in all_rooms:
                ...
                # Human player bonus
                human_count = sum(1 for slot in room.slots if slot and not slot.is_bot)
                    ...
                    trending_rooms.append(
                        {
                            ...
                            "created_minutes_ago": round(
                                (datetime.utcnow() - room.created_at).total_seconds()
                                / 60,
                                1,
                            ),
                        }
    ...
    async def _calculate_room_match_score(
        ...
        if hasattr(room_summary, "created_at"):
            age_minutes = (
                datetime.utcnow() - room_summary.created_at
            ).total_seconds() / 60
            if age_minutes < 2:
                score -= 5
    ...
    def _get_match_reasons(
        self, score: float, preferences: Dict[str, Any]
    ) -> List[str]:
    ...
    def _analyze_popular_settings(self, rooms: List[Any]) -> Dict[str, Any]:
        ...
        return {
            "popular_win_conditions": sorted(
                win_conditions.items(), key=lambda x: x[1], reverse=True
            )[:3],
            "popular_room_sizes": sorted(
                max_players_dist.items(), key=lambda x: x[1], reverse=True
            )[:3],
        }

    def _analyze_peak_times(self, rooms: List[Any]) -> Dict[str, int]:
