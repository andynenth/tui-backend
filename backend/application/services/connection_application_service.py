...
class ConnectionApplicationService(ApplicationService):
    ...
    def __init__(
        ...
        self._mark_ready_use_case = MarkClientReadyUseCase(unit_of_work, event_publisher, metrics)
    ...
    async def handle_client_connect(
        ...
        try:
            ...
            if self._metrics:
                self._metrics.increment(
                    ...
                    tags={
                        ...
                        "client_version": client_info.get("version", "unknown") if client_info else "unknown"
                    }
                )
            ...
            self._logger.error(
                f"Failed to handle client connect: {e}",
                exception=e
            )
    ...
    async def get_connection_analytics(
        ...
        if player_id:
            ...
            analytics = {
                ...
                "connection_metrics": {
                    "total_connections": metrics.get("disconnections", 0) + (1 if is_connected else 0),
                    "average_latency_ms": metrics.get("average_latency", 0),
                    "ping_count": metrics.get("ping_count", 0),
                    "last_session_duration_seconds": metrics.get("last_session_duration", 0)
                },
            ...
            return {
                ...
                "average_latency_ms": round(total_latency / total_pings, 2) if total_pings > 0 else 0,
