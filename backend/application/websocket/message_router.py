...
from infrastructure.monitoring.metrics_integration import (
    MetricsContext,
    metrics_collector,
)
...
class MessageRouter:
    ...
    async def route_message(
        self, websocket: WebSocket, message: Dict[str, Any], room_id: str
            ...
            async with MetricsContext(event) as ctx:
                ...
                if response:
                    await metrics_collector.record_message_sent(
                        response.get("event", event), response
                    )
    ...
    async def _route_to_use_case(
        self, websocket: WebSocket, message: Dict[str, Any], room_id: str, event: str
    ) -> Optional[Dict[str, Any]]:
        """
        ...
        if event == "client_ready" and room_id != "lobby":
            ...
            if data_player_name:
                ...
                if websocket_id and (
                    not connection or player_name != connection.player_name
                ):
                    ...
                    await connection_manager.register_player(
                        room_id, player_name, websocket_id, correct_player_id
                    )
                    logger.info(
                        f"Updated player registration: {player_name} for room {room_id} with player_id {correct_player_id}"
                    )
    ...
    async def _route_to_adapter(
        ...
        response = await self._adapter_wrapper.try_handle_with_adapter(
            websocket, message, room_id, room_state
        )
            ...
            async with uow:
                ...
                if room:
                    ...
                    return {
                        ...
                        "current_round": (
                            room.current_round if hasattr(room, "current_round") else 1
                        ),
                        "max_players": room.max_slots,
                    }
    ...
    async def _check_message_queue(self, websocket: WebSocket, room_id: str) -> None:
        """
    ...
    async def handle_room_validation(self, websocket: WebSocket, room_id: str) -> bool:
        """
