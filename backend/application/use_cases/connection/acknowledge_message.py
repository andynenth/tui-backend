...
class AcknowledgeMessageUseCase(
    UseCase[AcknowledgeMessageRequest, AcknowledgeMessageResponse]
):
    """
    ...
    async def execute(
        self, request: AcknowledgeMessageRequest
    ) -> AcknowledgeMessageResponse:
        """
        ...
        if request.success:
            ...
            retry_info = self._retry_queue.get(
                ack_key, {"attempts": 0, "first_failure": datetime.utcnow()}
            )
