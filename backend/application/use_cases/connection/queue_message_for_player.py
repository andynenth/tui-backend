...
class QueueMessageForPlayerUseCase:
    ...
    async def execute(
        self, request: QueueMessageForPlayerRequest
    ) -> QueueMessageForPlayerResponse:
        ...
        try:
            async with self._uow:
                ...
                if not queue:
                    ...
                    return QueueMessageForPlayerResponse(
                        success=True, queued=False, queue_size=0
                    )

                # Add the message
                initial_size = queue.size()
                queue.add_message(
                    request.event_type, request.event_data, request.is_critical
                )

                # Check if overflow occurred
                message_dropped = (
                    queue.size() == initial_size and initial_size >= queue.max_size
                )
            ...
            raise ApplicationException(
                f"Failed to queue message: {str(e)}", code="QUEUE_MESSAGE_FAILED"
            )
