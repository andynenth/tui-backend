...
from application.dto.connection import HandlePlayerReconnectRequest, HandlePlayerReconnectResponse
    ...
    async def execute(self, request: HandlePlayerReconnectRequest) -> HandlePlayerReconnectResponse:
        ...
        try:
            async with self._uow:
                ...
                if self._metrics:
                    ...
                    if queue and oldest_age is not None:
                        self._metrics.gauge(
                            "message_queue.oldest_message_age",
                            oldest_age
                        )
            ...
            raise ApplicationException(
                f"Failed to handle player reconnect: {str(e)}",
