        ...
        async with self._uow:
            ...
            if not room:
                raise ResourceNotFoundException(
                    "Room", request.room_code or request.room_id
                )
            ...
            room_details = RoomDetails(
                ...
                is_joinable=can_join
                and not PropertyMapper.get_room_attr(room, "settings.is_private"),
            ...
            if slot:
                players.append(
                    PlayerInfo(
                        ...
                        is_bot=slot.is_bot,
                        is_host=PropertyMapper.generate_player_id(room.room_id, i)
    ...
    async def _get_player_stats(self, room) -> Dict[str, Dict[str, Any]]:
        ...
        for slot in room.slots:
            if slot and not slot.is_bot:
                player_stats = await self._uow.player_stats.get_stats(
                    PropertyMapper.generate_player_id(room.room_id, i)
                )
                stats[PropertyMapper.generate_player_id(room.room_id, i)] = {
                    ...
                    "favorite_play_style": player_stats.get(
                        "favorite_play_style", "balanced"
                    ),
                }
