...
class ScheduleBotActionUseCase:
    ...
    async def execute(
        self, request: ScheduleBotActionRequest
    ) -> ScheduleBotActionResponse:
        ...
        try:
            ...
            if self._metrics:
                ...
                self._metrics.histogram(
                    "bot.action.delay", delay, tags={"action_type": request.action_type}
                )
            ...
            return ScheduleBotActionResponse(
                success=True, scheduled=True, delay_seconds=delay
            )
            ...
            raise ApplicationException(
                f"Failed to schedule bot action: {str(e)}", code="BOT_SCHEDULE_FAILED"
            )
