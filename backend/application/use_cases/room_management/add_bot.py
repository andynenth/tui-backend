...
class AddBotUseCase(UseCase[AddBotRequest, AddBotResponse]):
    ...
    async def execute(self, request: AddBotRequest) -> AddBotResponse:
        ...
        async with self._uow:
            ...
            if room.host_id != request.requesting_player_id:
                raise AuthorizationException("add bot", f"room {request.room_id}")
            ...
            if room.game:
                raise ConflictException(
                    "add bot", "Cannot add bot while game is in progress"
                )
            ...
            try:
                seat_position = room.add_player(
                    bot_name, is_bot=True, slot=request.seat_position
                )
            except ValueError as e:
                # Convert domain exception to application exception
                if "occupied" in str(e).lower():
                    raise ConflictException(
                        "add bot", f"Slot {request.seat_position} is already occupied"
                    )
                else:
            ...
            # Create response
            response = AddBotResponse(bot_info=bot_info, room_info=room_info)
    ...
    def _generate_bot_name(self, room) -> str:
        ...
        # Find available name
        available_names = [
            name for name in self.BOT_NAMES if name not in existing_names
        ]
