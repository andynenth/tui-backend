...
class CreateRoomUseCase(UseCase[CreateRoomRequest, CreateRoomResponse]):
    ...
    async def execute(self, request: CreateRoomRequest) -> CreateRoomResponse:
        ...
        async with self._uow:
            ...
            if existing_room:
                raise ConflictException(
                    "create room", f"Player is already in room {existing_room.room_id}"
                )
            ...
            host_player = Player(name=request.host_player_name, is_bot=False)
            ...
            for i, player in enumerate(room.slots):
                if player:
                    logger.info(
                        f"[USE_CASE_DEBUG]   Slot {i}: {player.name} (bot={player.is_bot})"
                    )
                else:
        ...
        logger.info(
            f"[CREATE_ROOM_DEBUG] Publishing RoomCreated event for room {room_info.room_id}"
        )
        logger.info(
            f"[CREATE_ROOM_DEBUG] Event publisher type: {self._event_publisher.__class__.__name__}"
        )
