...
from application.dto.game import (
    HandleRedealDecisionRequest,
    HandleRedealDecisionResponse,
)
...
class HandleRedealDecisionUseCase(
    UseCase[HandleRedealDecisionRequest, HandleRedealDecisionResponse]
):
    """
    ...
    async def execute(
        self, request: HandleRedealDecisionRequest
    ) -> HandleRedealDecisionResponse:
        """
        ...
        async with self._uow:
            ...
            if not hasattr(game, "active_redeal_id") or not game.active_redeal_id:
                raise ConflictException(
                    "handle redeal decision", "No active redeal vote"
                )
            ...
            for player in game.players:
                if (
                    ...
                    not in game.redeal_votes
                ):
                    missing_votes.append(
                        PropertyMapper.get_player_attr(player, "id", room.room_id, i)
                    )
            ...
            if redeal_approved:
                ...
                executed_event = RedealExecuted(
                    ...
                    new_starting_player_id=PropertyMapper.get_safe(
