    ...
    async def execute(self, request: PlayRequest) -> PlayResponse:
        ...
        if not request.pieces:
            raise ValidationException({
                "pieces": "Must play at least one piece"
            })
        
        if len(request.pieces) > 6:
            raise ValidationException({
                "pieces": "Cannot play more than 6 pieces"
            })
        ...
        async with self._uow:
            ...
            if game.phase != GamePhase.TURN:
                raise ConflictException(
                    "play pieces",
                    f"Game is in {game.phase} phase, not turn"
                )
            ...
            if not GameRules.is_valid_play(pieces, play_type):
                raise ValidationException({
                    "pieces": f"Invalid {play_type} play"
                })
            ...
            if turn_complete:
                turn_event = TurnCompleted(
                    ...
                    pieces_won=sum(len(play['pieces']) for play in game.current_turn_plays),
            ...
            if game_complete:
                ...
                game_event = GameEnded(
                    ...
                    end_condition="score" if winner.score >= game.win_condition_value else "rounds"
                )
            ...
            response = PlayResponse(
                ...
                scores={p.id: p.score for p in game.players} if round_complete else None
            )
