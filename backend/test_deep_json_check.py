    ...
    if isinstance(obj, dict):
        for key, value in obj.items():
            ...
            if hasattr(value, "__dict__") and not isinstance(
                ...
                issues.append(
                    f"Non-serializable object at {current_path}: {type(value)} - {value}"
                )
            else:
        ...
        for i, item in enumerate(obj):
            ...
            if hasattr(item, "__dict__") and not isinstance(
                item, (str, int, float, bool, list, dict, type(None))
            ):
                issues.append(
                    f"Non-serializable object at {current_path}: {type(item)} - {item}"
                )
            else:
