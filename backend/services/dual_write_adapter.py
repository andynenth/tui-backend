...
class DualWriteAdapter:
    ...
    def __init__(self):
        ...
        self.dual_write_enabled = (
            os.getenv("DB_DUAL_WRITE_MODE", "false").lower() == "true"
        )
        ...
        try:
            ...
            for event in reversed(events):  # Process in chronological order
                if event.event_type == "hands_dealt":
                    ...
                    round_data["starter_reason"] = event.payload.get(
                        "starter_reason", "default"
                    )
    ...
    async def get_play_history(
        ...
        if self.v2_primary and self.dual_write_enabled:
            ...
            try:
                ...
                if summary:
                    rounds = await self.v2_store.get_round_snapshots(
                        room_id, round_numbers
                    )
                    return {
                        "room_id": room_id,
                        "summary": summary,
    ...
    async def migrate_historical_data(
        self, batch_size: int = 100, max_rooms: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        ...
        """
        stats = {"rooms_processed": 0, "rounds_migrated": 0, "errors": 0}
        ...
        for event in events:
            if event.event_type == "game_started":
                ...
                if round_data and current_round > 0:
                    # Save previous round
                    await self.v2_store.store_round_snapshot(
                        room_id, current_round, round_data
                    )
