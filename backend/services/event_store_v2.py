...
class EventStoreV2:
    ...
    async def store_game_started(self, room_id: str, players: List[Dict[str, Any]]) -> None:
    ...
    async def store_round_snapshot(
        ...
        try:
            ...
            conn.commit()
            logger.info(f"Stored round {round_number} snapshot for room {room_id}")
        finally:
            conn.close()
    ...
    async def store_game_completed(
        ...
        try:
            # Get game start time
            cursor = conn.execute(
                "SELECT started_at FROM game_summaries WHERE room_id = ?",
                (room_id,)
            )
            row = cursor.fetchone()
        ...
        try:
            ...
            for row in cursor.fetchall():
                rounds.append({
                    ...
                    'created_at': row[9],
                    'total_turns': row[11] if len(row) > 11 else len(json.loads(row[6]))
                })
    ...
    async def cleanup_old_data(self, days_to_keep: int = 30) -> int:
        ...
        try:
            ...
            conn.execute(f"DELETE FROM turn_details WHERE room_id IN ({placeholders})", rooms_to_delete)
            conn.execute(f"DELETE FROM round_snapshots WHERE room_id IN ({placeholders})", rooms_to_delete)
            conn.execute(f"DELETE FROM game_events_v2 WHERE room_id IN ({placeholders})", rooms_to_delete)
            conn.execute(f"DELETE FROM game_summaries WHERE room_id IN ({placeholders})", rooms_to_delete)
