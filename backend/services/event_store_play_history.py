...
class EventStorePlayHistoryExtractor:
    ...
    def _find_round_boundaries(self, room_id: str) -> List[tuple]:
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            for i, scoring_seq in enumerate(scoring_sequences):
                ...
                start_seq = max([p for p in prep_sequences if p < scoring_seq], default=0)
                boundaries.append((start_seq, scoring_seq))
            
            return boundaries
    
    def _extract_round_data(self, room_id: str, round_num: int, start_seq: int, end_seq: int) -> Optional[Dict[str, Any]]:
                    ...
                    if phase == 'preparation':
                        ...
                        for player_name, player_info in players.items():
                            if player_info.get('hand_size', 0) > 0:
                                ...
                                initial_hands[player_name] = player_info.get('hand_size', 8)
    ...
    def _extract_current_round(self, room_id: str, completed_boundaries: List[tuple]) -> Optional[Dict[str, Any]]:
        """Extract data for the current round in progress"""
        with sqlite3.connect(self.db_path) as conn:
            # Find the last sequence from completed rounds
            last_completed_seq = max([end for _, end in completed_boundaries], default=0)
            ...
            """, (room_id, last_completed_seq)).fetchone()[0]
            
            if current_events > 0:
                # Extract current round data
                round_num = len(completed_boundaries) + 1
                return self._extract_round_data(room_id, round_num, last_completed_seq + 1, 999999)
