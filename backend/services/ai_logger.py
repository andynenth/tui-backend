...
class AILogger:
    ...
    def log_round_start(self, round_number: int, game):
        ...
        if self.should_log("summary"):
            ...
            round_hands = {}
            for player in game.players:
                round_hands[player.name] = [f"{p.name}_{p.color}" for p in player.hand]
    ...
    def _generate_game_summary(self) -> Dict:
        ...
        for player, stats in player_stats.items():
            if stats["total_declared"] > 0:
                stats["declaration_accuracy"] = (
                    stats["total_captured"] / stats["total_declared"]
                )
            else:
        ...
        return {
            ...
            "bugs_detected": len(
                [e for e in self.events if e.get("event") == "bug_detected"]
            ),
