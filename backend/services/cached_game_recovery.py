...
class CachedGameRecoveryService:
    ...
    async def _restore_preparation_phase(
        self, game: Game, state: Dict[str, Any]
    ) -> None:
    ...
    async def _restore_declaration_phase(
        self, game: Game, state: Dict[str, Any]
    ) -> None:
    ...
    async def save_game_state(
        self, room_id: str, game: Game, phase_data: Optional[Dict[str, Any]] = None
    ) -> None:
        """
        ...
        state = {
            ...
            "players": [
                {"player_name": p.name, "player_type": "ai" if p.is_bot else "human"}
        ...
        if game.current_phase == "DECLARATION":
            ...
            state["phase_data"]["current_player_index"] = getattr(
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        cache_hit_rate = (
            self._cache_hits / self._recovery_count if self._recovery_count > 0 else 0.0
        )
