...
class AIBugDetector:
    ...
    def check_declaration_bugs(
        self, player_name: str, declaration_data: Dict
    ) -> List[BugReport]:
        ...
        openers = sum(
            1
            for play in declaration_data.get("play_list", [])
                ...
                bugs.append(
                    BugReport(
                        ...
                        context={
                            ...
                            "pile_room": pile_room,
                            "position": position,
                            "previous_declarations": declaration_data.get(
    ...
    def check_turn_play_bugs(
        self, player_name: str, turn_data: Dict
    ) -> List[BugReport]:
        ...
        if selected_play:
            ...
            if (
                at_target
                and has_opener
                and pieces_remaining > 2
                and play_type != "INVALID"
            ):
                bugs.append(
                    BugReport(
            ...
            if (
                ...
                and len(selected_play) != required_count
            ):
                bugs.append(
                    BugReport(
    ...
    def check_combo_management_bugs(
        self, player_name: str, hand_data: List[str], play_data: Dict
    ) -> List[BugReport]:
    ...
    def analyze_game_performance(self, game_summary: Dict) -> Dict[str, Any]:
        ...
        for player, stats in player_stats.items():
            ...
            if total_declared > 0:
                ...
                # Check for red flags
                if accuracy < 40:
                    performance["red_flags"].append(
                        f"{player}: Very low accuracy ({accuracy:.1f}%)"
                    )
                elif accuracy > 90:
                    performance["red_flags"].append(
                        f"{player}: Too conservative ({accuracy:.1f}%)"
                    )
