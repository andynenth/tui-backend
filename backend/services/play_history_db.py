...
class PlayHistoryDatabaseService:
    ...
    async def get_play_history(self, room_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            logger.info(
                f"Getting play history for room {room_id} from database at {self.db_path}"
            )
            ...
            cursor = conn.cursor()
            cursor.execute("SELECT * FROM game_summaries WHERE room_id = ?", (room_id,))
            ...
            conn.close()
            logger.info(
                f"Returning play history for room {room_id} with {len(round_snapshots)} rounds"
            )
            return result
    ...
    def _build_response(
        self, room_id: str, summary: sqlite3.Row, round_snapshots: List[sqlite3.Row]
    ) -> Dict[str, Any]:
        """Build the response in the format expected by frontend transformer."""

        # Parse players from summary (using correct column name)
        players_data = (
            json.loads(summary["player_names"]) if summary["player_names"] else []
        )
        players = []
        ...
        for i, player_data in enumerate(players_data):
            ...
            players.append({"name": player_name, "type": player_type, "position": i})
        ...
        if round_snapshots:
            ...
            cumulative_scores = (
                json.loads(last_round["cumulative_scores"])
                if last_round["cumulative_scores"]
                else {}
            )
        ...
        return {
            ...
            "start_time": datetime.fromtimestamp(summary["started_at"]).isoformat()
            if summary["started_at"]
            else datetime.now().isoformat(),
            "end_time": datetime.fromtimestamp(summary["completed_at"]).isoformat()
    ...
    def _build_round(
        self, snapshot: sqlite3.Row, players: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """Build round data from snapshot in frontend format."""

        # Parse JSON fields
        initial_hands = (
            json.loads(snapshot["initial_hands"]) if snapshot["initial_hands"] else {}
        )
        declarations_data = (
            json.loads(snapshot["declarations"]) if snapshot["declarations"] else {}
        )
        turn_sequence = (
            json.loads(snapshot["turn_sequence"]) if snapshot["turn_sequence"] else []
        )
        round_scores = (
            json.loads(snapshot["round_scores"]) if snapshot["round_scores"] else {}
        )
        cumulative_scores = (
            json.loads(snapshot["cumulative_scores"])
            if snapshot["cumulative_scores"]
            else {}
        )
        ...
        for player_name in [p["name"] for p in players]:
            ...
            if isinstance(player_round_score, dict):
                declared = player_round_score.get(
                ...
                final_score = (
                    player_round_score
                    if isinstance(player_round_score, (int, float))
                    else 0
                )
                bonus = 0
            ...
            base_score = (
                player_round_score.get("base_score", abs(declared - actual))
                if isinstance(player_round_score, dict)
                else abs(declared - actual)
            )
        ...
        return {
            ...
            "initial_state": {
                "starter": {"player_name": snapshot["starter_player"] or "Unknown"}
            },
    ...
    def _build_turn(
        self, turn_data: Dict[str, Any], player_hands: Dict[str, List[Any]]
            ...
            for played_piece in pieces:
                ...
                for i, hand_piece in enumerate(hand_after):
                    if isinstance(hand_piece, dict) and isinstance(played_piece, dict):
                        if hand_piece.get("kind") == played_piece.get(
                            "kind"
                        ) and hand_piece.get("point") == played_piece.get("point"):
                            hand_after.pop(i)
                            break
