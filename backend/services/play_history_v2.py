...
class PlayHistoryV2Service:
        ...
        for snapshot in snapshots:
            try:
                if format == "compact":
                    ...
                    round_history = self._build_full_round(
                        snapshot, players, include_ai_analysis
                    )
                ...
                if round_history:
                    ...
                    logger.warning(
                        f"Failed to build round history for snapshot: {snapshot.get('round_number', 'unknown')}"
                    )
    ...
    def _build_full_round(
        ...
        initial_state = InitialState(
            starter=StarterInfo(
                ...
                highest_card=(
                    "GENERAL_RED(14)"
                    if snapshot.get("starter_reason") == "has_general_red"
                    else None
                ),
            ),
        ...
        for player_name, hand in snapshot["initial_hands"].items():
            hands_dealt[player_name] = [
                PieceInfo(kind=p["kind"], point=p["point"]) for p in hand
            ]

        # Build declaration phase
        declaration_phase = self._build_declaration_info(
            snapshot["declarations"], players
        )

        # Build turn history
        turn_history = self._build_turn_history(
            snapshot["turn_sequence"], snapshot["declarations"], include_ai_analysis
        )

        # Build round summary
        round_summary = self._build_round_summary(snapshot, players)
    ...
    def _build_compact_round(
        self, snapshot: Dict[str, Any], players: Dict[str, PlayerInfo]
    ) -> RoundHistory:
    ...
    def _build_declaration_info(
        self, declarations: Dict[str, int], players: Dict[str, PlayerInfo]
    ) -> DeclarationInfo:
    ...
    def _build_turn_history(
        ...
        for turn_data in turn_sequence:
            ...
            if turn_data.get("winner"):
                ...
                winner_play = next(
                    (p for p in plays if p.player_name == winner_name), None
                )
                if winner_play:
