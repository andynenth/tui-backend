...
class EventCompressor:
    ...
    def should_store_event(self, event_type: str) -> bool:
        ...
        importance = EventMapping.IMPORTANCE_SCORES.get(
            event_type, 
            EventImportance.LOW
        )
    ...
    def compress_event(self, room_id: str, event: Dict[str, Any]) -> Optional[CompressedEvent]:
        """
        ...
        logger.debug(f"Unknown event type for compression: {event_type}")
    ...
    def _accumulate_declaration(self, room_id: str, event: Dict[str, Any]) -> Optional[CompressedEvent]:
    ...
    def _accumulate_turn(self, room_id: str, event: Dict[str, Any]) -> None:
        ...
        turn_number = (
            payload.get("turn_number") or 
            payload.get("current_turn_number") or
            len(self._turn_accumulator[room_id]) + 1
        )
    ...
    def _compress_turn(self, room_id: str, event: Dict[str, Any]) -> Optional[CompressedEvent]:
        """Compress accumulated turn events when turn completes."""
        payload = event.get("payload", {})
        turn_number = (
            payload.get("turn_number") or 
            payload.get("current_turn_number") or
            len(self._turn_accumulator)
        )
    ...
    def _filter_redundant(self, room_id: str, event: Dict[str, Any]) -> Optional[CompressedEvent]:
    ...
    def flush_room_accumulators(self, room_id: str) -> List[CompressedEvent]:
        ...
        # Flush turn events
        turn_keys = [k for k in self._turn_accumulator.keys() if k.startswith(f"{room_id}:turn_")]
