...
from backend.services.game_cache import GameCache, initialize_game_cache, shutdown_game_cache
from backend.services.cached_event_store import CachedEventStore
from backend.services.historical_writer import HistoricalWriter, initialize_historical_writer, shutdown_historical_writer
...
class RealtimeGameSystem:
    ...
    def __init__(self):
        ...
        self.cache_enabled = os.getenv("REALTIME_CACHE_ENABLED", "true").lower() == "true"
        self.async_writes = os.getenv("ASYNC_HISTORICAL_WRITES", "true").lower() == "true"
