...
class EventBuffer:
    ...
    def __init__(
        self, max_size: int = 20, flush_interval: float = 2.0, event_store=None
    ):
        """
        ...
        logger.info(
            f"EventBuffer initialized (size: {max_size}, interval: {flush_interval}s)"
        )
    ...
    async def shutdown(self) -> None:
        ...
        logger.info(
            f"EventBuffer shutdown - flushed {self._metrics['events_flushed']} total events"
        )
