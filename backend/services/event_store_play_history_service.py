...
class EventStorePlayHistoryService:
    ...
    def _extract_initial_state(self, events: List[GameEvent], round_num: int) -> InitialState:
        ...
        # Final fallback: extract from first turn plays
        if not player_order:
            first_turn_players = []
            for event in events:
                if event.event_type == "action_processed" and event.payload.get("action_type") in ["play_pieces", "play"]:
                    player_name = event.payload.get("player_name")
                    if player_name and player_name not in first_turn_players:
                        first_turn_players.append(player_name)
                    if len(first_turn_players) >= 4:  # Assuming 4 players
                        break
            if first_turn_players:
                player_order = first_turn_players
    ...
    def _extract_declaration_phase(
        ...
        # Extract declarations from action_processed events
        declaration_map = {}
        for event in events:
            if event.event_type == "action_processed":
                if event.payload.get("action_type") == "declare":
                    player_name = event.payload.get("player_name")
                    value = event.payload.get("payload", {}).get("value", 0)
                    declaration_map[player_name] = value
    ...
    def _extract_turn_history(
        ...
        for event in events:
            if event.event_type == "action_processed" and event.payload.get("action_type") == "declare":
                player_name = event.payload.get("player_name")
                declared = event.payload.get("payload", {}).get("value", 0)
                turn_declarations[player_name] = declared
