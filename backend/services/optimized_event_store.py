...
class OptimizedEventStore:
    ...
    def __init__(self, db_path: Optional[str] = None):
        ...
        compression_enabled = os.getenv("EVENT_COMPRESSION_ENABLED", "true").lower() == "true"
        ...
        if compression_enabled:
            ...
            logger.info(f"OptimizedEventStore: Compression enabled (threshold: {importance_threshold})")
        else:
        ...
        if buffer_enabled:
            ...
            logger.info(f"OptimizedEventStore: Buffer enabled (size: {buffer_size}, interval: {buffer_interval}s)")
        else:
            self.buffer = None
        ...
        logger.info("OptimizedEventStore initialized - full optimization pipeline active")
    ...
    async def store_event(
        ...
        if self.compressor:
            ...
            compressed = self.compressor.compress_event(room_id, {
    ...
    async def store_event_direct(
        ...
        try:
            ...
            if event_type == SemanticEventType.GAME_STARTED.value:
                ...
                logger.info(f"Storing round_started event for room {room_id}, round {round_number}")
