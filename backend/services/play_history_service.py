...
class PlayHistoryService:
    ...
    async def build_play_history(self, game: Game, room_id: str, include_ai_analysis: bool = True, format: Optional[str] = None) -> PlayHistoryResponse:
        ...
        try:
            ...
            play_history = await self.event_store_service.build_play_history_from_events(
                room_id=room_id,
        ...
        for round_num in range(1, game.round_number + 1):
            if round_num < game.round_number or (game.current_phase == "SCORING" if game.current_phase else False):
                # Only include completed rounds or current round if in SCORING phase
                try:
                    if format == "compact":
                        round_history = self.build_compact_round_history(game, round_num)
                    else:
    ...
    def build_compact_round_history(self, game: Game, round_number: int) -> RoundHistory:
    ...
    def extract_player_info(self, game: Game) -> Dict[str, PlayerInfo]:
        ...
        for player in game.players:
            ...
            player_id = player_id.replace("/", "_").replace("@", "_").replace("#", "_").replace("$", "_")
    ...
    def extract_initial_state(self, game: Game, round_number: int) -> InitialState:
        ...
        if round_number == 1:
            ...
            try:
                has_general = starter_player.has_red_general() if hasattr(starter_player, 'has_red_general') else False
    ...
    def extract_hands_dealt(self, game: Game, round_number: int) -> Dict[str, List[PieceInfo]]:
        ...
        if round_number == game.round_number and game.players[0].hand:
            ...
            elif hasattr(game, 'initial_hands') and isinstance(game.initial_hands, dict):
    ...
    def sort_hand(self, hand: List[Piece]) -> List[Piece]:
        ...
        sorted_hand.sort(key=lambda piece: (
            0 if piece.color == "RED" else 1,  # RED pieces get priority (lower sort value)
            -piece.point  # Negative for descending order (14 before 2)
        ))
        
        return sorted_hand
    
    def extract_declaration_phase(self, game: Game, round_number: int) -> DeclarationInfo:
        ...
        for position, player in enumerate(player_order):
            ...
            if position == 0:
                ...
                if has_general_red and position > 0:
                    ...
                    pile_room = max(0, 8 - game_declarations.get(player_order[0].name, 0))
                else:
            ...
            if hasattr(self, 'include_ai_analysis') and self.include_ai_analysis and player.is_bot:
    ...
    def extract_turn_history(self, game: Game, round_number: int) -> List[TurnInfo]:
        ...
        if not hasattr(game, 'turn_history_this_round') or not game.turn_history_this_round:
            return []
        ...
        for turn_idx, turn_data in enumerate(game.turn_history_this_round):
            ...
            for i, turn_play in enumerate(plays):
                ...
                if 'initial_hands' in turn_data and player.name in turn_data['initial_hands']:
                    hand_before = self.sort_hand(turn_data['initial_hands'][player.name])
                ...
                if 'hands_after_play' in turn_data and player.name in turn_data['hands_after_play']:
                    hand_after = self.sort_hand(turn_data['hands_after_play'][player.name])
                elif hasattr(player, 'hand'):
                ...
                if hasattr(self, 'include_ai_analysis') and self.include_ai_analysis and player.is_bot:
                    turn_reasoning = self.extract_ai_turn_reasoning(player.name, turn_number, game)
                    if turn_reasoning:
    ...
    def extract_round_summary(self, game: Game, round_number: int) -> RoundSummary:
            ...
            for player_name in declarations:
                ...
                for r in range(1, round_number + 1):
                    if hasattr(game, 'round_history') and r in game.round_history:
                        ...
                        cumulative += game.round_history[r].get('scores', {}).get(player_name, 0)
        ...
        for player in game.players:
            ...
            cumulative_scores[player.name] = player.score if hasattr(player, 'score') else 0
    ...
    def extract_ai_analysis_for_play(self, play_data: dict, game: Game) -> Optional[dict]:
        """Extract AI decision analysis for a play if player is AI."""
        # Check if player is AI
        if not play_data.get('is_ai', True):  # Default to True for backward compatibility
            return None
    ...
    def extract_ai_declaration_reasoning(self, player_name: str, game: Game) -> Optional[dict]:
    ...
    def extract_ai_turn_reasoning(self, player_name: str, turn_number: int, game: Game) -> Optional[dict]:
    ...
    def include_ai_analysis(self, play_data: PlayData, player_id: str, game: Game) -> PlayData:
