...
class RateLimitConfig:
    ...
    debug_mode: bool = field(default_factory=lambda: os.getenv("RATE_LIMIT_DEBUG", "false").lower() == "true")
    
    # Global settings
    global_requests_per_minute: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_GLOBAL_RPM", "100")))
    burst_multiplier: float = field(default_factory=lambda: float(os.getenv("RATE_LIMIT_BURST_MULTIPLIER", "1.5")))
    block_duration_seconds: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_BLOCK_DURATION", "300")))
    
    # REST API limits
    health_endpoint_rpm: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_HEALTH_RPM", "60")))
    rooms_endpoint_rpm: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_ROOMS_RPM", "30")))
    recovery_endpoint_rpm: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_RECOVERY_RPM", "10")))
    
    # WebSocket limits - System events
    ws_ping_rpm: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_WS_PING_RPM", "120")))
    ws_ack_rpm: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_WS_ACK_RPM", "200")))
    ws_sync_rpm: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_WS_SYNC_RPM", "10")))
    
    # WebSocket limits - Lobby events
    ws_room_list_rpm: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_WS_ROOM_LIST_RPM", "30")))
    ws_create_room_rpm: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_WS_CREATE_ROOM_RPM", "5")))
    ws_join_room_rpm: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_WS_JOIN_ROOM_RPM", "10")))
    
    # WebSocket limits - Game events
    ws_declare_rpm: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_WS_DECLARE_RPM", "5")))
    ws_play_rpm: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_WS_PLAY_RPM", "20")))
    ws_redeal_rpm: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_WS_REDEAL_RPM", "3")))
    ws_start_game_rpm: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_WS_START_GAME_RPM", "3")))
    
    # Room flood protection
    room_flood_threshold: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_ROOM_FLOOD_THRESHOLD", "1000")))
    
    # Grace period settings
    grace_warning_threshold: float = field(default_factory=lambda: float(os.getenv("RATE_LIMIT_GRACE_WARNING", "0.8")))
    grace_multiplier: float = field(default_factory=lambda: float(os.getenv("RATE_LIMIT_GRACE_MULTIPLIER", "1.2")))
    grace_duration_seconds: int = field(default_factory=lambda: int(os.getenv("RATE_LIMIT_GRACE_DURATION", "30")))
    ...
    def get_rest_api_rules(self) -> Dict[str, RateLimitRule]:
        ...
        if not self.enabled:
            ...
            return {
                ...
                "/api/health/detailed": RateLimitRule(requests=99999, window_seconds=60),
        ...
        return {
            ...
            "global": RateLimitRule(
                ...
                window_seconds=60,
                burst_multiplier=self.burst_multiplier * 1.5  # More lenient global burst
            )
        }
    ...
    def get_websocket_rules(self) -> Dict[str, RateLimitRule]:
        ...
        if not self.enabled:
            # Return very high limits when disabled
            return {event: RateLimitRule(requests=99999, window_seconds=60) 
        ...
        return {
            ...
            # Lobby events
            "request_room_list": RateLimitRule(requests=self.ws_room_list_rpm, window_seconds=60),
            "get_rooms": RateLimitRule(requests=self.ws_room_list_rpm, window_seconds=60),
            "create_room": RateLimitRule(
                requests=self.ws_create_room_rpm,
                window_seconds=60,
                block_duration_seconds=self.block_duration_seconds
            ),
            "join_room": RateLimitRule(requests=self.ws_join_room_rpm, window_seconds=60),
            
            # Room management
            "get_room_state": RateLimitRule(requests=self.rooms_endpoint_rpm * 2, window_seconds=60),
