...
async def test_create_room_with_bypass():
    ...
    async with websockets.connect(WS_URL_LOBBY) as websocket:
        ...
        if (
            result["event"] == "error"
            and "Player name is required" in result["data"]["message"]
        ):
            ...
            logger.warning(
                "❌ Validation bypass might be TOO permissive - room created without player_name"
            )
            return result["data"]["room_id"], result["data"]["room_code"]
...
async def test_game_events_with_bypass(room_id):
    ...
    async with websockets.connect(WS_URL_ROOM.format(room_id=room_id)) as websocket:
        ...
        play_msg = {
            "event": "play",
            "data": {"game_id": room_id, "pieces": [{"value": 5, "kind": "black"}]},
        }
        ...
        redeal_msg = {
            ...
            "data": {"game_id": room_id, "hand_strength_score": 7},
        }
...
async def test_adapter_events():
    ...
    async with websockets.connect(WS_URL_LOBBY) as websocket:
        ...
        bad_msg = {"event": "unknown_event", "data": {}}
        ...
        if (
            result["event"] == "error"
            and "Unknown event type" in result["data"]["message"]
        ):
