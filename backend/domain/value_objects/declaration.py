...
class Declaration:
    ...
    def __post_init__(self):
        ...
        if not isinstance(self.pile_count, int):
            raise ValueError(
                f"Pile count must be an integer, got {type(self.pile_count)}"
            )

        if self.pile_count < 0 or self.pile_count > 8:
            raise ValueError(
                f"Pile count must be between 0 and 8, got {self.pile_count}"
            )
    ...
    @classmethod
    def create(
        cls, player_name: str, pile_count: int, is_forced: bool = False
    ) -> "Declaration":
        """
        ...
        """
        return cls(player_name=player_name, pile_count=pile_count, is_forced=is_forced)
...
class DeclarationSet:
    ...
    def from_dict(cls, data: dict) -> "DeclarationSet":
        """Create from dictionary."""
        declarations = [Declaration.from_dict(d) for d in data["declarations"]]
