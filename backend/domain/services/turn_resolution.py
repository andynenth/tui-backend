...
class TurnResult:
    ...
    def get_play_summary(self) -> Dict[str, List[str]]:
        ...
        return {play.player_name: [p.kind for p in play.pieces] for play in self.plays}
...
class TurnResolutionService:
    ...
    def validate_turn_plays(
        plays: List[TurnPlay], required_piece_count: Optional[int] = None
    ) -> Dict[str, str]:
        """
        ...
        for play in plays:
            if not play.is_valid():
                validation_results[play.player_name] = (
                    f"Invalid play type: {play.play_type}"
                )
