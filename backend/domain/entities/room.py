...
class HostMigrated(GameEvent):
    ...
    def _get_event_data(self) -> Dict:
        data = super()._get_event_data()
        data.update({"old_host": self.old_host, "new_host": self.new_host})
        return data
...
class GameStartedInRoom(GameEvent):
    ...
    def _get_event_data(self) -> Dict:
        data = super()._get_event_data()
        data.update({"player_names": self.player_names})
        return data
...
class Room:
    ...
    # Room state
    slots: List[Optional[Player]] = field(
    ...
    def __post_init__(self):
        ...
        logger.info(
            f"[ROOM_DEBUG] Creating room {self.room_id} with {self.max_slots} slots"
        )
        ...
        logger.info(
            f"[ROOM_DEBUG] Room initialization complete. Slots: {[p.name if p else 'empty' for p in self.slots]}"
        )
    ...
    def add_player(
        self, name: str, is_bot: bool = False, slot: Optional[int] = None
    ) -> int:
        """
        ...
        if slot is not None:
            ...
            logger.info(
                f"[ROOM_DEBUG] Added player {name} (bot={is_bot}) to slot {slot} in room {self.room_id}"
            )
    ...
    def _update_room_status(self) -> None:
        ...
        elif all(p is not None for p in self.slots) and self.status in [
            RoomStatus.WAITING,
            RoomStatus.READY,
        ]:
        ...
        elif any(p is None for p in self.slots) and self.status in [
            RoomStatus.READY,
            RoomStatus.WAITING,
        ]:
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "slots": [
                {"index": i, "player": p.to_dict() if p else None}
