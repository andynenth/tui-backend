...
async def test_logging_detailed_scenario():
    ...
    async def monitored_check_transition(self):
        ...
        print(f"🔄 {elapsed:.2f}s - Transition check: scores={self.scores_calculated}, "
    ...
    if scoring_state:
        ...
        scoring_state.check_transition_conditions = lambda: monitored_check_transition(scoring_state)
...
async def main():
    ...
    try:
        ...
        if scenario_1 == "game_complete_blocks_transition":
            print("❌ CRITICAL: When game is complete (50+ points), scoring phase never transitions!")
