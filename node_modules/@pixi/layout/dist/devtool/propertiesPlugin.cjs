...
function updateNodeFlex(props, node) {
  node.layout = props;
}
function createPropertyEntry(propBase, label, section) {
  return {
  ...
  getProperties(container) {
    const activeProps = properties().reduce((result, property) => {
      ...
      if (flexProp === "_computedBounds") {
        ...
        const { x, y, offsetX, offsetY, scaleX, scaleY, originX, originY } = container.layout._computedPixiLayout;
...
//# sourceMappingURL=propertiesPlugin.cjs.map
