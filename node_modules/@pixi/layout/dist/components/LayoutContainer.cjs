...
class LayoutContainer extends pixi_js.Container {
  constructor(params = {}) {
    ...
    this.background = background ?? new pixi_js.Graphics({ label: "background" });
  ...
  _drawBackground(computedLayout) {
    var _a;
    const borderWidth = this.layout.yoga.getBorder(load.Edge.All);
    const boxSizing = this.layout.yoga.getBoxSizing();
    const alignment = boxSizing === load.BoxSizing.BorderBox ? 1 : 0;
    const layoutStyles = this.layout.style;
    ...
    if (overflow !== "visible") {
      ...
      const borderOffset = boxSizing === load.BoxSizing.BorderBox ? borderWidth : 0;
      setTimeout(() => {
...
//# sourceMappingURL=LayoutContainer.cjs.map
